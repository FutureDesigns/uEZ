...
UINT32 HTTPIntrnConnectionClose (P_HTTP_SESSION pHTTPSession)
{
    ...
    do
    {
        ...
        if(pHTTPSession->HttpConnection.HttpSocket != HTTP_INVALID_SOCKET)// INVALID_SOCKET
        {
            ...
            UEZNetworkSocketDelete(pHTTPSession->HttpConnection.HttpNetwork, pHTTPSession->HttpConnection.HttpSocket);
...
UINT32 HTTPIntrnConnectionOpen (P_HTTP_SESSION pHTTPSession)
{
    ...
    T_uezNetworkAddr ipAddress = {0,0,0,0};
    TUInt16          port;
    ...
    do
    {
        ...
        if(pHTTPSession->HttpConnection.HttpSocket == HTTP_INVALID_SOCKET)
        {
            ...
            UEZNetworkOpen("WirelessNetwork0", &pHTTPSession->HttpConnection.HttpNetwork);

            // Get the wired network
            //UEZNetworkOpen("WiredNetwork0", &pHTTPSession->HttpConnection.HttpNetwork);

            // Create a TCP/IP socket
            UEZNetworkSocketCreate(pHTTPSession->HttpConnection.HttpNetwork, UEZ_NETWORK_SOCKET_TYPE_TCP, &pHTTPSession->HttpConnection.HttpSocket);       
        }
        ...
        // always non-blocking in uEZNetwork
        ...
        if((pHTTPSession->HttpFlags & HTTP_CLIENT_FLAG_USINGPROXY) != HTTP_CLIENT_FLAG_USINGPROXY)
        {
            ...
            port = pHTTPSession->HttpUrl.nPort;       // Host Port number   
        }
        else
        {
            // Use the proxy port
            port = pHTTPSession->HttpProxy.nProxyPort;  // Proxy Port number 
        }
        ...
        if(pHTTPSession->HttpConnection.HttpClientPort != 0)
        {
            ...
            LocalAddress.sin_port           = htons((unsigned short)pHTTPSession->HttpConnection.HttpClientPort);
        ...
        nRetCode = UEZNetworkSocketConnect(pHTTPSession->HttpConnection.HttpNetwork,
                                               pHTTPSession->HttpConnection.HttpSocket, 
                                               &ipAddress, port);
        
        if(nRetCode == 0)
        {      
            // Set TLS Nego flag to false
                      ...
                      UINT32 *nLength)        // [IN OUT] Length of data to send and the transmitted bytes count
{
    ...
    do
    {
        ...
        {           
            ...
            else
            {
                ...
                if (UEZNetworkSocketWrite(pConnection->HttpNetwork, pConnection->HttpSocket, 
                               (void *)pData, *(nLength), TRUE, UEZ_TIMEOUT_INFINITE) != UEZ_ERROR_NONE)
                {
    ...
    if(pHTTPSession->pDebug)
    {
        ...
        {
            pHTTPSession->pDebug("HTTPIntrnSend", "", 0, "");// pData, *(nLength), "");
        }
    }
#endif
...
// Purpose      : Receive data from the connected socke
// Returns      : HTTP Status
// Last updated : 01/09/2005
//
///////////////////////////////////////////////////////////////////////////////
    ...
    do
    {
        ...
        while(1)
        {
            ...
            {     
                ...
                {
                    nRetCode = UEZNetworkSocketRead(pConnection->HttpNetwork, pConnection->HttpSocket, 
                                                 pData, *(nLength), &numRead, 500);
                    if((nRetCode != UEZ_ERROR_NONE) && (nRetCode != UEZ_ERROR_TIMEOUT))
                    {
                        // Socket error
                        nRetCode =  HTTP_CLIENT_ERROR_SOCKET_RECV;
                        break;
            ...
            if ( (numRead == 0) && (nRetCode != UEZ_ERROR_TIMEOUT)/*|| nRetCode == HTTP_ECONNRESET*/)
            {        
    ...
    *(nLength) = totalRead;
    ...
    return nRetCode;
                                                ...
                                                UINT32 HTTPIntrnConnectionOpen (P_HTTP_SESSION pHTTPSession)
                                                {
                                                    ...
                                                    HTTP_SOCKADDR_IN LocalAddress;                       // Socket address structure (for client binding)
                                                    ...
                                                    do
                                                    {
                                                        ...
                                                        */
                                                        ...
                                                        memset(&LocalAddress, 0, sizeof(HTTP_SOCKADDR_IN)); 
