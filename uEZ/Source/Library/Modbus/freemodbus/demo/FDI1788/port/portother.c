...
/* ----------------------- Defines ------------------------------------------*/
#define MB_FRAME_LOG_BUFSIZE    768

/* ----------------------- Start implementation -----------------------------*/

#ifdef MB_TCP_DEBUG
static T_uezSemaphore      aFrameLogSem;
static BOOL bFrameLogSemInit = FALSE;

static T_uezSemaphore      aLogSem;
static BOOL bLogSemInit = FALSE;
#endif
...
prvvMBTCPLogFrame( UCHAR * pucMsg, UCHAR * pucFrame, USHORT usFrameLen )
{
    ...
    if (bFrameLogSemInit == FALSE)
    {
        UEZSemaphoreCreateBinary(&aFrameLogSem);
        bFrameLogSemInit = TRUE;
    }

    UEZSemaphoreGrab(aFrameLogSem, UEZ_TIMEOUT_INFINITE);
    ...
    if( iBufLeft )
    {
        ...
        {
            vMBPortLog( MB_LOG_DEBUG, (const CHAR *) pucMsg, "%s", "Frame not decoded, decode buffer too small\r\n" );
        }
    }

    UEZSemaphoreRelease(aFrameLogSem);
}
#endif
...
vMBPortLog( eMBPortLogLevel eLevel, const CHAR * szModule, const CHAR * szFmt, ... )
{
    ...
    if (bLogSemInit == FALSE)
    {
        UEZSemaphoreCreateBinary(&aLogSem);
        bLogSemInit = TRUE;
    }

    UEZSemaphoreGrab(aLogSem, UEZ_TIMEOUT_INFINITE);
