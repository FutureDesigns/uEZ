...
int32_t DeriveKeys(CYASSL* ssl)
{
    ...
    byte   md5Input[(uint32_t)SECRET_LEN + (uint32_t)SHA_DIGEST_SIZE];
    byte   shaInput[(uint32_t)KEY_PREFIX + (uint32_t)SECRET_LEN + 2 * (uint32_t)RAN_LEN];
    byte   keyData[(uint32_t)KEY_PREFIX * (uint32_t)MD5_DIGEST_SIZE];
    Md5    md5[1];
    Sha    sha[1];
#endif
    ...
    if (ret == 0) {
        ...
        for (i = 0; i < rounds; ++i) {
            ...
            ShaUpdate(sha, shaInput, ((uint32_t)KEY_PREFIX + (uint32_t)SECRET_LEN + 2 * (uint32_t)RAN_LEN)
                                                              - (uint32_t)KEY_PREFIX + j);
            ShaFinal(sha, shaOutput);

            XMEMCPY(md5Input + SECRET_LEN, shaOutput, SHA_DIGEST_SIZE);
            Md5Update(md5, md5Input, (uint32_t)SECRET_LEN + (uint32_t)SHA_DIGEST_SIZE);
...
#else
    byte   shaOutput[SHA_DIGEST_SIZE];
    byte   md5Input[(uint32_t)ENCRYPT_LEN + (uint32_t)SHA_DIGEST_SIZE];
    byte   shaInput[(uint32_t)PREFIX + (uint32_t)ENCRYPT_LEN + 2 * (uint32_t)RAN_LEN];
    Md5    md5[1];
    Sha    sha[1];
#endif
