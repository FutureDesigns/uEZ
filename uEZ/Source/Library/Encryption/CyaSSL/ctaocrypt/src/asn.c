...
{
    output[0] = (uint32_t)ASN_SEQUENCE | (uint32_t)ASN_CONSTRUCTED;
...
{
    output[0] = (uint32_t)ASN_SET | (uint32_t)ASN_CONSTRUCTED;
    return SetLength(len, output + 1) + 1;
}
...
static int32_t DecodeSubtree(byte* input, int32_t sz, Base_entry** head, void* heap)
{
    ...
    while (idx < (word32)sz) {
        ...
        if (b == ((uint32_t)ASN_CONTEXT_SPECIFIC | (uint32_t)ASN_DNS_TYPE) ||
            b == ((uint32_t)ASN_CONTEXT_SPECIFIC | (uint32_t)ASN_RFC822_TYPE) ||
            b == ((uint32_t)ASN_CONTEXT_SPECIFIC | (uint32_t)ASN_CONSTRUCTED | (uint32_t)ASN_DIR_TYPE)) {
