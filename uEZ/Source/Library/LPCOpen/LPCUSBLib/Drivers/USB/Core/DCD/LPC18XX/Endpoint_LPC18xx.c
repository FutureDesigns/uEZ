...
extern void EVENT_USB_Device_IRQActivity(void);
...
void HAL_Reset(uint8_t corenum)
{
	...
	USB_Reg->USBINTR_D =  USBINTR_D_UsbIntEnable | USBINTR_D_UsbErrorIntEnable |
									 USBINTR_D_PortChangeIntEnable | USBINTR_D_UsbResetEnable |
                                                                           USBINTR_D_SuspendEnable | USBINTR_D_NAKEnable /* LES:TESITNG | USBINTR_D_SofReceivedEnable*/;
...
void DcdIrqHandler(uint8_t corenum)
{
	...
	if (USBSTS_D & USBSTS_D_SuspendInt) {					/* Suspend */
	    // Suspend causes IND1 to no longer activate
	    // TODO: Currently turns ON the indicator
        USB_Reg->PORTSC1_D &= ~(1<<14);
        flagActivity = 1;
	}
