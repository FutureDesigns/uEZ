...
LPC_USBHS_T * const USB_REG_BASE_ADDR[LPC18_43_MAX_USB_CORE] = {(LPC_USBHS_T*)LPC_USB0, (LPC_USBHS_T*)LPC_USB1};
...
void HAL_USBForceFullSpeed(uint8_t corenum, uint32_t onoff)
{
    if (corenum == 0) {
        if (onoff) {
            LPC_USB0->PORTSC1_D |= (1 << 24);
...
void LPCUSBLib_USB1_IRQHandler(void)
{
