...
#ifndef UNUSED
/**
 * This macro can be used to suppress warnings about unused
 * variables.
 */
		...
		taskENTER_CRITICAL();
		{
			...
			if( uxCurrentBasePriority != uxNewPriority )
			{
				...
				if( listIS_CONTAINED_WITHIN( &( pxReadyTasksLists[ uxPriorityUsedOnEntry ] ), &( pxTCB->xStateListItem ) ) != pdFALSE )
				{
					...
					prvReaddTaskToReadyList( pxTCB ); // SystemView
				}
				else
				{
					mtCOVERAGE_TEST_MARKER();
		...
		taskENTER_CRITICAL();
		{
			...
			else
			{
				mtCOVERAGE_TEST_MARKER();
          		...
          		traceMOVED_TASK_TO_SUSPENDED_LIST(pxTCB); // SystemView
...
const TickType_t xConstTickCount = xTickCount;
	...
	#if ( INCLUDE_vTaskSuspend == 1 )
	{
		if( ( xTicksToWait == portMAX_DELAY ) && ( xCanBlockIndefinitely != pdFALSE ) )
		{
			...
			indefinitely. */
			traceMOVED_TASK_TO_SUSPENDED_LIST(pxCurrentTCB); // SystemView
		...
		else
		{
			...
			{
                                traceMOVED_TASK_TO_OVERFLOW_DELAYED_LIST(); // System View
			...
			{
                                traceMOVED_TASK_TO_DELAYED_LIST(); // System View
	...
	#else /* INCLUDE_vTaskSuspend */
	{
		...
		if( xTimeToWake < xConstTickCount )
		{
                        traceMOVED_TASK_TO_OVERFLOW_DELAYED_LIST(); // SystemView
			/* Wake time has overflowed.  Place this item in the overflow list. */
			vListInsert( pxOverflowDelayedTaskList, &( pxCurrentTCB->xStateListItem ) );
		}
		else
		{
                        traceMOVED_TASK_TO_DELAYED_LIST(); // SystemView
