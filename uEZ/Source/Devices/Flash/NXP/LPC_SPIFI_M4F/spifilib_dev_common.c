...
SPIFI_ERR_T spifiDevInit(const SPIFI_HANDLE_T *pHandle)
{
    ...
    //spifiDevSetMemMode(pHandle, 0); // called in spifiDevSetOpts after options are set
...
SPIFI_HANDLE_T *spifiInitDevice(void *pMem, uint32_t sizePMem, uint32_t spifiCtrlAddr, uint32_t baseAddr)
{
    ...
    // Set SPIFI options such as quad mode, calls spifiDevSetMemMode near end of function.
    spifiDevSetOpts(pSpifiHandle, (SPIFI_CAP_QUAD_READ | SPIFI_CAP_QUAD_WRITE), 1);
