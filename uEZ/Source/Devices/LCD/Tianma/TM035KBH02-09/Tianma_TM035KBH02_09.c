/*-------------------------------------------------------------------------*
 * File:  Tianma_TM035KBH02_09.c
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the Tianma TM035KBH02_09.
 *-------------------------------------------------------------------------*/
 ...
 * Globals:
 *---------------------------------------------------------------------------*/
static T_LCDControllerSettings LCD_TM035KBH02_09_settings;

static const T_LCDControllerSettings LCD_TM035KBH02_09_params16bit = {
    LCD_ADVANCED_TFT,
...
static const T_LCDControllerSettings LCD_TM035KBH02_09_paramsI15bit = {
    LCD_ADVANCED_TFT,
...
static const T_LCDControllerSettings LCD_TM035KBH02_09_params8bit = {
    LCD_ADVANCED_TFT,
    LCD_COLOR_RES_8,
...
static T_uezLCDConfiguration LCD_TM035KBH02_09_configuration_16Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_TM035KBH02_09_configuration_I15Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_TM035KBH02_09_configuration_8Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
 ...
 * Prototypes:
 *---------------------------------------------------------------------------*/
extern const DEVICE_LCD LCD_Tianma_TM035KBH02_09_Interface;

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TM035KBH02_09_InitializeWorkspace_16Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for TM035KBH02_09 LCD.
 * Inputs:
 *      void *aW                    -- Particular workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_TM035KBH02_09_InitializeWorkspace_16Bit(void *aW)
{
    T_TM035KBH02_09Workspace *p = (T_TM035KBH02_09Workspace *)aW;
    p->iBaseAddress = LCD_DISPLAY_BASE_ADDRESS;
    p->aNumOpen = 0;
    p->iBacklightLevel = 0; // 0%
    p->iConfiguration = &LCD_TM035KBH02_09_configuration_16Bit;
    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TM035KBH02_09_InitializeWorkspace_I15Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for TM035KBH02_09 LCD.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_TM035KBH02_09_InitializeWorkspace_I15Bit(void *aW)
{
    T_TM035KBH02_09Workspace *p = (T_TM035KBH02_09Workspace *)aW;
    p->iBaseAddress = LCD_DISPLAY_BASE_ADDRESS;
    p->aNumOpen = 0;
    p->iBacklightLevel = 0; // 0%
    p->iConfiguration = &LCD_TM035KBH02_09_configuration_I15Bit;
    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TM035KBH02_09_InitializeWorkspace_8Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for TM035KBH02_09 LCD.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_TM035KBH02_09_InitializeWorkspace_8Bit(void *aW)
{
    T_TM035KBH02_09Workspace *p = (T_TM035KBH02_09Workspace *)aW;
    p->iBaseAddress = LCD_DISPLAY_BASE_ADDRESS;
    p->aNumOpen = 0;
    p->iBacklightLevel = 0; // 0%
    p->iConfiguration = &LCD_TM035KBH02_09_configuration_8Bit;
    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TM035KBH02_09_SetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TM035KBH02_09_SetBacklightLevel(void *aW, TUInt32 aLevel)
{
    T_TM035KBH02_09Workspace *p = (T_TM035KBH02_09Workspace *)aW;
    TUInt16 level;
 ...
 *                                  UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError ISPIConfigure(T_TM035KBH02_09Workspace *p)
{
    T_uezError error = UEZ_ERROR_NONE;
    if(strcmp(p->iSPIBus, "\0") == 0){
        //See LCD_TM035KBH02_09_Create to fix.
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TM035KBH02_09_MSTimerStart
 *---------------------------------------------------------------------------*
 * Description:
...
*---------------------------------------------------------------------------*/
//static T_uezError LCD_TM035KBH02_09_MSTimerStart(void *aW, float milliseconds){
//  T_TM035KBH02_09Workspace *p = (T_TM035KBH02_09Workspace *)aW;
//  T_uezError error = UEZ_ERROR_NONE;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TM035KBH02_09_TimerCallback
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezTimerCallback *aCallbackWorkspace  -- Workspace
 *---------------------------------------------------------------------------*/
//static void LCD_TM035KBH02_09_TimerCallback(T_uezTimerCallback *aCallbackWorkspace){
//  T_TM035KBH02_09Workspace *p = aCallbackWorkspace->iData;
//  p->itimerDone = ETrue;
//  UEZTimerClose(p->itimer);
//}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TM035KBH02_09_On
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TM035KBH02_09_On(void *aW) {
    T_TM035KBH02_09Workspace *p = (T_TM035KBH02_09Workspace *)aW;
    T_uezError error = UEZ_ERROR_NONE;

    //if (UEZTimerOpen("Timer0", &p->itimer) == UEZ_ERROR_NONE) {
        //LCD_TM035KBH02_09_MSTimerStart(p, 1.25); // 1.25ms itimer, minimum 1ms needed
        ...
        //LCD_TM035KBH02_09_MSTimerStart(p, 6.25);// 6.25ms itimer, minimum 5ms
        //while (p->itimerDone == EFalse){;} // wait for timer before starting next stage
        UEZTaskDelay(6);

        //LCD_TM035KBH02_09_MSTimerStart(p, 184.0); // minimum 166ms timer, 184ms = 11 frame skip
        ...
        if(p->iBacklight){
            ...
            LCD_TM035KBH02_09_SetBacklightLevel(p, p->iBacklightLevel);
        }
      //}
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TM035KBH02_09_Off(void *aW)
{
    T_TM035KBH02_09Workspace *p = (T_TM035KBH02_09Workspace *)aW;
    ...
    if (p->iBacklight){ // Turn off backlight
      ...
      //if (UEZTimerOpen("Timer0", &p->itimer) == UEZ_ERROR_NONE) {
      //  LCD_TM035KBH02_09_MSTimerStart(p, 166.6); // minimum 166.6ms timer
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TM035KBH02_09_Open
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TM035KBH02_09_Open(void *aW)
{
  T_TM035KBH02_09Workspace *p = (T_TM035KBH02_09Workspace *)aW;
  ...
  //p->icallback.iCallback = LCD_TM035KBH02_09_TimerCallback;
  ...
  if (p->aNumOpen == 1) {
    plcdc = p->iLCDController;
      ...
      case UEZLCD_COLOR_DEPTH_8_BIT:
        LCD_TM035KBH02_09_settings = LCD_TM035KBH02_09_params8bit;
        break;
      default:
      case UEZLCD_COLOR_DEPTH_16_BIT:
        LCD_TM035KBH02_09_settings = LCD_TM035KBH02_09_params16bit;
        break;
      case UEZLCD_COLOR_DEPTH_I15_BIT:
        LCD_TM035KBH02_09_settings = LCD_TM035KBH02_09_paramsI15bit;
        break;
      }
      LCD_TM035KBH02_09_settings.iBaseAddress = p->iBaseAddress;
        //pre-configure LCD controller first so that there is no delay for turn on
        error = (*plcdc)->Configure(plcdc, &LCD_TM035KBH02_09_settings);
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TM035KBH02_09_Close
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TM035KBH02_09_Close(void *aW)
{
    T_TM035KBH02_09Workspace *p = (T_TM035KBH02_09Workspace *)aW;
    p->aNumOpen--;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TM035KBH02_09_Configure
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_TM035KBH02_09_Configure(
            void *aW,
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TM035KBH02_09_GetInfo
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TM035KBH02_09_GetInfo(void *aW, T_uezLCDConfiguration *aConfiguration)
{
    T_TM035KBH02_09Workspace *p = (T_TM035KBH02_09Workspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TM035KBH02_09_GetFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TM035KBH02_09_GetFrame(
            void *aW,
            TUInt32 aFrame,
            void **aFrameBuffer)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TM035KBH02_09_ShowFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TM035KBH02_09_ShowFrame(void *aW, TUInt32 aFrame)
{
    T_TM035KBH02_09Workspace *p = (T_TM035KBH02_09Workspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TM035KBH02_09_GetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  no backlight for LCD.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TM035KBH02_09_GetBacklightLevel(
        void *aW, 
        TUInt32 *aLevel,
        TUInt32 *aNumLevels)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TM035KBH02_09_SetPaletteColor
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TM035KBH02_09_SetPaletteColor(
                    void *aWorkspace,
                    TUInt32 aColorIndex,
                    TUInt16 aRed,
                    TUInt16 aGreen,
                    TUInt16 aBlue)
{
    T_TM035KBH02_09Workspace *p = (T_TM035KBH02_09Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TM035KBH02_09_VerticalSyncCallback
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static void LCD_TM035KBH02_09_VerticalSyncCallback(void *aCallbackWorkspace)
{
    T_TM035KBH02_09Workspace *p = (T_TM035KBH02_09Workspace *)aCallbackWorkspace;

    _isr_UEZSemaphoreRelease(p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TM035KBH02_09_WaitForVerticalSync
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TM035KBH02_09_WaitForVerticalSync(
    void *aWorkspace,
    TUInt32 aTimeout)
{
    T_TM035KBH02_09Workspace *p = (T_TM035KBH02_09Workspace *)aWorkspace;
    ...
    (*p_hal)->EnableVerticalSync(p_hal, LCD_TM035KBH02_09_VerticalSyncCallback, p);
    return UEZSemaphoreGrab(p->iVSyncSem, aTimeout);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TM035KBH02_09_Create
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *
 *---------------------------------------------------------------------------*/
void LCD_Tianma_TM035NKH02_09_Create(const char *aName,
                            char* aSPIBus,
                            T_uezGPIOPortPin aSPICSPin)
{
    ...
    p = (T_TM035KBH02_09Workspace *)p_lcd;
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const DEVICE_LCD LCD_Tianma_TM035KBH02_09_Interface_16Bit = {
    {
        // Common interface
        "LCD:Tianma_TM035KBH02_09:16Bit",
        0x0100,
        LCD_TM035KBH02_09_InitializeWorkspace_16Bit,
        sizeof(T_TM035KBH02_09Workspace),
    },

    // Functions
    LCD_TM035KBH02_09_Open,
    LCD_TM035KBH02_09_Close,
    LCD_TM035KBH02_09_Configure,
    LCD_TM035KBH02_09_GetInfo,
    LCD_TM035KBH02_09_GetFrame,
    LCD_TM035KBH02_09_ShowFrame,
    LCD_TM035KBH02_09_On,
    LCD_TM035KBH02_09_Off,
    LCD_TM035KBH02_09_SetBacklightLevel,
    LCD_TM035KBH02_09_GetBacklightLevel,
    LCD_TM035KBH02_09_SetPaletteColor,

    // v2.04
    LCD_TM035KBH02_09_WaitForVerticalSync,
};

const DEVICE_LCD LCD_Tianma_TM035KBH02_09_Interface_I15Bit = {
    {
        // Common interface
        "LCD:Tianma_TM035KBH02_09:I15Bit",
        0x0100,
        LCD_TM035KBH02_09_InitializeWorkspace_I15Bit,
        sizeof(T_TM035KBH02_09Workspace),
    },
...
const DEVICE_LCD LCD_Tianma_TM035KBH02_09_Interface_8Bit = {
    {
        // Common interface
        "LCD:Tianma_TM035KBH02_09:8Bit",
        0x0100,
        LCD_TM035KBH02_09_InitializeWorkspace_8Bit,
        sizeof(T_TM035KBH02_09Workspace),
    },
...
// List of interfaces
const T_uezDeviceInterface *Tianma_TM035KBH02_09_InterfaceArray[] = {
    0,      // 1 bit
    0,      // 2 bit
    0,      // 4 bit
    (T_uezDeviceInterface *)&LCD_Tianma_TM035KBH02_09_Interface_8Bit,      // 8 bit
    (T_uezDeviceInterface *)&LCD_Tianma_TM035KBH02_09_Interface_16Bit,      // 16 bit
    (T_uezDeviceInterface *)&LCD_Tianma_TM035KBH02_09_Interface_I15Bit,      // I15 bit
    0,      // 24 bit
};

/*-------------------------------------------------------------------------*
 * End of File:  Tianma_TM035KBH02_09.c
 *-------------------------------------------------------------------------*/
