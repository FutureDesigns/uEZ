/*-------------------------------------------------------------------------*
 * File:  DLC0430BCP12RF.c
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the DLC0430BCP12RF.
 *-------------------------------------------------------------------------*/
...
} T_DLC0430BCP12RF_Workspace;

/*---------------------------------------------------------------------------*
 * Globals:
 *---------------------------------------------------------------------------*/
static T_LCDControllerSettings LCD_DLC0430BCP12RF_settings;
#if (LCD_BACKLIGHT_FREERTOS_TIMER==1)
static TimerHandle_t lcdBacklightDelayTimer = NULL;
#else
#endif

static const T_LCDControllerSettings LCD_DLC0430BCP12RF_params16bit = {
    LCD_ADVANCED_TFT,
...
static const T_LCDControllerSettings LCD_DLC0430BCP12RF_paramsI15bit = {
    LCD_ADVANCED_TFT,
...
static const T_LCDControllerSettings LCD_DLC0430BCP12RF_params8bit = {
    LCD_ADVANCED_TFT,
    LCD_COLOR_RES_8,
...
static T_uezLCDConfiguration LCD_DLC0430BCP12RF_configuration_16Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_DLC0430BCP12RF_configuration_I15Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_DLC0430BCP12RF_configuration_8Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
 ...
 * Prototypes:
 *---------------------------------------------------------------------------*/
extern const DEVICE_LCD LCD_DLC0430BCP12RF_Interface;

/*---------------------------------------------------------------------------*
 * Routine:  LCD_DLC0430BCP12RF_InitializeWorkspace_16Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for DLC0430BCP12RF LCD.
 * Inputs:
 *      void *aW                    -- Particular workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_DLC0430BCP12RF_InitializeWorkspace_16Bit(void *aW)
{
    T_DLC0430BCP12RF_Workspace *p = (T_DLC0430BCP12RF_Workspace *)aW;
    p->iBaseAddress = LCD_DISPLAY_BASE_ADDRESS;
    p->aNumOpen = 0;
    p->iBacklightLevel = 0; // 0%
    p->iConfiguration = &LCD_DLC0430BCP12RF_configuration_16Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_DLC0430BCP12RF_InitializeWorkspace_I15Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for DLC0430BCP12RF LCD.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_DLC0430BCP12RF_InitializeWorkspace_I15Bit(void *aW)
{
    T_DLC0430BCP12RF_Workspace *p = (T_DLC0430BCP12RF_Workspace *)aW;
    p->iBaseAddress = LCD_DISPLAY_BASE_ADDRESS;
    p->aNumOpen = 0;
    p->iBacklightLevel = 0; // 0%
    p->iConfiguration = &LCD_DLC0430BCP12RF_configuration_I15Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_DLC0430BCP12RF_InitializeWorkspace_8Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for DLC0430BCP12RF LCD.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_DLC0430BCP12RF_InitializeWorkspace_8Bit(void *aW)
{
    T_DLC0430BCP12RF_Workspace *p = (T_DLC0430BCP12RF_Workspace *)aW;
    p->iBaseAddress = LCD_DISPLAY_BASE_ADDRESS;
    p->aNumOpen = 0;
    p->iBacklightLevel = 0; // 0%
    p->iConfiguration = &LCD_DLC0430BCP12RF_configuration_8Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_DLC0430BCP12RF_Close
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_DLC0430BCP12RF_Close(void *aW)
{
    T_DLC0430BCP12RF_Workspace *p = (T_DLC0430BCP12RF_Workspace *)aW;
    p->aNumOpen--;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_DLC0430BCP12RF_Configure
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_DLC0430BCP12RF_Configure(
            void *aW,
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_DLC0430BCP12RF_GetInfo
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_DLC0430BCP12RF_GetInfo(void *aW, T_uezLCDConfiguration *aConfiguration)
{
    T_DLC0430BCP12RF_Workspace *p = (T_DLC0430BCP12RF_Workspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_DLC0430BCP12RF_GetFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_DLC0430BCP12RF_GetFrame(
            void *aW,
            TUInt32 aFrame,
            void **aFrameBuffer)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_DLC0430BCP12RF_ShowFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_DLC0430BCP12RF_ShowFrame(void *aW, TUInt32 aFrame)
{
    T_DLC0430BCP12RF_Workspace *p = (T_DLC0430BCP12RF_Workspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_DLC0430BCP12RF_SetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_DLC0430BCP12RF_SetBacklightLevel(void *aW, TUInt32 aLevel)
{
    T_DLC0430BCP12RF_Workspace *p = (T_DLC0430BCP12RF_Workspace *)aW;
    TUInt16 level;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_DLC0430BCP12RF_MSTimerStart
 *---------------------------------------------------------------------------*
 * Description:
...
#else
 #if (LCD_BACKLIGHT_FREERTOS_TIMER==1)
static T_uezError LCD_DLC0430BCP12RF_MSTimerStart(void *aW, float milliseconds) {
  T_DLC0430BCP12RF_Workspace *p = (T_DLC0430BCP12RF_Workspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_DLC0430BCP12RF_TimerCallback
 *---------------------------------------------------------------------------*
 * Description:
...
#else
 #if (LCD_BACKLIGHT_FREERTOS_TIMER==1)
static void LCD_DLC0430BCP12RF_TimerCallback( TimerHandle_t xTimer ) {
    //p->itimerDone = ETrue;
 ...
 #else
static void LCD_DLC0430BCP12RF_TimerCallback(T_uezTimerCallback *aCallbackWorkspace){
  T_DLC0430BCP12RF_Workspace *p = aCallbackWorkspace->iData;
  p->itimerDone = ETrue;
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_DLC0430BCP12RF_On(void *aW) {  
    T_DLC0430BCP12RF_Workspace *p = (T_DLC0430BCP12RF_Workspace *)aW;
    ...
    if (p->iBacklight){
...
#else
 #if (LCD_BACKLIGHT_FREERTOS_TIMER==1)
      LCD_DLC0430BCP12RF_MSTimerStart(p, 167.0); // minimum 167ms timer
      ...
      LCD_DLC0430BCP12RF_SetBacklightLevel(p, p->iBacklightLevel); // Turn back on to the remembered level
    }
    return UEZ_ERROR_NONE;
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_DLC0430BCP12RF_Off
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_DLC0430BCP12RF_Off(void *aW) {
    T_DLC0430BCP12RF_Workspace *p = (T_DLC0430BCP12RF_Workspace *)aW;    
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_DLC0430BCP12RF_Open
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_DLC0430BCP12RF_Open(void *aW)
{
    T_DLC0430BCP12RF_Workspace *p = (T_DLC0430BCP12RF_Workspace *)aW;
        ...
        if (!error) {
            switch (p->iConfiguration->iColorDepth) {
                case UEZLCD_COLOR_DEPTH_8_BIT:
                    LCD_DLC0430BCP12RF_settings = LCD_DLC0430BCP12RF_params8bit;
                    break;
                default:
                case UEZLCD_COLOR_DEPTH_16_BIT:
                    LCD_DLC0430BCP12RF_settings = LCD_DLC0430BCP12RF_params16bit;
                    break;
                case UEZLCD_COLOR_DEPTH_I15_BIT:
                    LCD_DLC0430BCP12RF_settings = LCD_DLC0430BCP12RF_paramsI15bit;
                    break;
            }
            LCD_DLC0430BCP12RF_settings.iBaseAddress = p->iBaseAddress;
            error = (*plcdc)->Configure(plcdc, &LCD_DLC0430BCP12RF_settings);
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_DLC0430BCP12RF_GetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  no backlight for LCD.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_DLC0430BCP12RF_GetBacklightLevel(
        void *aW, 
        TUInt32 *aLevel,
        TUInt32 *aNumLevels)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_DLC0430BCP12RF_SetPaletteColor
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_DLC0430BCP12RF_SetPaletteColor(
                    void *aWorkspace,
                    TUInt32 aColorIndex,
                    TUInt16 aRed,
                    TUInt16 aGreen,
                    TUInt16 aBlue)
{
    T_DLC0430BCP12RF_Workspace *p = (T_DLC0430BCP12RF_Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_DLC0430BCP12RF_VerticalSyncCallback
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static void LCD_DLC0430BCP12RF_VerticalSyncCallback(void *aCallbackWorkspace)
{
    T_DLC0430BCP12RF_Workspace *p = (T_DLC0430BCP12RF_Workspace *)aCallbackWorkspace;

    _isr_UEZSemaphoreRelease(p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_DLC0430BCP12RF_WaitForVerticalSync
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_DLC0430BCP12RF_WaitForVerticalSync(
    void *aWorkspace,
    TUInt32 aTimeout)
{
    T_DLC0430BCP12RF_Workspace *p = (T_DLC0430BCP12RF_Workspace *)aWorkspace;
    ...
    (*p_hal)->EnableVerticalSync(p_hal, LCD_DLC0430BCP12RF_VerticalSyncCallback, p);
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const DEVICE_LCD LCD_DLC0430BCP12RF_Interface_16Bit = {
	{
	    // Common interface
	    "LCD:DLC0430BCP12RF:16Bit",
	    0x0100,
	    LCD_DLC0430BCP12RF_InitializeWorkspace_16Bit,
	    sizeof(T_DLC0430BCP12RF_Workspace),
	},
	
    // Functions
    LCD_DLC0430BCP12RF_Open,
    LCD_DLC0430BCP12RF_Close,
    LCD_DLC0430BCP12RF_Configure,
    LCD_DLC0430BCP12RF_GetInfo,
    LCD_DLC0430BCP12RF_GetFrame,
    LCD_DLC0430BCP12RF_ShowFrame,
    LCD_DLC0430BCP12RF_On,
    LCD_DLC0430BCP12RF_Off,
    LCD_DLC0430BCP12RF_SetBacklightLevel,
    LCD_DLC0430BCP12RF_GetBacklightLevel,
    LCD_DLC0430BCP12RF_SetPaletteColor,
    // v2.04
    LCD_DLC0430BCP12RF_WaitForVerticalSync,
};

const DEVICE_LCD LCD_DLC0430BCP12RF_Interface_I15Bit = {
	{
	    // Common interface
	    "LCD:DLC0430BCP12RF:I15Bit",
	    0x0100,
	    LCD_DLC0430BCP12RF_InitializeWorkspace_I15Bit,
	    sizeof(T_DLC0430BCP12RF_Workspace),
	},
...
const DEVICE_LCD LCD_DLC0430BCP12RF_Interface_8Bit = {
	{
	    // Common interface
	    "LCD:DLC0430BCP12RF:8Bit",
	    0x0100,
	    LCD_DLC0430BCP12RF_InitializeWorkspace_8Bit,
	    sizeof(T_DLC0430BCP12RF_Workspace),
	},
...
// List of interfaces
const T_uezDeviceInterface *LCD_DLC0430BCP12RF_InterfaceArray[] = {
    0,      // 1 bit
    0,      // 2 bit
    0,      // 4 bit
    (T_uezDeviceInterface *)&LCD_DLC0430BCP12RF_Interface_8Bit,      // 8 bit
    (T_uezDeviceInterface *)&LCD_DLC0430BCP12RF_Interface_16Bit,      // 16 bit
    (T_uezDeviceInterface *)&LCD_DLC0430BCP12RF_Interface_I15Bit,      // I15 bit
    0,      // 24 bit
};

/*-------------------------------------------------------------------------*
 * End of File:  DLC0430BCP12RF.c
 *-------------------------------------------------------------------------*/
