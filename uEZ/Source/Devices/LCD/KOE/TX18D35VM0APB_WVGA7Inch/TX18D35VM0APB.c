/*-------------------------------------------------------------------------*
 * File:  TX18D35VM0APB.c
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the KOE_TX18D35VM0APB.
 *-------------------------------------------------------------------------*/
...
#ifndef LCD_KOE_TX18D35VM0APB_FLIP_SCREEN_X
#define LCD_KOE_TX18D35VM0APB_FLIP_SCREEN_X       0
#endif

#ifndef LCD_KOE_TX18D35VM0APB_FLIP_SCREEN_Y
#define LCD_KOE_TX18D35VM0APB_FLIP_SCREEN_Y       0
#endif

// cannot reach 33.3Mhz on LPC1788 with default Crystal, max is 36MHz!
// 30MHz is min spec!
#ifndef KOE_TX18D35VM0APB_DOTCLOCK_HZ
    #define     KOE_TX18D35VM0APB_DOTCLOCK_HZ       33260000
#endif
...
} T_TX18D35VM0APBWorkspace;

/*---------------------------------------------------------------------------*
 * Globals:
 *---------------------------------------------------------------------------*/
static T_LCDControllerSettings LCD_TX18D35VM0APB_settings;

static const T_LCDControllerSettings LCD_TX18D35VM0APB_params16bit = {
    LCD_ADVANCED_TFT,
    ...
    KOE_TX18D35VM0APB_DOTCLOCK_HZ,
};

static const T_LCDControllerSettings LCD_TX18D35VM0APB_paramsI15bit = {
    LCD_ADVANCED_TFT,
    LCD_COLOR_RES_16_I555,

    216, //89         /* Horizontal back porch */
    40, //40         /* Horizontal front porch */
    128,          /* HSYNC pulse width */
    800,        /* Pixels per line */
...
static const T_LCDControllerSettings LCD_TX18D35VM0APB_params8bit = {
    LCD_ADVANCED_TFT,
    LCD_COLOR_RES_8,
...
static T_uezLCDConfiguration LCD_TX18D35VM0APB_configuration_16Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_TX18D35VM0APB_configuration_I15Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_TX18D35VM0APB_configuration_8Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
 ...
 * Prototypes:
 *---------------------------------------------------------------------------*/
extern const DEVICE_LCD LCD_KOE_TX18D35VM0APB_Interface;

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX18D35VM0APB_InitializeWorkspace_16Bit
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
T_uezError LCD_TX18D35VM0APB_InitializeWorkspace_16Bit(void *aW)
{
    T_TX18D35VM0APBWorkspace *p = (T_TX18D35VM0APBWorkspace *)aW;
    p->iBaseAddress = LCD_DISPLAY_BASE_ADDRESS;
    p->aNumOpen = 0;
    p->iBacklightLevel = 256; // 100%
    p->iConfiguration = &LCD_TX18D35VM0APB_configuration_16Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX18D35VM0APB_InitializeWorkspace_I15Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for TM070RBHG04 LCD.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_TX18D35VM0APB_InitializeWorkspace_I15Bit(void *aW)
{
    T_TX18D35VM0APBWorkspace *p = (T_TX18D35VM0APBWorkspace *)aW;
    p->iBaseAddress = LCD_DISPLAY_BASE_ADDRESS;
    p->aNumOpen = 0;
    p->iBacklightLevel = 256; // 100%
    p->iConfiguration = &LCD_TX18D35VM0APB_configuration_I15Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX18D35VM0APB_InitializeWorkspace_8Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for TM070RBHG04 LCD.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_TX18D35VM0APB_InitializeWorkspace_8Bit(void *aW)
{
    T_TX18D35VM0APBWorkspace *p = (T_TX18D35VM0APBWorkspace *)aW;
    p->iBaseAddress = LCD_DISPLAY_BASE_ADDRESS;
    p->aNumOpen = 0;
    p->iBacklightLevel = 256; // 100%
    p->iConfiguration = &LCD_TX18D35VM0APB_configuration_8Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX18D35VM0APB_Open
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX18D35VM0APB_Open(void *aW)
{
    T_TX18D35VM0APBWorkspace *p = (T_TX18D35VM0APBWorkspace *)aW;
    ...
    if (p->aNumOpen == 1) {
        ...
        // Data enable pin MUST be used with active high so not hard set here!
        
        // Up Down Selection
        UEZGPIOSetMux(p->iUpDownPin, 0);
        UEZGPIOOutput(p->iUpDownPin);
#if LCD_KOE_TX18D35VM0APB_FLIP_SCREEN_Y
...
#if LCD_KOE_TX18D35VM0APB_FLIP_SCREEN_X
        ...
        if (!error) {
            switch (p->iConfiguration->iColorDepth) {
              case UEZLCD_COLOR_DEPTH_8_BIT:
                LCD_TX18D35VM0APB_settings = LCD_TX18D35VM0APB_params8bit;
                break;
              default:
              case UEZLCD_COLOR_DEPTH_16_BIT:
                LCD_TX18D35VM0APB_settings = LCD_TX18D35VM0APB_params16bit;
                break;
              case UEZLCD_COLOR_DEPTH_I15_BIT:
                LCD_TX18D35VM0APB_settings = LCD_TX18D35VM0APB_paramsI15bit;
                break;
            }
            LCD_TX18D35VM0APB_settings.iBaseAddress = p->iBaseAddress;
            error = (*plcdc)->Configure(plcdc, &LCD_TX18D35VM0APB_settings);
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX18D35VM0APB_Close
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX18D35VM0APB_Close(void *aW)
{
    T_TX18D35VM0APBWorkspace *p = (T_TX18D35VM0APBWorkspace *)aW;
    p->aNumOpen--;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX18D35VM0APB_Configure
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_TX18D35VM0APB_Configure(
            void *aW,
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX18D35VM0APB_GetInfo
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX18D35VM0APB_GetInfo(void *aW, T_uezLCDConfiguration *aConfiguration)
{
    T_TX18D35VM0APBWorkspace *p = (T_TX18D35VM0APBWorkspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX18D35VM0APB_GetFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX18D35VM0APB_GetFrame(
            void *aW,
            TUInt32 aFrame,
            void **aFrameBuffer)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX18D35VM0APB_ShowFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX18D35VM0APB_ShowFrame(void *aW, TUInt32 aFrame)
{
    T_TX18D35VM0APBWorkspace *p = (T_TX18D35VM0APBWorkspace *)aW;
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX18D35VM0APB_On(void *aW)
{
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX18D35VM0APB_Off(void *aW)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX18D35VM0APB_SetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX18D35VM0APB_SetBacklightLevel(void *aW, TUInt32 aLevel)
{
    T_TX18D35VM0APBWorkspace *p = (T_TX18D35VM0APBWorkspace *)aW;
    TUInt16 level;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX18D35VM0APB_GetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  no backlight for LCD.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX18D35VM0APB_GetBacklightLevel(
        void *aW,
        TUInt32 *aLevel,
        TUInt32 *aNumLevels)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX18D35VM0APB_SetPaletteColor
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX18D35VM0APB_SetPaletteColor(
                    void *aWorkspace,
                    TUInt32 aColorIndex,
                    TUInt16 aRed,
                    TUInt16 aGreen,
                    TUInt16 aBlue)
{
    T_TX18D35VM0APBWorkspace *p = (T_TX18D35VM0APBWorkspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX18D35VM0APB_VerticalSyncCallback
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static void LCD_TX18D35VM0APB_VerticalSyncCallback(void *aCallbackWorkspace)
{
    T_TX18D35VM0APBWorkspace *p = (T_TX18D35VM0APBWorkspace *)aCallbackWorkspace;

    _isr_UEZSemaphoreRelease(p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX18D35VM0APB_WaitForVerticalSync
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX18D35VM0APB_WaitForVerticalSync(
    void *aWorkspace,
    TUInt32 aTimeout)
{
    T_TX18D35VM0APBWorkspace *p = (T_TX18D35VM0APBWorkspace *)aWorkspace;
    ...
    (*p_hal)->EnableVerticalSync(p_hal, LCD_TX18D35VM0APB_VerticalSyncCallback, p);
    return UEZSemaphoreGrab(p->iVSyncSem, aTimeout);
}

void LCD_TX18D35VM0APB_Create(char* aName,
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const DEVICE_LCD LCD_KOE_TX18D35VM0APB_Interface_16Bit = {
	{
	    // Common interface
	    "LCD:KOE_TX18D35VM0APB:16Bit",
	    0x0100,
	    LCD_TX18D35VM0APB_InitializeWorkspace_16Bit,
	    sizeof(T_TX18D35VM0APBWorkspace),
	},
	
    // Functions
    LCD_TX18D35VM0APB_Open,
    LCD_TX18D35VM0APB_Close,
    LCD_TX18D35VM0APB_Configure,
    LCD_TX18D35VM0APB_GetInfo,
    LCD_TX18D35VM0APB_GetFrame,
    LCD_TX18D35VM0APB_ShowFrame,
    LCD_TX18D35VM0APB_On,
    LCD_TX18D35VM0APB_Off,
    LCD_TX18D35VM0APB_SetBacklightLevel,
    LCD_TX18D35VM0APB_GetBacklightLevel,
    LCD_TX18D35VM0APB_SetPaletteColor,
    // v2.04
    LCD_TX18D35VM0APB_WaitForVerticalSync,
};

const DEVICE_LCD LCD_KOE_TX18D35VM0APB_Interface_I15Bit = {
	{
	    // Common interface
	    "LCD:KOE_TX18D35VM0APB:I15Bit",
	    0x0100,
	    LCD_TX18D35VM0APB_InitializeWorkspace_I15Bit,
	    sizeof(T_TX18D35VM0APBWorkspace),
	},
...
const DEVICE_LCD LCD_KOE_TX18D35VM0APB_Interface_8Bit = {
	{
	    // Common interface
	    "LCD:KOE_TX18D35VM0APB:8Bit",
	    0x0100,
	    LCD_TX18D35VM0APB_InitializeWorkspace_8Bit,
	    sizeof(T_TX18D35VM0APBWorkspace),
	},
...
// List of interfaces
const T_uezDeviceInterface *LCD_KOE_TX18D35VM0APB_InterfaceArray[] = {
    0,      // 1 bit
    0,      // 2 bit
    0,      // 4 bit
    (T_uezDeviceInterface *)&LCD_KOE_TX18D35VM0APB_Interface_8Bit,      // 8 bit
    (T_uezDeviceInterface *)&LCD_KOE_TX18D35VM0APB_Interface_16Bit,      // 16 bit
    (T_uezDeviceInterface *)&LCD_KOE_TX18D35VM0APB_Interface_I15Bit,      // I15 bit
    0,      // 24 bit
};

/*-------------------------------------------------------------------------*
 * End of File:  TX18D35VM0APB.c
 *-------------------------------------------------------------------------*/
