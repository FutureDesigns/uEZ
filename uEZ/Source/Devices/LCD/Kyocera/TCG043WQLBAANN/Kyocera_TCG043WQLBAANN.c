/*-------------------------------------------------------------------------*
 * File:  Kyocera_TCG043WQLBAANN.c
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the Kyocera_TCG043WQLBAANN.
 *-------------------------------------------------------------------------*/
...
} T_TCG043WQLBAANNWorkspace;


/*---------------------------------------------------------------------------*
 * Globals:
 *---------------------------------------------------------------------------*/
static T_LCDControllerSettings LCD_TCG043WQLBAANN_settings;

static const T_LCDControllerSettings LCD_TCG043WQLBAANN_params16bit = {
    LCD_ADVANCED_TFT,
...
static const T_LCDControllerSettings LCD_TCG043WQLBAANN_params8bit = {
    LCD_ADVANCED_TFT,
    LCD_COLOR_RES_8,
...
static T_uezLCDConfiguration LCD_TCG043WQLBAANN_configuration_16Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_TCG043WQLBAANN_configuration_I15Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_TCG043WQLBAANN_configuration_8Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
 ...
 * Prototypes:
 *---------------------------------------------------------------------------*/
extern const DEVICE_LCD LCD_Kyocera_TCG043WQLBAANN_Interface;

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TCG043WQLBAANN_InitializeWorkspace_16Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for TCG043WQLBAANN LCD.
 * Inputs:
 *      void *aW                    -- Particular workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_TCG043WQLBAANN_InitializeWorkspace_16Bit(void *aW)
{
    T_TCG043WQLBAANNWorkspace *p = (T_TCG043WQLBAANNWorkspace *)aW;
    p->iBaseAddress = 0xA0000000;
    p->aNumOpen = 0;
    p->iBacklightLevel = 256; // 100%
    p->iConfiguration = &LCD_TCG043WQLBAANN_configuration_16Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TCG043WQLBAANN_InitializeWorkspace_I15Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for TCG043WQLBAANN LCD.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_TCG043WQLBAANN_InitializeWorkspace_I15Bit(void *aW)
{
    T_TCG043WQLBAANNWorkspace *p = (T_TCG043WQLBAANNWorkspace *)aW;
    p->iBaseAddress = 0xA0000000;
    p->aNumOpen = 0;
    p->iBacklightLevel = 256; // 100%
    p->iConfiguration = &LCD_TCG043WQLBAANN_configuration_I15Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TCG043WQLBAANN_InitializeWorkspace_8Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for TCG043WQLBAANN LCD.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_TCG043WQLBAANN_InitializeWorkspace_8Bit(void *aW)
{
    T_TCG043WQLBAANNWorkspace *p = (T_TCG043WQLBAANNWorkspace *)aW;
    p->iBaseAddress = 0xA0000000;
    p->aNumOpen = 0;
    p->iBacklightLevel = 256; // 100%
    p->iConfiguration = &LCD_TCG043WQLBAANN_configuration_8Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TCG043WQLBAANN_Open
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TCG043WQLBAANN_Open(void *aW)
{
    T_TCG043WQLBAANNWorkspace *p = (T_TCG043WQLBAANNWorkspace *)aW;
        ...
        if (!error) {
            switch (p->iConfiguration->iColorDepth) {
                case UEZLCD_COLOR_DEPTH_8_BIT:
                    LCD_TCG043WQLBAANN_settings = LCD_TCG043WQLBAANN_params8bit;
                    break;
                default:
                case UEZLCD_COLOR_DEPTH_16_BIT:
                    LCD_TCG043WQLBAANN_settings = LCD_TCG043WQLBAANN_params16bit;
                    break;
                case UEZLCD_COLOR_DEPTH_I15_BIT:
                    LCD_TCG043WQLBAANN_settings = LCD_TCG043WQLBAANN_paramsI15bit;
                    break;
            }
            LCD_TCG043WQLBAANN_settings.iBaseAddress = p->iBaseAddress;
            error = (*plcdc)->Configure(plcdc, &LCD_TCG043WQLBAANN_settings);
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TCG043WQLBAANN_Close
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TCG043WQLBAANN_Close(void *aW)
{
    T_TCG043WQLBAANNWorkspace *p = (T_TCG043WQLBAANNWorkspace *)aW;
    p->aNumOpen--;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TCG043WQLBAANN_Configure
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_TCG043WQLBAANN_Configure(
            void *aW,
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TCG043WQLBAANN_GetInfo
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TCG043WQLBAANN_GetInfo(void *aW, T_uezLCDConfiguration *aConfiguration)
{
    T_TCG043WQLBAANNWorkspace *p = (T_TCG043WQLBAANNWorkspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TCG043WQLBAANN_GetFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TCG043WQLBAANN_GetFrame(
            void *aW,
            TUInt32 aFrame,
            void **aFrameBuffer)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TCG043WQLBAANN_ShowFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TCG043WQLBAANN_ShowFrame(void *aW, TUInt32 aFrame)
{
    T_TCG043WQLBAANNWorkspace *p = (T_TCG043WQLBAANNWorkspace *)aW;
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TCG043WQLBAANN_On(void *aW)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TCG043WQLBAANN_Off
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TCG043WQLBAANN_Off(void *aW)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TCG043WQLBAANN_SetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TCG043WQLBAANN_SetBacklightLevel(void *aW, TUInt32 aLevel)
{
    T_TCG043WQLBAANNWorkspace *p = (T_TCG043WQLBAANNWorkspace *)aW;
    TUInt16 level;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TCG043WQLBAANN_GetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  no backlight for LCD.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TCG043WQLBAANN_GetBacklightLevel(
        void *aW, 
        TUInt32 *aLevel,
        TUInt32 *aNumLevels)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TCG043WQLBAANN_SetPaletteColor
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TCG043WQLBAANN_SetPaletteColor(
                    void *aWorkspace,
                    TUInt32 aColorIndex,
                    TUInt16 aRed,
                    TUInt16 aGreen,
                    TUInt16 aBlue)
{
    T_TCG043WQLBAANNWorkspace *p = (T_TCG043WQLBAANNWorkspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TCG043WQLBAANN_VerticalSyncCallback
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static void LCD_TCG043WQLBAANN_VerticalSyncCallback(void *aCallbackWorkspace)
{
    T_TCG043WQLBAANNWorkspace *p = (T_TCG043WQLBAANNWorkspace *)aCallbackWorkspace;

    _isr_UEZSemaphoreRelease(p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TCG043WQLBAANN_WaitForVerticalSync
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TCG043WQLBAANN_WaitForVerticalSync(
    void *aWorkspace,
    TUInt32 aTimeout)
{
    T_TCG043WQLBAANNWorkspace *p = (T_TCG043WQLBAANNWorkspace *)aWorkspace;
    ...
    (*p_hal)->EnableVerticalSync(p_hal, LCD_TCG043WQLBAANN_VerticalSyncCallback, p);
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const DEVICE_LCD LCD_Kyocera_TCG043WQLBAANN_Interface_16Bit = {
	{
	    // Common interface
	    "LCD:Kyocera_TCG043WQLBAANN:16Bit",
	    0x0100,
	    LCD_TCG043WQLBAANN_InitializeWorkspace_16Bit,
	    sizeof(T_TCG043WQLBAANNWorkspace),
	},
	
    // Functions
    LCD_TCG043WQLBAANN_Open,
    LCD_TCG043WQLBAANN_Close,
    LCD_TCG043WQLBAANN_Configure,
    LCD_TCG043WQLBAANN_GetInfo,
    LCD_TCG043WQLBAANN_GetFrame,
    LCD_TCG043WQLBAANN_ShowFrame,
    LCD_TCG043WQLBAANN_On,
    LCD_TCG043WQLBAANN_Off,
    LCD_TCG043WQLBAANN_SetBacklightLevel,
    LCD_TCG043WQLBAANN_GetBacklightLevel,
    LCD_TCG043WQLBAANN_SetPaletteColor,
    // v2.04
    LCD_TCG043WQLBAANN_WaitForVerticalSync,
};

const DEVICE_LCD LCD_Kyocera_TCG043WQLBAANN_Interface_I15Bit = {
	{
	    // Common interface
	    "LCD:Kyocera_TCG043WQLBAANN:I15Bit",
	    0x0100,
	    LCD_TCG043WQLBAANN_InitializeWorkspace_I15Bit,
	    sizeof(T_TCG043WQLBAANNWorkspace),
	},
...
const DEVICE_LCD LCD_Kyocera_TCG043WQLBAANN_Interface_8Bit = {
	{
	    // Common interface
	    "LCD:Kyocera_TCG043WQLBAANN:8Bit",
	    0x0100,
	    LCD_TCG043WQLBAANN_InitializeWorkspace_8Bit,
	    sizeof(T_TCG043WQLBAANNWorkspace),
	},
...
// List of interfaces
const T_uezDeviceInterface *LCD_Kyocera_TCG043WQLBAANN_InterfaceArray[] = {
    0,      // 1 bit
    0,      // 2 bit
    0,      // 4 bit
    (T_uezDeviceInterface *)&LCD_Kyocera_TCG043WQLBAANN_Interface_8Bit,      // 8 bit
    (T_uezDeviceInterface *)&LCD_Kyocera_TCG043WQLBAANN_Interface_16Bit,      // 16 bit
    (T_uezDeviceInterface *)&LCD_Kyocera_TCG043WQLBAANN_Interface_I15Bit,      // I15 bit
    0,      // 24 bit
};

/*-------------------------------------------------------------------------*
 * End of File:  Kyocera_TCG043WQLBAANN.c
 *-------------------------------------------------------------------------*/
