/*-------------------------------------------------------------------------*
 * File:  NewHaven_NHD43480272EF_ASXV.c
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the NewHaven_NHD43480272EF_ASXV.
 *-------------------------------------------------------------------------*/
...
#include "NewHaven_NHD43480272EF-ASXV.h"
#include <uEZGPIO.h>
#include <uEZTimer.h>
#include <timers.h>
...
} T_NHD43480272EF_ASXVWorkspace;

/*---------------------------------------------------------------------------*
 * Globals:
 *---------------------------------------------------------------------------*/
static T_LCDControllerSettings LCD_NHD43480272EF_ASXV_settings;
#if (LCD_BACKLIGHT_FREERTOS_TIMER==1)
static TimerHandle_t lcdBacklightDelayTimer = NULL;
#else
#endif

static const T_LCDControllerSettings LCD_NHD43480272EF_ASXV_params16bit = {
    LCD_ADVANCED_TFT,
...
static const T_LCDControllerSettings LCD_NHD43480272EF_ASXV_paramsI15bit = {
    LCD_ADVANCED_TFT,
...
static const T_LCDControllerSettings LCD_NHD43480272EF_ASXV_params8bit = {
    LCD_ADVANCED_TFT,
    LCD_COLOR_RES_8,
...
static T_uezLCDConfiguration LCD_NHD43480272EF_ASXV_configuration_16Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_NHD43480272EF_ASXV_configuration_I15Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_NHD43480272EF_ASXV_configuration_8Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
 ...
 * Prototypes:
 *---------------------------------------------------------------------------*/
extern const DEVICE_LCD LCD_NewHaven_NHD43480272EF_ASXV_Interface;

/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXV_InitializeWorkspace_16Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for NHD43480272EF_ASXV LCD.
 * Inputs:
 *      void *aW                    -- Particular workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_NHD43480272EF_ASXV_InitializeWorkspace_16Bit(void *aW)
{
    T_NHD43480272EF_ASXVWorkspace *p = (T_NHD43480272EF_ASXVWorkspace *)aW;
    p->iBaseAddress = LCD_DISPLAY_BASE_ADDRESS;
    p->aNumOpen = 0;
    p->iBacklightLevel = 0; // 0%
    p->iConfiguration = &LCD_NHD43480272EF_ASXV_configuration_16Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXV_InitializeWorkspace_I15Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for NHD43480272EF_ASXV LCD.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_NHD43480272EF_ASXV_InitializeWorkspace_I15Bit(void *aW)
{
    T_NHD43480272EF_ASXVWorkspace *p = (T_NHD43480272EF_ASXVWorkspace *)aW;
    p->iBaseAddress = LCD_DISPLAY_BASE_ADDRESS;
    p->aNumOpen = 0;
    p->iBacklightLevel = 0; // 0%
    p->iConfiguration = &LCD_NHD43480272EF_ASXV_configuration_I15Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXV_InitializeWorkspace_8Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for NHD43480272EF_ASXV LCD.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_NHD43480272EF_ASXV_InitializeWorkspace_8Bit(void *aW)
{
    T_NHD43480272EF_ASXVWorkspace *p = (T_NHD43480272EF_ASXVWorkspace *)aW;
    p->iBaseAddress = LCD_DISPLAY_BASE_ADDRESS;
    p->aNumOpen = 0;
    p->iBacklightLevel = 0; // 0%
    p->iConfiguration = &LCD_NHD43480272EF_ASXV_configuration_8Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXV_Close
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXV_Close(void *aW)
{
    T_NHD43480272EF_ASXVWorkspace *p = (T_NHD43480272EF_ASXVWorkspace *)aW;
    p->aNumOpen--;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXV_Configure
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_NHD43480272EF_ASXV_Configure(
            void *aW,
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXV_GetInfo
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXV_GetInfo(void *aW, T_uezLCDConfiguration *aConfiguration)
{
    T_NHD43480272EF_ASXVWorkspace *p = (T_NHD43480272EF_ASXVWorkspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXV_GetFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXV_GetFrame(
            void *aW,
            TUInt32 aFrame,
            void **aFrameBuffer)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXV_ShowFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXV_ShowFrame(void *aW, TUInt32 aFrame)
{
    T_NHD43480272EF_ASXVWorkspace *p = (T_NHD43480272EF_ASXVWorkspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXV_SetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXV_SetBacklightLevel(void *aW, TUInt32 aLevel)
{
    T_NHD43480272EF_ASXVWorkspace *p = (T_NHD43480272EF_ASXVWorkspace *)aW;
    TUInt16 level;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXV_MSTimerStart
 *---------------------------------------------------------------------------*
 * Description:
...
#else
 #if (LCD_BACKLIGHT_FREERTOS_TIMER==1)
static T_uezError LCD_NHD43480272EF_ASXV_MSTimerStart(void *aW, float milliseconds) {
  T_NHD43480272EF_ASXVWorkspace *p = (T_NHD43480272EF_ASXVWorkspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXV_TimerCallback
 *---------------------------------------------------------------------------*
 * Description:
...
#else
 #if (LCD_BACKLIGHT_FREERTOS_TIMER==1)
static void LCD_NHD43480272EF_ASXV_TimerCallback( TimerHandle_t xTimer ) {
    //p->itimerDone = ETrue;
 ...
 #else
static void LCD_NHD43480272EF_ASXV_TimerCallback(T_uezTimerCallback *aCallbackWorkspace){
  T_NHD43480272EF_ASXVWorkspace *p = aCallbackWorkspace->iData;
  p->itimerDone = ETrue;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXV_On
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXV_On(void *aW) {  
    T_NHD43480272EF_ASXVWorkspace *p = (T_NHD43480272EF_ASXVWorkspace *)aW;
    ...
    if (p->iBacklight){
...
#else
 #if (LCD_BACKLIGHT_FREERTOS_TIMER==1)
      LCD_NHD43480272EF_ASXV_MSTimerStart(p, 167.0); // minimum 167ms timer
      ...
      LCD_NHD43480272EF_ASXV_SetBacklightLevel(p, p->iBacklightLevel); // Turn back on to the remembered level
    }
    return UEZ_ERROR_NONE;
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXV_Off
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXV_Off(void *aW) {
    T_NHD43480272EF_ASXVWorkspace *p = (T_NHD43480272EF_ASXVWorkspace *)aW;    
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXV_Open
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXV_Open(void *aW)
{
    T_NHD43480272EF_ASXVWorkspace *p = (T_NHD43480272EF_ASXVWorkspace *)aW;
        ...
        if (!error) {
            switch (p->iConfiguration->iColorDepth) {
                case UEZLCD_COLOR_DEPTH_8_BIT:
                    LCD_NHD43480272EF_ASXV_settings = LCD_NHD43480272EF_ASXV_params8bit;
                    break;
                default:
                case UEZLCD_COLOR_DEPTH_16_BIT:
                    LCD_NHD43480272EF_ASXV_settings = LCD_NHD43480272EF_ASXV_params16bit;
                    break;
                case UEZLCD_COLOR_DEPTH_I15_BIT:
                    LCD_NHD43480272EF_ASXV_settings = LCD_NHD43480272EF_ASXV_paramsI15bit;
                    break;
            }
            LCD_NHD43480272EF_ASXV_settings.iBaseAddress = p->iBaseAddress;
            error = (*plcdc)->Configure(plcdc, &LCD_NHD43480272EF_ASXV_settings);
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXV_GetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  no backlight for LCD.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXV_GetBacklightLevel(
        void *aW, 
        TUInt32 *aLevel,
        TUInt32 *aNumLevels)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXV_SetPaletteColor
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXV_SetPaletteColor(
                    void *aWorkspace,
                    TUInt32 aColorIndex,
                    TUInt16 aRed,
                    TUInt16 aGreen,
                    TUInt16 aBlue)
{
    T_NHD43480272EF_ASXVWorkspace *p = (T_NHD43480272EF_ASXVWorkspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXV_VerticalSyncCallback
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static void LCD_NHD43480272EF_ASXV_VerticalSyncCallback(void *aCallbackWorkspace)
{
    T_NHD43480272EF_ASXVWorkspace *p = (T_NHD43480272EF_ASXVWorkspace *)aCallbackWorkspace;

    _isr_UEZSemaphoreRelease(p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXV_WaitForVerticalSync
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXV_WaitForVerticalSync(
    void *aWorkspace,
    TUInt32 aTimeout)
{
    T_NHD43480272EF_ASXVWorkspace *p = (T_NHD43480272EF_ASXVWorkspace *)aWorkspace;
    ...
    (*p_hal)->EnableVerticalSync(p_hal, LCD_NHD43480272EF_ASXV_VerticalSyncCallback, p);
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const DEVICE_LCD LCD_NewHaven_NHD43480272EF_ASXV_Interface_16Bit = {
	{
	    // Common interface
	    "LCD:NewHaven_NHD43480272EF_ASXV:16Bit",
	    0x0100,
	    LCD_NHD43480272EF_ASXV_InitializeWorkspace_16Bit,
	    sizeof(T_NHD43480272EF_ASXVWorkspace),
	},
	
    // Functions
    LCD_NHD43480272EF_ASXV_Open,
    LCD_NHD43480272EF_ASXV_Close,
    LCD_NHD43480272EF_ASXV_Configure,
    LCD_NHD43480272EF_ASXV_GetInfo,
    LCD_NHD43480272EF_ASXV_GetFrame,
    LCD_NHD43480272EF_ASXV_ShowFrame,
    LCD_NHD43480272EF_ASXV_On,
    LCD_NHD43480272EF_ASXV_Off,
    LCD_NHD43480272EF_ASXV_SetBacklightLevel,
    LCD_NHD43480272EF_ASXV_GetBacklightLevel,
    LCD_NHD43480272EF_ASXV_SetPaletteColor,
    // v2.04
    LCD_NHD43480272EF_ASXV_WaitForVerticalSync,
};

const DEVICE_LCD LCD_NewHaven_NHD43480272EF_ASXV_Interface_I15Bit = {
	{
	    // Common interface
	    "LCD:NewHaven_NHD43480272EF_ASXV:I15Bit",
	    0x0100,
	    LCD_NHD43480272EF_ASXV_InitializeWorkspace_I15Bit,
	    sizeof(T_NHD43480272EF_ASXVWorkspace),
	},
...
const DEVICE_LCD LCD_NewHaven_NHD43480272EF_ASXV_Interface_8Bit = {
	{
	    // Common interface
	    "LCD:NewHaven_NHD43480272EF_ASXV:8Bit",
	    0x0100,
	    LCD_NHD43480272EF_ASXV_InitializeWorkspace_8Bit,
	    sizeof(T_NHD43480272EF_ASXVWorkspace),
	},
...
// List of interfaces
const T_uezDeviceInterface *LCD_NewHaven_NHD43480272EF_ASXV_InterfaceArray[] = {
    0,      // 1 bit
    0,      // 2 bit
    0,      // 4 bit
    (T_uezDeviceInterface *)&LCD_NewHaven_NHD43480272EF_ASXV_Interface_8Bit,      // 8 bit
    (T_uezDeviceInterface *)&LCD_NewHaven_NHD43480272EF_ASXV_Interface_16Bit,      // 16 bit
    (T_uezDeviceInterface *)&LCD_NewHaven_NHD43480272EF_ASXV_Interface_I15Bit,      // I15 bit
    0,      // 24 bit
};

/*-------------------------------------------------------------------------*
 * End of File:  NewHaven_NHD43480272EF_ASXV.c
 *-------------------------------------------------------------------------*/
