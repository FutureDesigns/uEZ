/*-------------------------------------------------------------------------*
 * File:  NewHaven_NHD43480272EF_ASXP.c
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the NewHaven_NHD43480272EF_ASXP.
 *-------------------------------------------------------------------------*/
...
#include "NewHaven_NHD43480272EF-ASXP.h"
#include <uEZGPIO.h>
#include <uEZTimer.h>
#include <timers.h>
...
} T_NHD43480272EF_ASXPWorkspace;

/*---------------------------------------------------------------------------*
 * Globals:
 *---------------------------------------------------------------------------*/
static T_LCDControllerSettings LCD_NHD43480272EF_ASXP_settings;
#if (LCD_BACKLIGHT_FREERTOS_TIMER==1)
static TimerHandle_t lcdBacklightDelayTimer = NULL;
#else
#endif

static const T_LCDControllerSettings LCD_NHD43480272EF_ASXP_params16bit = {
    LCD_ADVANCED_TFT,
...
static const T_LCDControllerSettings LCD_NHD43480272EF_ASXP_paramsI15bit = {
    LCD_ADVANCED_TFT,
...
static const T_LCDControllerSettings LCD_NHD43480272EF_ASXP_params8bit = {
    LCD_ADVANCED_TFT,
    LCD_COLOR_RES_8,
...
static T_uezLCDConfiguration LCD_NHD43480272EF_ASXP_configuration_16Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_NHD43480272EF_ASXP_configuration_I15Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_NHD43480272EF_ASXP_configuration_8Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
 ...
 * Prototypes:
 *---------------------------------------------------------------------------*/
extern const DEVICE_LCD LCD_NewHaven_NHD43480272EF_ASXP_Interface;

/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXP_InitializeWorkspace_16Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for NHD43480272EF_ASXP LCD.
 * Inputs:
 *      void *aW                    -- Particular workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_NHD43480272EF_ASXP_InitializeWorkspace_16Bit(void *aW)
{
    T_NHD43480272EF_ASXPWorkspace *p = (T_NHD43480272EF_ASXPWorkspace *)aW;
    p->iBaseAddress = LCD_DISPLAY_BASE_ADDRESS;
    p->aNumOpen = 0;
    p->iBacklightLevel = 0; // 0%
    p->iConfiguration = &LCD_NHD43480272EF_ASXP_configuration_16Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXP_InitializeWorkspace_I15Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for NHD43480272EF_ASXP LCD.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_NHD43480272EF_ASXP_InitializeWorkspace_I15Bit(void *aW)
{
    T_NHD43480272EF_ASXPWorkspace *p = (T_NHD43480272EF_ASXPWorkspace *)aW;
    p->iBaseAddress = LCD_DISPLAY_BASE_ADDRESS;
    p->aNumOpen = 0;
    p->iBacklightLevel = 0; // 0%
    p->iConfiguration = &LCD_NHD43480272EF_ASXP_configuration_I15Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXP_InitializeWorkspace_8Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for NHD43480272EF_ASXP LCD.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_NHD43480272EF_ASXP_InitializeWorkspace_8Bit(void *aW)
{
    T_NHD43480272EF_ASXPWorkspace *p = (T_NHD43480272EF_ASXPWorkspace *)aW;
    p->iBaseAddress = LCD_DISPLAY_BASE_ADDRESS;
    p->aNumOpen = 0;
    p->iBacklightLevel = 0; // 0%
    p->iConfiguration = &LCD_NHD43480272EF_ASXP_configuration_8Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXP_Close
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXP_Close(void *aW)
{
    T_NHD43480272EF_ASXPWorkspace *p = (T_NHD43480272EF_ASXPWorkspace *)aW;
    p->aNumOpen--;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXP_Configure
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_NHD43480272EF_ASXP_Configure(
            void *aW,
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXP_GetInfo
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXP_GetInfo(void *aW, T_uezLCDConfiguration *aConfiguration)
{
    T_NHD43480272EF_ASXPWorkspace *p = (T_NHD43480272EF_ASXPWorkspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXP_GetFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXP_GetFrame(
            void *aW,
            TUInt32 aFrame,
            void **aFrameBuffer)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXP_ShowFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXP_ShowFrame(void *aW, TUInt32 aFrame)
{
    T_NHD43480272EF_ASXPWorkspace *p = (T_NHD43480272EF_ASXPWorkspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXP_SetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXP_SetBacklightLevel(void *aW, TUInt32 aLevel)
{
    T_NHD43480272EF_ASXPWorkspace *p = (T_NHD43480272EF_ASXPWorkspace *)aW;
    TUInt16 level;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXP_MSTimerStart
 *---------------------------------------------------------------------------*
 * Description:
...
#else
 #if (LCD_BACKLIGHT_FREERTOS_TIMER==1)
static T_uezError LCD_NHD43480272EF_ASXP_MSTimerStart(void *aW, float milliseconds) {
  T_NHD43480272EF_ASXPWorkspace *p = (T_NHD43480272EF_ASXPWorkspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXP_TimerCallback
 *---------------------------------------------------------------------------*
 * Description:
...
#else
 #if (LCD_BACKLIGHT_FREERTOS_TIMER==1)
static void LCD_NHD43480272EF_ASXP_TimerCallback( TimerHandle_t xTimer ) {
    //p->itimerDone = ETrue;
 ...
 #else
static void LCD_NHD43480272EF_ASXP_TimerCallback(T_uezTimerCallback *aCallbackWorkspace){
  T_NHD43480272EF_ASXPWorkspace *p = aCallbackWorkspace->iData;
  p->itimerDone = ETrue;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXP_On
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXP_On(void *aW) {  
    T_NHD43480272EF_ASXPWorkspace *p = (T_NHD43480272EF_ASXPWorkspace *)aW;
    ...
    if (p->iBacklight){
...
#else
 #if (LCD_BACKLIGHT_FREERTOS_TIMER==1)
      LCD_NHD43480272EF_ASXP_MSTimerStart(p, 167.0); // minimum 167ms timer
      ...
      LCD_NHD43480272EF_ASXP_SetBacklightLevel(p, p->iBacklightLevel); // Turn back on to the remembered level
    }
    return UEZ_ERROR_NONE;
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXP_Off
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXP_Off(void *aW) {
    T_NHD43480272EF_ASXPWorkspace *p = (T_NHD43480272EF_ASXPWorkspace *)aW;    
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXP_Open
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXP_Open(void *aW)
{
    T_NHD43480272EF_ASXPWorkspace *p = (T_NHD43480272EF_ASXPWorkspace *)aW;
        ...
        if (!error) {
            switch (p->iConfiguration->iColorDepth) {
                case UEZLCD_COLOR_DEPTH_8_BIT:
                    LCD_NHD43480272EF_ASXP_settings = LCD_NHD43480272EF_ASXP_params8bit;
                    break;
                default:
                case UEZLCD_COLOR_DEPTH_16_BIT:
                    LCD_NHD43480272EF_ASXP_settings = LCD_NHD43480272EF_ASXP_params16bit;
                    break;
                case UEZLCD_COLOR_DEPTH_I15_BIT:
                    LCD_NHD43480272EF_ASXP_settings = LCD_NHD43480272EF_ASXP_paramsI15bit;
                    break;
            }
            LCD_NHD43480272EF_ASXP_settings.iBaseAddress = p->iBaseAddress;
            error = (*plcdc)->Configure(plcdc, &LCD_NHD43480272EF_ASXP_settings);
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXP_GetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  no backlight for LCD.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXP_GetBacklightLevel(
        void *aW, 
        TUInt32 *aLevel,
        TUInt32 *aNumLevels)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXP_SetPaletteColor
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXP_SetPaletteColor(
                    void *aWorkspace,
                    TUInt32 aColorIndex,
                    TUInt16 aRed,
                    TUInt16 aGreen,
                    TUInt16 aBlue)
{
    T_NHD43480272EF_ASXPWorkspace *p = (T_NHD43480272EF_ASXPWorkspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXP_VerticalSyncCallback
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static void LCD_NHD43480272EF_ASXP_VerticalSyncCallback(void *aCallbackWorkspace)
{
    T_NHD43480272EF_ASXPWorkspace *p = (T_NHD43480272EF_ASXPWorkspace *)aCallbackWorkspace;

    _isr_UEZSemaphoreRelease(p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD43480272EF_ASXP_WaitForVerticalSync
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD43480272EF_ASXP_WaitForVerticalSync(
    void *aWorkspace,
    TUInt32 aTimeout)
{
    T_NHD43480272EF_ASXPWorkspace *p = (T_NHD43480272EF_ASXPWorkspace *)aWorkspace;
    ...
    (*p_hal)->EnableVerticalSync(p_hal, LCD_NHD43480272EF_ASXP_VerticalSyncCallback, p);
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const DEVICE_LCD LCD_NewHaven_NHD43480272EF_ASXP_Interface_16Bit = {
	{
	    // Common interface
	    "LCD:NewHaven_NHD43480272EF_ASXP:16Bit",
	    0x0100,
	    LCD_NHD43480272EF_ASXP_InitializeWorkspace_16Bit,
	    sizeof(T_NHD43480272EF_ASXPWorkspace),
	},
	
    // Functions
    LCD_NHD43480272EF_ASXP_Open,
    LCD_NHD43480272EF_ASXP_Close,
    LCD_NHD43480272EF_ASXP_Configure,
    LCD_NHD43480272EF_ASXP_GetInfo,
    LCD_NHD43480272EF_ASXP_GetFrame,
    LCD_NHD43480272EF_ASXP_ShowFrame,
    LCD_NHD43480272EF_ASXP_On,
    LCD_NHD43480272EF_ASXP_Off,
    LCD_NHD43480272EF_ASXP_SetBacklightLevel,
    LCD_NHD43480272EF_ASXP_GetBacklightLevel,
    LCD_NHD43480272EF_ASXP_SetPaletteColor,
    // v2.04
    LCD_NHD43480272EF_ASXP_WaitForVerticalSync,
};

const DEVICE_LCD LCD_NewHaven_NHD43480272EF_ASXP_Interface_I15Bit = {
	{
	    // Common interface
	    "LCD:NewHaven_NHD43480272EF_ASXP:I15Bit",
	    0x0100,
	    LCD_NHD43480272EF_ASXP_InitializeWorkspace_I15Bit,
	    sizeof(T_NHD43480272EF_ASXPWorkspace),
	},
...
const DEVICE_LCD LCD_NewHaven_NHD43480272EF_ASXP_Interface_8Bit = {
	{
	    // Common interface
	    "LCD:NewHaven_NHD43480272EF_ASXP:8Bit",
	    0x0100,
	    LCD_NHD43480272EF_ASXP_InitializeWorkspace_8Bit,
	    sizeof(T_NHD43480272EF_ASXPWorkspace),
	},
...
// List of interfaces
const T_uezDeviceInterface *LCD_NewHaven_NHD43480272EF_ASXP_InterfaceArray[] = {
    0,      // 1 bit
    0,      // 2 bit
    0,      // 4 bit
    (T_uezDeviceInterface *)&LCD_NewHaven_NHD43480272EF_ASXP_Interface_8Bit,      // 8 bit
    (T_uezDeviceInterface *)&LCD_NewHaven_NHD43480272EF_ASXP_Interface_16Bit,      // 16 bit
    (T_uezDeviceInterface *)&LCD_NewHaven_NHD43480272EF_ASXP_Interface_I15Bit,      // I15 bit
    0,      // 24 bit
};

/*-------------------------------------------------------------------------*
 * End of File:  NewHaven_NHD43480272EF_ASXP.c
 *-------------------------------------------------------------------------*/
