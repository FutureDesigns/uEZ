/*-------------------------------------------------------------------------*
 * File:  NewHaven_NHD50800480TF.c
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the NewHaven_NHD50800480TF.
 * This LCD shipped on Rev 1.X of uEZGUI-4357-50WVN only.
 *-------------------------------------------------------------------------*/
...
} T_NHD50800480TFWorkspace;

/*---------------------------------------------------------------------------*
 * Globals:
 *---------------------------------------------------------------------------*/
static T_LCDControllerSettings LCD_NHD50800480TF_settings;
#if (LCD_BACKLIGHT_FREERTOS_TIMER==1)
...
static const T_LCDControllerSettings LCD_NHD50800480TF_params8bit = {
    LCD_ADVANCED_TFT,
    LCD_COLOR_RES_8,
...
static T_uezLCDConfiguration LCD_NHD50800480TF_configuration_16Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_NHD50800480TF_configuration_I15Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_NHD50800480TF_configuration_8Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
 ...
 * Prototypes:
 *---------------------------------------------------------------------------*/
extern const DEVICE_LCD LCD_NewHaven_NHD50800480TF_Interface;

/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD50800480TF_InitializeWorkspace_16Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for NHD50800480TF LCD.
 * Inputs:
 *      void *aW                    -- Particular workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_NHD50800480TF_InitializeWorkspace_16Bit(void *aW)
{
    T_NHD50800480TFWorkspace *p = (T_NHD50800480TFWorkspace *)aW;
    p->iBaseAddress = LCD_DISPLAY_BASE_ADDRESS;
    p->aNumOpen = 0;
    p->iBacklightLevel = 0; // 0%
    p->iConfiguration = &LCD_NHD50800480TF_configuration_16Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD50800480TF_InitializeWorkspace_I15Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for NHD50800480TF LCD.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_NHD50800480TF_InitializeWorkspace_I15Bit(void *aW)
{
    T_NHD50800480TFWorkspace *p = (T_NHD50800480TFWorkspace *)aW;
    p->iBaseAddress = LCD_DISPLAY_BASE_ADDRESS;
    p->aNumOpen = 0;
    p->iBacklightLevel = 0; // 0%
    p->iConfiguration = &LCD_NHD50800480TF_configuration_I15Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD50800480TF_InitializeWorkspace_8Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for NHD50800480TF LCD.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_NHD50800480TF_InitializeWorkspace_8Bit(void *aW)
{
    T_NHD50800480TFWorkspace *p = (T_NHD50800480TFWorkspace *)aW;
    p->iBaseAddress = LCD_DISPLAY_BASE_ADDRESS;
    p->aNumOpen = 0;
    p->iBacklightLevel = 0; // 0%
    p->iConfiguration = &LCD_NHD50800480TF_configuration_8Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD50800480TF_Close
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD50800480TF_Close(void *aW)
{
    T_NHD50800480TFWorkspace *p = (T_NHD50800480TFWorkspace *)aW;
    p->aNumOpen--;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD50800480TF_Configure
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_NHD50800480TF_Configure(
            void *aW,
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD50800480TF_GetInfo
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD50800480TF_GetInfo(void *aW, T_uezLCDConfiguration *aConfiguration)
{
    T_NHD50800480TFWorkspace *p = (T_NHD50800480TFWorkspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD50800480TF_GetFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD50800480TF_GetFrame(
            void *aW,
            TUInt32 aFrame,
            void **aFrameBuffer)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD50800480TF_ShowFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD50800480TF_ShowFrame(void *aW, TUInt32 aFrame)
{
    T_NHD50800480TFWorkspace *p = (T_NHD50800480TFWorkspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD50800480TF_SetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD50800480TF_SetBacklightLevel(void *aW, TUInt32 aLevel)
{
    T_NHD50800480TFWorkspace *p = (T_NHD50800480TFWorkspace *)aW;
    TUInt16 level;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD50800480TF_MSTimerStart
 *---------------------------------------------------------------------------*
 * Description:
...
#else
 #if (LCD_BACKLIGHT_FREERTOS_TIMER==1)
static T_uezError LCD_NHD50800480TF_MSTimerStart(void *aW, float milliseconds) {
  T_NHD50800480TFWorkspace *p = (T_NHD50800480TFWorkspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD50800480TF_TimerCallback
 *---------------------------------------------------------------------------*
 * Description:
...
#else
 #if (LCD_BACKLIGHT_FREERTOS_TIMER==1)
static void LCD_NHD50800480TF_TimerCallback( TimerHandle_t xTimer ) {
    //p->itimerDone = ETrue;
 ...
 #else
static void LCD_NHD50800480TF_TimerCallback(T_uezTimerCallback *aCallbackWorkspace){
  T_NHD50800480TFWorkspace *p = aCallbackWorkspace->iData;
  p->itimerDone = ETrue;
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD50800480TF_On(void *aW) {  
    T_NHD50800480TFWorkspace *p = (T_NHD50800480TFWorkspace *)aW;
    ...
    if (p->iBacklight){
...
#else
 #if (LCD_BACKLIGHT_FREERTOS_TIMER==1)
      LCD_NHD50800480TF_MSTimerStart(p, 167.0); // minimum 167ms timer
      ...
      if (UEZTimerOpen("Timer0", &p->itimer) == UEZ_ERROR_NONE) {
       ...
       LCD_NHD50800480TF_SetBacklightLevel(p, p->iBacklightLevel); // Turn back on to the remembered level
    }
    return UEZ_ERROR_NONE;
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD50800480TF_Off
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD50800480TF_Off(void *aW) {
    T_NHD50800480TFWorkspace *p = (T_NHD50800480TFWorkspace *)aW;    
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD50800480TF_Open
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD50800480TF_Open(void *aW)
{
    T_NHD50800480TFWorkspace *p = (T_NHD50800480TFWorkspace *)aW;
        ...
        if (!error) {
            switch (p->iConfiguration->iColorDepth) {
                case UEZLCD_COLOR_DEPTH_8_BIT:
                    LCD_NHD50800480TF_settings = LCD_NHD50800480TF_params8bit;
                    break;
                default:
                case UEZLCD_COLOR_DEPTH_16_BIT:
                    LCD_NHD50800480TF_settings = LCD_NHD50800480TF_params16bit;
                    break;
                case UEZLCD_COLOR_DEPTH_I15_BIT:
                    LCD_NHD50800480TF_settings = LCD_NHD50800480TF_paramsI15bit;
                    break;
            }
            LCD_NHD50800480TF_settings.iBaseAddress = p->iBaseAddress;
            error = (*plcdc)->Configure(plcdc, &LCD_NHD50800480TF_settings);
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD50800480TF_GetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  no backlight for LCD.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD50800480TF_GetBacklightLevel(
        void *aW, 
        TUInt32 *aLevel,
        TUInt32 *aNumLevels)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD50800480TF_SetPaletteColor
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD50800480TF_SetPaletteColor(
                    void *aWorkspace,
                    TUInt32 aColorIndex,
                    TUInt16 aRed,
                    TUInt16 aGreen,
                    TUInt16 aBlue)
{
    T_NHD50800480TFWorkspace *p = (T_NHD50800480TFWorkspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD50800480TF_VerticalSyncCallback
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static void LCD_NHD50800480TF_VerticalSyncCallback(void *aCallbackWorkspace)
{
    T_NHD50800480TFWorkspace *p = (T_NHD50800480TFWorkspace *)aCallbackWorkspace;

    _isr_UEZSemaphoreRelease(p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_NHD50800480TF_WaitForVerticalSync
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_NHD50800480TF_WaitForVerticalSync(
    void *aWorkspace,
    TUInt32 aTimeout)
{
    T_NHD50800480TFWorkspace *p = (T_NHD50800480TFWorkspace *)aWorkspace;
    ...
    (*p_hal)->EnableVerticalSync(p_hal, LCD_NHD50800480TF_VerticalSyncCallback, p);
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const DEVICE_LCD LCD_NewHaven_NHD50800480TF_Interface_16Bit = {
	{
	    // Common interface
	    "LCD:NewHaven_NHD50800480TF:16Bit",
	    0x0100,
	    LCD_NHD50800480TF_InitializeWorkspace_16Bit,
	    sizeof(T_NHD50800480TFWorkspace),
	},
	
    // Functions
    LCD_NHD50800480TF_Open,
    LCD_NHD50800480TF_Close,
    LCD_NHD50800480TF_Configure,
    LCD_NHD50800480TF_GetInfo,
    LCD_NHD50800480TF_GetFrame,
    LCD_NHD50800480TF_ShowFrame,
    LCD_NHD50800480TF_On,
    LCD_NHD50800480TF_Off,
    LCD_NHD50800480TF_SetBacklightLevel,
    LCD_NHD50800480TF_GetBacklightLevel,
    LCD_NHD50800480TF_SetPaletteColor,
    // v2.04
    LCD_NHD50800480TF_WaitForVerticalSync,
};

const DEVICE_LCD LCD_NewHaven_NHD50800480TF_Interface_I15Bit = {
	{
	    // Common interface
	    "LCD:NewHaven_NHD50800480TF:I15Bit",
	    0x0100,
	    LCD_NHD50800480TF_InitializeWorkspace_I15Bit,
	    sizeof(T_NHD50800480TFWorkspace),
	},
...
const DEVICE_LCD LCD_NewHaven_NHD50800480TF_Interface_8Bit = {
	{
	    // Common interface
	    "LCD:NewHaven_NHD50800480TF:8Bit",
	    0x0100,
	    LCD_NHD50800480TF_InitializeWorkspace_8Bit,
	    sizeof(T_NHD50800480TFWorkspace),
	},
...
// List of interfaces
const T_uezDeviceInterface *LCD_NewHaven_NHD50800480TF_InterfaceArray[] = {
    0,      // 1 bit
    0,      // 2 bit
    0,      // 4 bit
    (T_uezDeviceInterface *)&LCD_NewHaven_NHD50800480TF_Interface_8Bit,      // 8 bit
    (T_uezDeviceInterface *)&LCD_NewHaven_NHD50800480TF_Interface_16Bit,      // 16 bit
    (T_uezDeviceInterface *)&LCD_NewHaven_NHD50800480TF_Interface_I15Bit,      // I15 bit
    0,      // 24 bit
};

/*-------------------------------------------------------------------------*
 * End of File:  NewHaven_NHD50800480TF.c
 *-------------------------------------------------------------------------*/
