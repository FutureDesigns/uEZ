/*-------------------------------------------------------------------------*
 * File:  EEPROM_NXP_LPC546xx_driver.c
 *-------------------------------------------------------------------------*
 * Description:
 *      Device implementation of the EEPROM for the LPC546xx
 *-------------------------------------------------------------------------*/
...
#define EEPROM_START                    (0x40108000)
...
#define LPC546xx_EEPROM_PAGE_SIZE         EEPROM_PAGE_SIZE
...
} T_EEPROM_NXP_LPC546xx_Workspace;
...
static T_EEPROM_NXP_LPC546xx_Workspace *G_lpc546xxEEPROMWorkspace;

/*---------------------------------------------------------------------------*
 * Routine:  EEPROM_NXP_LPC546xx_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup of this workspace for LPC546xx.
 * Inputs:
 *      void *aW                    -- Workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
static T_uezError EEPROM_NXP_LPC546xx_InitializeWorkspace(void *aW)
{
    T_uezError error;

    T_EEPROM_NXP_LPC546xx_Workspace *p = (T_EEPROM_NXP_LPC546xx_Workspace *)aW;
    ...
    p->iConfig.iPageSize = LPC546xx_EEPROM_PAGE_SIZE;
    p->iConfig.iWriteTime = 25;
    p->iInitialized = EFalse;

    // Record the global
    G_lpc546xxEEPROMWorkspace = p;

    return error;
}

static void IProcessIRQ(T_EEPROM_NXP_LPC546xx_Workspace *p)
{
    TUInt32 status = EEPROM->INTSTAT;
...
{
    IRQ_START();
    IProcessIRQ(G_lpc546xxEEPROMWorkspace);
    IRQ_END();
}
 ...
 * Inputs:
 *      T_EEPROM_NXP_LPC546xx_Workspace *p -- Workspace
 *---------------------------------------------------------------------------*/
static void IInitialize(T_EEPROM_NXP_LPC546xx_Workspace *p)
{
    uint32_t v;

    LPC546xxPowerOn(kCLOCK_Eeprom);
    POWER_DisablePD(kPDRUNCFG_PD_EEPROM);//Turn on power
    ...
    EEPROM->INTENSET = ((0x1 << END_OF_RDWR) | (0x1 << END_OF_PROG));
...
/*---------------------------------------------------------------------------*
 * Routine:  EEPROM_NXP_LPC546xx_Read
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError EEPROM_NXP_LPC546xx_Read(
        void *aWorkspace,
        TUInt32 aAddress,
        TUInt8 *aBuffer,
        TUInt32 aNumBytes)
{
    T_EEPROM_NXP_LPC546xx_Workspace *p = (T_EEPROM_NXP_LPC546xx_Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  EEPROM_NXP_LPC546xx_Write
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError EEPROM_NXP_LPC546xx_Write(
        void *aWorkspace,
        TUInt32 aAddress,
        TUInt8 *aBuffer,
        TUInt32 aNumBytes)
{
    ...
    while (size) {
        ...
        EEPROM->INTSTATCLR = 1<<2;             // Clear Program int32_t status bit
...
T_uezError EEPROM_NXP_LPC546xx_SetConfiguration(
        void *aWorkspace,
...
void EEPROM_NXP_LPC546xx_Create(const char *aName)
{
    UEZDeviceTableRegister(
            aName,
            (T_uezDeviceInterface *)&EEPROM_NXP_LPC546xx_Interface,
            0,
            0);
}
 ...
 *---------------------------------------------------------------------------*/
const DEVICE_EEPROM EEPROM_NXP_LPC546xx_Interface = {
	{
		// Common device interface
	    "EEPROM:NXP:LPC546xx",
	    0x0100,
	    EEPROM_NXP_LPC546xx_InitializeWorkspace,
	    sizeof(T_EEPROM_NXP_LPC546xx_Workspace),
	},

    // Functions
    EEPROM_NXP_LPC546xx_SetConfiguration,
    EEPROM_NXP_LPC546xx_Read,
    EEPROM_NXP_LPC546xx_Write,
} ;

/*-------------------------------------------------------------------------*
 * End of File:  EEPROM_NXP_LPC546xx_driver.c
 *-------------------------------------------------------------------------*/
