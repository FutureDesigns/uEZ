...
static UEZ_PUT_SECTION(".usbhostmem", TUInt8 G_usbHostMemory[8*1024]);
...
static UEZ_PUT_SECTION(".mcimem", TUInt8 G_mciDummyMemory[16*136*4]);
...
{ // Per manual can read set register to determine the set state.
    if((LPC_GPIO4->SET & (1 << 23)) == (1 << 23)){ // if on
      LPC_GPIO4->CLR |= (1 << 23); // set off
    } else {
      LPC_GPIO4->SET |= (1 << 23); // set on
    }
}
...
/* For now just use dummy functions so standard main.c routines will build,
 * but currently we don't have a port b host included on CARRIER board, but it 
 * should be possible to upgrade CARRIER to change mini-B to OTG port. */
TBool UEZPlatform_Host_Port_B_Detect()
{
    return ETrue; // return true that port b is device port
}
...
{    
    ...
    if(!error) {   
      // In case audio mixer not used set this to onboard speaker to prevent overdrive.
...
void UEZPlatform_AudioAmp_1_Require(void)
{
    ...
    G_Amp_In_Use = AUDIO_AMP_WOLFSON;
}
            ...
            TUInt8 aLevel)
{
    ...
    switch(aChangedOutput){
        ...
        /*case  UEZ_AUDIO_MIXER_OUTPUT_OFFBOARD_SPEAKER:
            ...
            break;*/
        case UEZ_AUDIO_MIXER_OUTPUT_ONBOARD_HEADPHONES:// TODO add and test volume scaling
            if (G_Amp1){
                if(aMute){
...
void UEZPlatform_AudioMixer_Require(void)
{
    ...
    UEZAudioMixerSetLevel(UEZ_AUDIO_MIXER_OUTPUT_ONBOARD_HEADPHONES, UEZ_DEFAULT_AUDIO_LEVEL);
