...
UEZ_PUT_SECTION(".frames", TUInt8 _framesMemory [FRAME_SIZE*2]); // only allocate 2 frames permanently, use heap for more
...
#if UEZGUI_EXPANSION_DEVKIT // mci memory should exist
#else
  #if UEZGUI_EXP_BRK_OUT    // mci memory should exist
...
T_uezError UEZPlatform_I2C1_Reset(void) // Not tested on this uezgui yet
{
...
{    
    ...
    if(!error) {
        AudioAmp_LM48110_Create("AMP0", "I2C1", 0x1F); // New Revs use I2C AMP
    } else {
        LPC17xx_40xx_GPIO0_Require();
        // P2.3 = VOL_UD
        // P2.5 = AMP_MODE
        AudioAmp_8551T_Create("AMP0", GPIO_P2_3, GPIO_P2_5, GPIO_NONE, 64); // Part Discontinued
    }
...
void UEZPlatform_WiFiRunMode(void)
{
    ...
    UEZGPIOClear(GPIO_P1_6);          // WIFI PROGRAM OFF
    UEZGPIOOutput(GPIO_P1_6);       // WIFI PROGRAM OFF
