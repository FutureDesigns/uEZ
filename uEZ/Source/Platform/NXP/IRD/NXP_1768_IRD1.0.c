...
void UEZPlatform_CharDisplay_Require(void)
{
    ...
    LPC1768_GPIO0_Require();
    LPC1768_GPIO2_Require();
    //TODO FIX ******************Lumex_SO2004DSR_Create("CharDisplay", GPIO_P2_10, GPIO_P0_22, GPIO_P2_8,GPIO_P2_0);
}
...
void UEZBSP_FatalError(int32_t aErrorCode)
{
   ...
   // LPC_GPIO0->DIR |= (1 << 7);
    ...
    HAL_GPIOPort **p_gpio;
    HALInterfaceFind("GPIO0", (T_halWorkspace **)&p_gpio);
    (*p_gpio)->SetOutputMode(p_gpio, 1 << 7);
    (*p_gpio)->SetMux(p_gpio, 7, 0); // set to GPIO
    ...
    count = 0;
    while (1) {
        //LPC_GPIO0->SET |= (1 << 7); // on
        (*p_gpio)->Set(p_gpio, 1 << 7);
        for (i = 0; i < 15000000; i++)
            nop();
        //LPC_GPIO0->CLR |= (1 << 7); // off
        (*p_gpio)->Clear(p_gpio, 1 << 7);
        for (i = 0; i < 15000000; i++)
            nop();
        count++;
        if (count >= aErrorCode) {
...
/*
    ...
    (*p_gpio)->SetOutputMode(p_gpio, 1 << 26);
    (*p_gpio)->SetMux(p_gpio, 26, 0); // set to GPIO
    (*p_gpio)->Clear(p_gpio, 1 << 26);
