...
/*---------------------------------------------------------------------------*
 * Routine:  ILPC546xx_CRC_SetComputationType
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
T_uezError ILPC546xx_CRC_SetComputationType(
        void *aWorkspace,
        T_uezCRCComputationType aType)
{
    switch (aType) {
        case UEZ_CRC_16_CCITT:
            CRC_ENGINE->MODE = 0x00;
            break;
        case UEZ_CRC_16_IBM:
            CRC_ENGINE->MODE = 0x15;
            break;
        case UEZ_CRC_32_ANSI:
            CRC_ENGINE->MODE = 0x36;
            break;
        default:
...
/*---------------------------------------------------------------------------*
 * Routine:  ILPC546xx_CRC_AddModifier
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError ILPC546xx_CRC_AddModifier(
        void *aWorkspace,
...
/*---------------------------------------------------------------------------*
 * Routine:  ILPC546xx_CRC_RemoveModifier
 *---------------------------------------------------------------------------*
 * Description:
 *      Add a modifier to the algorithm.  If the modifier already has been
 *      added, do nothing.
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError ILPC546xx_CRC_RemoveModifier(
        void *aWorkspace,
        T_uezCRCModifier aModifier)
{
    TUInt32 bits = ICRCModifierBits(aModifier);
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError ILPC546xx_CRC_SetSeed(void *aWorkspace, TUInt32 aSeed)
{
    PARAM_NOT_USED(aWorkspace);

    CRC_ENGINE->SEED = aSeed;

    return UEZ_ERROR_NONE;
}

/*---------------------------------------------------------------------------*
 * Routine:  ILPC546xx_CRC_ComputeData
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError ILPC546xx_CRC_ComputeData(
        void *aWorkspace,
        void *aData,
    ...
    switch (aSize) {
        case UEZ_CRC_DATA_SIZE_UINT8: {
            ...
            while (aNumDataElements--)
                CRC_ENGINE->WR_DATA = *(p8++);
        }
            break;
        case UEZ_CRC_DATA_SIZE_UINT16: {
            p16 = (TUInt16 *)aData;
            while (aNumDataElements--)
                CRC_ENGINE->WR_DATA = *(p16++);
        }
            break;
        case UEZ_CRC_DATA_SIZE_UINT32: {
            p32 = (TUInt32 *)aData;
            while (aNumDataElements--)
                CRC_ENGINE->WR_DATA = *(p32++);
        }
            break;
        default:
            return UEZ_ERROR_INVALID_PARAMETER;
...
/*---------------------------------------------------------------------------*
 * Routine:  ILPC546xx_CRC_ReadChecksum
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError ILPC546xx_CRC_ReadChecksum(void *aWorkspace, TUInt32 *aValue)
{
    PARAM_NOT_USED(aWorkspace);

    *aValue = (TUInt32)CRC_ENGINE->SUM;

    return UEZ_ERROR_NONE;
}

/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_CRC_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC546xx_CRC_InitializeWorkspace(void *aWorkspace)
{
    //T_LPC546xx_CRC_Workspace *p = (T_LPC546xx_CRC_Workspace *)aWorkspace;
    PARAM_NOT_USED(aWorkspace);
...
/*---------------------------------------------------------------------------*
 * Interface: LPC546xx CRC
 *---------------------------------------------------------------------------*/
const HAL_CRC CRC_LPC546xx_Interface = {
        {
                "NXP:LPC546xx:CRC",
                0x0110,
                LPC546xx_CRC_InitializeWorkspace,
                sizeof(T_LPC546xx_CRC_Workspace), },

        ILPC546xx_CRC_SetComputationType,
        ILPC546xx_CRC_AddModifier,
        ILPC546xx_CRC_RemoveModifier,
        ILPC546xx_CRC_SetSeed,
        ILPC546xx_CRC_ComputeData,
        ILPC546xx_CRC_ReadChecksum };
