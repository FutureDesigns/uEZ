/*-------------------------------------------------------------------------*
 * File:  LPC546xx_EMAC.c
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the LPC546xx EMAC (Ethernet) Driver.
 * Implementation:
 *
 *-------------------------------------------------------------------------*/
 ...
 * Options:
 *---------------------------------------------------------------------------*/
#ifndef DEBUG_LPC546xx_EMAC_STARTUP
#define DEBUG_LPC546xx_EMAC_STARTUP  1
#endif

#ifndef DEBUG_LPC546xx_EMAC_PACKETS
#define DEBUG_LPC546xx_EMAC_PACKETS               0
#endif

#define DEBUG_LPC546xx_EMAC_PACKETS_IN       DEBUG_LPC546xx_EMAC_PACKETS
#define DEBUG_LPC546xx_EMAC_PACKETS_OUT      DEBUG_LPC546xx_EMAC_PACKETS

#ifndef EMAC_USE_BITBANG_MDIO_DETECT
/* Bit banging to the EMAC can be used to avoid the dread LPC546xx lock up
 * that occurs when a PHY is attempting to be communicated with and there
 * is NO oscillation.  The LPC546xx state machine locks up hard and only
 ...
 * Globals:
 *---------------------------------------------------------------------------*/
extern const HAL_EMAC EMAC_LPC546xx_Interface;
static T_LPC546xx_EMAC_Workspace *G_LPC546xx_EMAC;
 ...
 * Inputs:
 *      T_LPC546xx_EMAC_Workspace *p -- EMAC's Workspace
 *      TUInt32 aPhyReg             -- PHY register to write
 *      TUInt32 aValue              -- Value to write to register
 *---------------------------------------------------------------------------*/
static void IPHYWrite(T_LPC546xx_EMAC_Workspace *p, TUInt8 PhyReg, TUInt32 Value)
{
    TUInt32 timeout;
 ...
 *      TUInt32 aValue              -- Value to write to register
 *---------------------------------------------------------------------------*/
static TUInt16 IPHYRead(T_LPC546xx_EMAC_Workspace *p, TUInt8 PhyReg)
{
    TUInt32 timeout;
 ...
 *---------------------------------------------------------------------------*/
static T_uezError IEMACConfigPHY_National(T_LPC546xx_EMAC_Workspace *p)
{
 ...
 *      T_LPC17xx_40xx_EMAC_Workspace *p -- EMAC's Workspace
 *---------------------------------------------------------------------------*/
static T_uezError IEMACConfigPHY_Micrel(T_LPC546xx_EMAC_Workspace *p)
{
    T_uezError error = UEZ_ERROR_NONE;
    TUInt32 timeout, regv;
#if DEBUG_LPC546xx_EMAC_STARTUP
    TUInt32 reg;
#endif
    // auto negotiation, restart AN
    // RE_AN should be self cleared
...
/*---------------------------------------------------------------------------*
 * Routine:  ILPC546xx_EMAC_InitRxDescriptors
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *
 *---------------------------------------------------------------------------*/
static void ILPC546xx_EMAC_InitRxDescriptors(void)
{
    TUInt32 i;
...
/*---------------------------------------------------------------------------*
 * Routine:  ILPC546xx_EMAC_InitTxDescriptors
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *
 *---------------------------------------------------------------------------*/
void ILPC546xx_EMAC_InitTxDescriptors(void)
{
    TUInt32 i;
...
static T_uezError LPC546xx_EMAC_DetectEMAC(T_LPC546xx_EMAC_Workspace *p)
{
    ...
    if ((pinMDC != GPIO_NONE) && (pinMDIO != GPIO_NONE)) {
        ...
        for (n=0; n<32; n++) {
#if DEBUG_LPC546xx_EMAC_STARTUP
            printf("LPC546xx_EMAC_DetectEMAC: PHY %d, Reg 2 = ", n);
#endif
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_EMAC_Configure
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
T_uezError LPC546xx_EMAC_Configure(void *aWorkspace, T_EMACSettings *aSettings)
{
    T_uezError error = UEZ_ERROR_NONE;
    T_LPC546xx_EMAC_Workspace *p = (T_LPC546xx_EMAC_Workspace *)aWorkspace;
    ...
    // Because the LPC546xx has a problem with EMAC's that are missing,
    ...
    ILPC546xx_EMAC_InitTxDescriptors();
    ILPC546xx_EMAC_InitRxDescriptors();
    p->iReceiveIndex = 0;
    p->iTransmitIndex = 0;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_EMAC_CheckFrameReceived
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
TBool LPC546xx_EMAC_CheckFrameReceived(void *aWorkspace)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_EMAC_StartReadFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
TUInt16 LPC546xx_EMAC_StartReadFrame(void *aWorkspace)
{
    TUInt16 RxLen = 0;
    T_LPC546xx_EMAC_Workspace *p = (T_LPC546xx_EMAC_Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_EMAC_EndReadFrame
 *---------------------------------------------------------------------------*
 * Description:
 *      End the last frame.
 * Inputs:
 *      void *aWorkspace          -- EMAC Controller's workspace
 *---------------------------------------------------------------------------*/
void LPC546xx_EMAC_EndReadFrame(void *aWorkspace)
{
    T_LPC546xx_EMAC_Workspace *p = (T_LPC546xx_EMAC_Workspace *)aWorkspace;
    ENET_ENHRXDESC_T *pd = &RXDescs[p->iReceiveIndex];
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_EMAC_RequestSend
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      void *aWorkspace          -- EMAC Controller's workspace
 *---------------------------------------------------------------------------*/
void LPC546xx_EMAC_RequestSend(void *aWorkspace)
{
    T_LPC546xx_EMAC_Workspace *p = (T_LPC546xx_EMAC_Workspace *)aWorkspace;
...
#if DEBUG_LPC546xx_EMAC_PACKETS_OUT
 ...
 *---------------------------------------------------------------------------*/
void LPC546xx_EMAC_DoSend(void *aWorkspace, TUInt16 aFrameSize)
{
    T_LPC546xx_EMAC_Workspace *p = (T_LPC546xx_EMAC_Workspace *)aWorkspace;
    ENET_ENHTXDESC_T *pd = &TXDescs[p->iTransmitIndex];
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_EMAC_CopyToFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
void LPC546xx_EMAC_CopyToFrame(void *aWorkspace, void *aSource, TUInt32 aSize)
{
    T_LPC546xx_EMAC_Workspace *p = (T_LPC546xx_EMAC_Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_EMAC_CopyFromFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
void LPC546xx_EMAC_CopyFromFrame(
    void *aWorkspace,
    void *aDestination,
    TUInt32 aSize)
{
...
#if DEBUG_LPC546xx_EMAC_PACKETS_IN
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_EMAC_ProcessInterrupt
 *---------------------------------------------------------------------------*
 * Description:
 *      An interrupt on this EMAC has been received.  Catch it and
 *      possible pass it on to a callback routine.
 *---------------------------------------------------------------------------*/
static void LPC546xx_EMAC_ProcessInterrupt(void)
{
    T_LPC546xx_EMAC_Workspace *p = G_LPC546xx_EMAC;
    TUInt32 status;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_EMAC_ISR
 *---------------------------------------------------------------------------*
 * Description:
 *      Stub to catch a EMAC interrupt.
 *---------------------------------------------------------------------------*/
IRQ_ROUTINE( LPC546xx_EMAC_ISR)
{
    IRQ_START();
    LPC546xx_EMAC_ProcessInterrupt();
    IRQ_END();
}

/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_EMAC_EnableReceiveInterrupt
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *          or 0 for none.
 *---------------------------------------------------------------------------*/
void LPC546xx_EMAC_EnableReceiveInterrupt(
    void *aWorkspace,
    ...
    void *aCallbackWorkspace)
{
    ...
    InterruptRegister(ETHERNET_IRQn, (TISRFPtr)LPC546xx_EMAC_ISR,
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_EMAC_DisableReceiveInterrupt
 *---------------------------------------------------------------------------*
 * Description:
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_EMAC_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
T_uezError LPC546xx_EMAC_InitializeWorkspace(void *aWorkspace)
{
    T_LPC546xx_EMAC_Workspace *p = (T_LPC546xx_EMAC_Workspace *)aWorkspace;
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const HAL_EMAC G_LPC546xx_EMAC_Interface = {
    {
        "LPC546xx EMAC",
        0x0100,
        LPC546xx_EMAC_InitializeWorkspace,
        sizeof(T_LPC546xx_EMAC_Workspace), },

    LPC546xx_EMAC_Configure,
    LPC546xx_EMAC_CheckFrameReceived,
    LPC546xx_EMAC_StartReadFrame,
    LPC546xx_EMAC_EndReadFrame,
    LPC546xx_EMAC_RequestSend,
    LPC546xx_EMAC_DoSend,
    LPC546xx_EMAC_CopyToFrame,
    LPC546xx_EMAC_CopyFromFrame,

    // uEZ v1.10
    LPC546xx_EMAC_EnableReceiveInterrupt,
    LPC546xx_EMAC_DisableReceiveInterrupt,
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_EMAC_RMII_Require
 *---------------------------------------------------------------------------*
 * Description:
 *      Creates the EMAC HAL driver for the LPC546xx device.  Also looks
 ...
 * Inputs:
 *      T_LPC546xx_EMAC_Settings *aSettings -- Settings to configure EMAC as
...
#define EMAC_DRIVE(mode)          SCU_NORMAL_DRIVE(mode, \
                                                   ...
                                                   SCU_EZI_ENABLE, \
                                                   SCU_ZIF_ENABLE)
void LPC546xx_EMAC_RMII_Require(const T_LPC546xx_EMAC_Settings *aSettings)
{
    ...
    if (LPC546xx_EMAC_DetectEMAC(p) == UEZ_ERROR_NONE)
        p->iIsDetected = ETrue;
#else
    p->iIsDetected = ETrue;
#endif
