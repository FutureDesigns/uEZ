 ...
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the LPC546xx Timers Interface.
 * Implementation:
...
} T_LPC546xx_Timer_Info;

typedef struct {
    T_HALTimer_Callback iCallbackFunc;
    void *iCallbackWorkspace;
} T_LPC546xx_Timer_InterruptCallback;

typedef struct {
    const HAL_Timer *iHAL;
    T_LPC546xx_Timer_Registers *iReg;
    T_LPC546xx_Timer_InterruptCallback iCallbacks[4]; // one per match register
    const T_LPC546xx_Timer_Info *iInfo;
} T_LPC546xx_Timer_Workspace;

/*---------------------------------------------------------------------------*
 * Prototypes:
 *---------------------------------------------------------------------------*/
static const T_LPC546xx_Timer_Info G_Timer0_Info;
static const T_LPC546xx_Timer_Info G_Timer1_Info;
static const T_LPC546xx_Timer_Info G_Timer2_Info;
static const T_LPC546xx_Timer_Info G_Timer3_Info;
T_LPC546xx_Timer_Workspace *G_Timer0_Workspace;
T_LPC546xx_Timer_Workspace *G_Timer1_Workspace;
T_LPC546xx_Timer_Workspace *G_Timer2_Workspace;
T_LPC546xx_Timer_Workspace *G_Timer3_Workspace;
static void ITimerProcessIRQ(T_LPC546xx_Timer_Workspace *p);
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_Timer_SetTimerMode
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LPC546xx_Timer_SetTimerMode(
        void *aWorkspace,
        T_Timer_Mode aMode)
{
    T_LPC546xx_Timer_Workspace *p = (T_LPC546xx_Timer_Workspace *)aWorkspace;
    T_LPC546xx_Timer_Registers *r = p->iReg;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_Timer_SetCaptureSource
 *---------------------------------------------------------------------------*
 * Description:
 *      Timer is going to start -- chip select enabled for device
 * Inputs:
 ...
 *                                  does not exist.
 *---------------------------------------------------------------------------*/
static T_uezError LPC546xx_Timer_SetCaptureSource(
        void *aWorkspace,
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_Timer_SetMatchRegister
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  when match occurs.
 *---------------------------------------------------------------------------*/
static T_uezError LPC546xx_Timer_SetMatchRegister(
        void *aWorkspace,
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_Timer_SetExternalControl
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LPC546xx_Timer_SetExternalControl(
        void *aWorkspace,
        TUInt8 aMatchRegister,
 ...
 *      void *aWorkspace        -- Timer's workspace
 *---------------------------------------------------------------------------*/
static void LPC546xx_Timer_Reset(void *aWorkspace)
{
    T_LPC546xx_Timer_Workspace *p = (T_LPC546xx_Timer_Workspace *)aWorkspace;
    T_LPC546xx_Timer_Registers *r = p->iReg;
    int i;
 ...
 *      void *aWorkspace        -- Timer's workspace
 *---------------------------------------------------------------------------*/
static void LPC546xx_Timer_Enable(void *aWorkspace)
{
    T_LPC546xx_Timer_Workspace *p = (T_LPC546xx_Timer_Workspace *)aWorkspace;
    T_LPC546xx_Timer_Registers *r = p->iReg;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_Timer_Disable
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      void *aWorkspace        -- Timer's workspace
 *---------------------------------------------------------------------------*/
static void LPC546xx_Timer_Disable(void *aWorkspace)
{
    T_LPC546xx_Timer_Workspace *p = (T_LPC546xx_Timer_Workspace *)aWorkspace;
    T_LPC546xx_Timer_Registers *r = p->iReg;
 ...
 *          of range.
 *---------------------------------------------------------------------------*/
static T_uezError LPC546xx_Timer_SetMatchCallback(
        void *aWorkspace,
        TUInt8 aMatchRegister,
 ...
 *---------------------------------------------------------------------------*/
static void IClear(T_LPC546xx_Timer_InterruptCallback aCallbacks[4])
{
    TUInt8 i;
    for (i = 0; i < 4; i++) {
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_Timer0_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC546xx Timer0 workspace.
 * Inputs:
 *      void *aW                    -- Particular Timer workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC546xx_Timer0_InitializeWorkspace(void *aWorkspace)
{
    T_LPC546xx_Timer_Workspace *p = (T_LPC546xx_Timer_Workspace *)aWorkspace;
    p->iReg = (T_LPC546xx_Timer_Registers *)LPC_TIMER0_BASE;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_Timer1_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC546xx Timer1 workspace.
 * Inputs:
 *      void *aW                    -- Particular Timer workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC546xx_Timer1_InitializeWorkspace(void *aWorkspace)
{
    T_LPC546xx_Timer_Workspace *p = (T_LPC546xx_Timer_Workspace *)aWorkspace;
    p->iReg = (T_LPC546xx_Timer_Registers *)LPC_TIMER1_BASE;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_Timer2_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC546xx Timer1 workspace.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC546xx_Timer2_InitializeWorkspace(void *aWorkspace)
{
    T_LPC546xx_Timer_Workspace *p = (T_LPC546xx_Timer_Workspace *)aWorkspace;
    p->iReg = (T_LPC546xx_Timer_Registers *)LPC_TIMER2_BASE;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_Timer3_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC546xx Timer1 workspace.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC546xx_Timer3_InitializeWorkspace(void *aWorkspace)
{
    T_LPC546xx_Timer_Workspace *p = (T_LPC546xx_Timer_Workspace *)aWorkspace;
    p->iReg = (T_LPC546xx_Timer_Registers *)LPC_TIMER3_BASE;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC546xx_Timer_SetMatchRegisterFunctions
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *          UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LPC546xx_Timer_SetMatchRegisterFunctions(
        void *aWorkspace,
        TUInt8 aMatchRegister,
        ...
        TUInt32 aMatchPoint)
{
    ...
    T_LPC546xx_Timer_Registers *r = p->iReg;
    TUInt32 aFlags = 0;
    //const T_LPC546xx_Timer_Info *p_info = p->iInfo;
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const HAL_Timer LPC546xx_Timer0_Interface = {
    {
    "NXP:LPC546xx:Timer0",
    0x0100,
    LPC546xx_Timer0_InitializeWorkspace,
    sizeof(T_LPC546xx_Timer_Workspace),
    },

    LPC546xx_Timer_SetTimerMode,
    LPC546xx_Timer_SetCaptureSource,
    LPC546xx_Timer_SetMatchRegister,
    LPC546xx_Timer_SetExternalControl,
    LPC546xx_Timer_Reset,
    LPC546xx_Timer_Enable,
    LPC546xx_Timer_Disable,
    LPC546xx_Timer_SetMatchCallback,
    LPC546xx_Timer_SetMatchRegisterFunctions,
};

const HAL_Timer LPC546xx_Timer1_Interface = {
    {
    "NXP:LPC546xx:Timer1",
    0x0100,
    LPC546xx_Timer1_InitializeWorkspace,
    sizeof(T_LPC546xx_Timer_Workspace),
    },
...
const HAL_Timer LPC546xx_Timer2_Interface = {
    {
    "NXP:LPC546xx:Timer2",
    0x0100,
    LPC546xx_Timer2_InitializeWorkspace,
    sizeof(T_LPC546xx_Timer_Workspace),
    },
...
const HAL_Timer LPC546xx_Timer3_Interface = {
    {
    "NXP:LPC546xx:Timer3",
    0x0100,
    LPC546xx_Timer3_InitializeWorkspace,
    sizeof(T_LPC546xx_Timer_Workspace),
    },
