...
} T_LPC17xx_40xx_CAN_Registers;

typedef struct {
        const HAL_CAN *iHAL;
        T_LPC17xx_40xx_CAN_Registers *iReg;
        T_CAN_Settings iSettings;
        TUInt8 iCanNum;
}T_LPC17xx_40xx_CAN_Workspace;
/*---------------------------------------------------------------------------*
 * Function Prototypes:
 *---------------------------------------------------------------------------*/
 ...
 * Globals:
 *---------------------------------------------------------------------------*/
static T_LPC17xx_40xx_CAN_Workspace *G_CAN1Workspace;
static T_LPC17xx_40xx_CAN_Workspace *G_CAN2Workspace;

/*---------------------------------------------------------------------------*
 * Routine:  ILPC17xx_40xx_CAN_ISR
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC1788 CAN workspace.
 * Inputs:
 *      T_LPC17xx_40xx_ADC_Workspace *aW -- CAN Workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
static void ILPC17xx_40xx_CAN_ISR(T_LPC17xx_40xx_CAN_Workspace *aW)
{
    T_LPC17xx_40xx_CAN_Registers *p = aW->iReg;
    TUInt32 wordA, wordB;
...
/*---------------------------------------------------------------------------*
 * Interrupt Routine:  ILPC17xx_40xx_CAN_Interrupt
 *---------------------------------------------------------------------------*
 * Description:
 *      Catch CAN interrupts and use the ISR routine for it
 *---------------------------------------------------------------------------*/
IRQ_ROUTINE(ILPC17xx_40xx_CAN_Interrupt)
{
    IRQ_START();
    
    if((G_CAN1Workspace) && (G_CAN1Workspace->iReg->iGSR & 0x00000001))
        ILPC17xx_40xx_CAN_ISR(G_CAN1Workspace);
        
    if((G_CAN2Workspace) && (G_CAN2Workspace->iReg->iGSR & 0x00000001))
        ILPC17xx_40xx_CAN_ISR(G_CAN2Workspace);
    
    IRQ_END();
}

/*---------------------------------------------------------------------------*
 * Interrupt Routine:  LPC17xx_40xx_CAN_Send_CAN
 *---------------------------------------------------------------------------*
 * Description:
 *      
 *---------------------------------------------------------------------------*/
T_uezError LPC17xx_40xx_CAN_Send(
    void *aWorkspace,
    TUInt16 aID,
    TUInt8 *aData,
    TUInt8 aDataLen)
{
    T_LPC17xx_40xx_CAN_Workspace *aW = (T_LPC17xx_40xx_CAN_Workspace *)aWorkspace;
    T_LPC17xx_40xx_CAN_Registers *p = aW->iReg;
...
/*---------------------------------------------------------------------------*
 * Interrupt Routine:  LPC17xx_40xx_CAN_Enable_CAN
 *---------------------------------------------------------------------------*
 * Description:
 *      
 *---------------------------------------------------------------------------*/
T_uezError LPC17xx_40xx_CAN_Activate(void *aWorkspace)
{
    T_LPC17xx_40xx_CAN_Workspace *aW = (T_LPC17xx_40xx_CAN_Workspace *)aWorkspace;
    T_LPC17xx_40xx_CAN_Registers *p = aW->iReg;
...
/*---------------------------------------------------------------------------*
 * Interrupt Routine:  LPC17xx_40xx_CAN_Disable_CAN
 *---------------------------------------------------------------------------*
 * Description:
 *      
 *---------------------------------------------------------------------------*/
T_uezError LPC17xx_40xx_CAN_Deactivate(void *aWorkspace)
{
    T_LPC17xx_40xx_CAN_Workspace *aW = (T_LPC17xx_40xx_CAN_Workspace *)aWorkspace;
    T_LPC17xx_40xx_CAN_Registers *p = aW->iReg;
...
static void ICANSetBaudRate(
    T_LPC17xx_40xx_CAN_Workspace *aW,
    TUInt32 aBaudRate)
{
    T_LPC17xx_40xx_CAN_Registers *p = aW->iReg;
...
/*---------------------------------------------------------------------------*
 * Routine: LPC17xx_40xx_CAN_SetCANSettings
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                 -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC17xx_40xx_CAN_SetSettings(
        void *aWorkspace,
...
/*---------------------------------------------------------------------------*
 * Routine: LPC17xx_40xx_CAN_GetCANSettings
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                 -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC17xx_40xx_CAN_GetSettings(
        void *aWorkspace,
        T_CAN_Settings *aSettings)
{
    T_LPC17xx_40xx_CAN_Workspace *aW = (T_LPC17xx_40xx_CAN_Workspace *)aWorkspace;
 ...
 *      
 *---------------------------------------------------------------------------*/
 T_uezError LPC17xx_40xx_CAN_Configure(
        void *aWorkspace,
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC17xx_40xx_CAN_CAN1_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC1788 CAN workspace.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC17xx_40xx_CAN_CAN1_InitializeWorkspace(void * aWorkspace)
{
    T_LPC17xx_40xx_CAN_Workspace *aW = (T_LPC17xx_40xx_CAN_Workspace *)aWorkspace;
    aW->iReg = (T_LPC17xx_40xx_CAN_Registers *)LPC_CAN1_BASE;
    G_CAN2Workspace = aW;
    
    aW->iCanNum = 1;

    //Register the interrupt for this CAN but don't start it
    InterruptRegister(CAN_IRQn, ILPC17xx_40xx_CAN_Interrupt,
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC17xx_40xx_CAN_CAN2_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC1788 CAN workspace.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC17xx_40xx_CAN_CAN2_InitializeWorkspace(void * aWorkspace)
{
    T_LPC17xx_40xx_CAN_Workspace *aW = (T_LPC17xx_40xx_CAN_Workspace *)aWorkspace;
    aW->iReg = (T_LPC17xx_40xx_CAN_Registers *)LPC_CAN2_BASE;
    G_CAN2Workspace = aW;
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
 const HAL_CAN LPC17xx_40xx_CAN_CAN1_Interface = {
        {
                "CAN:LPC1788:CAN1",
                0x0100,
                LPC17xx_40xx_CAN_CAN1_InitializeWorkspace,
                sizeof(T_LPC17xx_40xx_CAN_Workspace),
        },

        LPC17xx_40xx_CAN_Configure,
        LPC17xx_40xx_CAN_SetSettings,
        LPC17xx_40xx_CAN_GetSettings,
        LPC17xx_40xx_CAN_Activate,
        LPC17xx_40xx_CAN_Deactivate,
        LPC17xx_40xx_CAN_Send,
};

const HAL_CAN LPC17xx_40xx_CAN_CAN2_Interface = {
        {
                "CAN:LPC1788:CAN2",
                0x0100,
                LPC17xx_40xx_CAN_CAN2_InitializeWorkspace,
                sizeof(T_LPC17xx_40xx_CAN_Workspace),
        },
 ...
 *---------------------------------------------------------------------------*/
void LPC17xx_40xx_CAN1_Require(
    T_uezGPIOPortPin aPinRD,
    T_uezGPIOPortPin aPinTD)
{
    static const T_LPC17xx_40xx_IOCON_ConfigList rd1[] = {
            {GPIO_P0_0,  IOCON_D_DEFAULT(1)},
            {GPIO_P0_21, IOCON_D_DEFAULT(4)},
    };
    static const T_LPC17xx_40xx_IOCON_ConfigList td1[] = {
    ...
    HALInterfaceRegister("CAN1", (T_halInterface *)&LPC17xx_40xx_CAN_CAN1_Interface, 0, 0);

    LPC17xx_40xx_IOCON_ConfigPinOrNone(aPinRD, rd1, ARRAY_COUNT(rd1));
    LPC17xx_40xx_IOCON_ConfigPinOrNone(aPinTD, td1, ARRAY_COUNT(td1));
    LPC17xx_40xxPowerOn(1<<13);
}

void LPC17xx_40xx_CAN2_Require(
    T_uezGPIOPortPin aPinRD,
    T_uezGPIOPortPin aPinTD)
{
    static const T_LPC17xx_40xx_IOCON_ConfigList rd2[] = {
            {GPIO_P0_4,  IOCON_D_DEFAULT(2)},
            {GPIO_P2_7,  IOCON_D_DEFAULT(1)},
    };
    static const T_LPC17xx_40xx_IOCON_ConfigList td2[] = {
    ...
    HALInterfaceRegister("CAN2", (T_halInterface *)&LPC17xx_40xx_CAN_CAN2_Interface, 0, 0);

    LPC17xx_40xx_IOCON_ConfigPinOrNone(aPinRD, rd2, ARRAY_COUNT(rd2));
    LPC17xx_40xx_IOCON_ConfigPinOrNone(aPinTD, td2, ARRAY_COUNT(td2));
    LPC17xx_40xxPowerOn(1<<14);
}

/*-------------------------------------------------------------------------*
 * End of File:  LPC17xx_40xx_CAN.c
 *-------------------------------------------------------------------------*/
