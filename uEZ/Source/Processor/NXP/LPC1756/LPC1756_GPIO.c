 ...
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the LPC1756 GPIO Interface.
 * Implementation:
...
typedef struct {
    ...
    TVUInt32 *iIOIntClr;
} LPC1756_GPIO_PortInfo;

typedef struct {
    const HAL_GPIOPort *iHAL;
    const LPC1756_GPIO_PortInfo *iPortInfo;

    // uEZ v2.02
    TUInt32 iLocked;
    T_gpioInterruptHandler iInterruptCallback;
    void *iInterruptCallbackWorkspace;
} T_LPC1756_GPIO_Workspace;

/*---------------------------------------------------------------------------*
 * Globals:
 *---------------------------------------------------------------------------*/
const LPC1756_GPIO_PortInfo GPIO_LPC1756_Port0_PortInfo = {
...
const LPC1756_GPIO_PortInfo GPIO_LPC1756_Port1_PortInfo = {
...
const LPC1756_GPIO_PortInfo GPIO_LPC1756_Port2_PortInfo = {
...
const LPC1756_GPIO_PortInfo GPIO_LPC1756_Port4_PortInfo = {
...
static T_LPC1756_GPIO_Workspace *G_LPC1756Port0Workspace = 0;
static T_LPC1756_GPIO_Workspace *G_LPC1756Port2Workspace = 0;

/*---------------------------------------------------------------------------*
 * Prototypes:
 *---------------------------------------------------------------------------*/
void LPC1756_GPIO_ProcessIRQ(T_LPC1756_GPIO_Workspace *p);

/*---------------------------------------------------------------------------*
 * Routine:  LPC1756_GPIO_Port0_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC1756 GPIO Port0 workspace.
 * Inputs:
 *      void *aW                    -- Particular GPIO workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPIO_Port0_InitializeWorkspace(void *aWorkspace)
{
    T_LPC1756_GPIO_Workspace *p = (T_LPC1756_GPIO_Workspace *)aWorkspace;
    p->iPortInfo = &GPIO_LPC1756_Port0_PortInfo;
    p->iLocked = 0;
    p->iInterruptCallback = 0;
    p->iInterruptCallbackWorkspace = 0;
    G_LPC1756Port0Workspace = p;

    return UEZ_ERROR_NONE;
}

/*---------------------------------------------------------------------------*
 * Routine:  LPC1756_GPIO_Port1_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC1756 GPIO Port1 workspace.
 * Inputs:
 *      void *aW                    -- Particular GPIO workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPIO_Port1_InitializeWorkspace(void *aWorkspace)
{
    T_LPC1756_GPIO_Workspace *p = (T_LPC1756_GPIO_Workspace *)aWorkspace;
    p->iPortInfo = &GPIO_LPC1756_Port1_PortInfo;
    p->iLocked = 0;
    p->iInterruptCallback = 0;
    p->iInterruptCallbackWorkspace = 0;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1756_GPIO_Port2_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC1756 GPIO Port2 workspace.
 * Inputs:
 *      void *aW                    -- Particular GPIO workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPIO_Port2_InitializeWorkspace(void *aWorkspace)
{
    T_LPC1756_GPIO_Workspace *p = (T_LPC1756_GPIO_Workspace *)aWorkspace;
    p->iPortInfo = &GPIO_LPC1756_Port2_PortInfo;
    p->iLocked = 0;
    p->iInterruptCallback = 0;
    p->iInterruptCallbackWorkspace = 0;
    G_LPC1756Port2Workspace = p;

    return UEZ_ERROR_NONE;
}

/*---------------------------------------------------------------------------*
 * Routine:  LPC1756_GPIO_Port4_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC1756 GPIO Port4 workspace.
 * Inputs:
 *      void *aW                    -- Particular GPIO workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPIO_Port4_InitializeWorkspace(void *aWorkspace)
{
    T_LPC1756_GPIO_Workspace *p = (T_LPC1756_GPIO_Workspace *)aWorkspace;
    p->iPortInfo = &GPIO_LPC1756_Port4_PortInfo;
    p->iLocked = 0;
    p->iInterruptCallback = 0;
    p->iInterruptCallbackWorkspace = 0;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1756_GPIO_Configure
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPIO_Configure(void *aWorkspace, TUInt32 aUsablePins)
{
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPIO_Activate(void *aWorkspace, TUInt32 aPortPins)
{
    PARAM_NOT_USED(aWorkspace);
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1756_GPIO_Deactivate
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPIO_Deactivate(void *aWorkspace, TUInt32 aPortPins)
{
    PARAM_NOT_USED(aWorkspace);
    PARAM_NOT_USED(aPortPins);
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1756_GPIO_SetOutputMode
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPIO_SetOutputMode(void *aWorkspace, TUInt32 aPortPins)
{
    T_LPC1756_GPIO_Workspace *p = (T_LPC1756_GPIO_Workspace *)aWorkspace;
    const LPC1756_GPIO_PortInfo *p_info = p->iPortInfo;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1756_GPIO_SetInputMode
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPIO_SetInputMode(void *aWorkspace, TUInt32 aPortPins)
{
    T_LPC1756_GPIO_Workspace *p = (T_LPC1756_GPIO_Workspace *)aWorkspace;
    const LPC1756_GPIO_PortInfo *p_info = p->iPortInfo;
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPIO_Set(void *aWorkspace, TUInt32 aPortPins)
{
    T_LPC1756_GPIO_Workspace *p = (T_LPC1756_GPIO_Workspace *)aWorkspace;
    const LPC1756_GPIO_PortInfo *p_info = p->iPortInfo;
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPIO_Clear(void *aWorkspace, TUInt32 aPortPins)
{
    T_LPC1756_GPIO_Workspace *p = (T_LPC1756_GPIO_Workspace *)aWorkspace;
    const LPC1756_GPIO_PortInfo *p_info = p->iPortInfo;
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPIO_SetPull(
            void *aWorkspace,
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPIO_SetMux(
            void *aWorkspace,
            TUInt8 aPortPinIndex,
            T_gpioMux aMux)
{
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPIO_GetMux(
        void *aWorkspace,
        TUInt8 aPortPinIndex,
        T_gpioMux *aMux)
{
...
T_uezError LPC1756_GPIO_Control(
        void *aWorkspace,
        TUInt8 aPortPinIndex,
        ...
        TUInt32 aValue)
{
    ...
    switch (aControl) {
        ...
        case GPIO_CONTROL_SET_CONFIG_BITS:
            ...
            error = LPC1756_GPIO_SetMux(p, aPortPinIndex, aValue & 3);
            if (error)
                return error;
            error = LPC1756_GPIO_SetPull(p, aPortPinIndex,
                (T_gpioPull)((aValue >> 2) & 7));
            if (error)
                return error;
            if (aValue & IOCON_OPEN_DRAIN)
                error = LPC1756_GPIO_Control(p, aPortPinIndex,
...
T_uezError LPC1756_GPIO_Lock(void *aWorkspace, TUInt32 aPortPins)
{
#ifndef NDEBUG
    char buffer [50];
#endif
...
T_uezError LPC1756_GPIO_Unlock(void *aWorkspace, TUInt32 aPortPins)
{
    T_LPC1756_GPIO_Workspace *p = (T_LPC1756_GPIO_Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Interrupt Routine:  LPC1756_GPIO_IRQ
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static IRQ_ROUTINE(LPC1756_GPIO_IRQ)
{
    IRQ_START();

    // It's port 0 and/or port 2, is this port 0 firing?
    if (GPIOINT->IntStatus & IOIntStatus_P0Int)
        LPC1756_GPIO_ProcessIRQ(G_LPC1756Port0Workspace);
    if (GPIOINT->IntStatus & IOIntStatus_P2Int)
        LPC1756_GPIO_ProcessIRQ(G_LPC1756Port2Workspace);

    IRQ_END();
}

/*---------------------------------------------------------------------------*
 * Routine:  LPC1756_GPIO_SetInterrupt
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPIO_ConfigureInterruptCallback(
        void *aWorkspace,
        TUInt32 aPortPins,
        ...
        void *aInterruptCallbackWorkspace)
{
    ...
    InterruptRegister(EINT3_IRQn, LPC1756_GPIO_IRQ, INTERRUPT_PRIORITY_HIGH,
            "GPIOIrq");
...
T_uezError LPC1756_GPIO_ConfigureInterruptCallback_NotSupported(
        void *aWorkspace,
        TUInt32 aPortPins,
        T_gpioInterruptHandler aInterruptCallback,
        void *aInterruptCallbackWorkspace)
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1756_GPIO_ProcessIRQ
 *---------------------------------------------------------------------------*
 * Description:
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1756_GPIO_EnableInterrupts
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPIO_EnableInterrupts(
        void *aWorkspace,
        TUInt32 aPortPins,
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1756_GPIO_DisableInterrupts
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPIO_DisableInterrupts(
        void *aWorkspace,
        TUInt32 aPortPins,
        T_gpioInterruptType aType)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1756_GPIO_ClearInterrupts
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      TUInt32 aPortPins           -- Masked list of ports to disable.
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPIO_ClearInterrupts(void *aWorkspace, TUInt32 aPortPins)
{
    T_LPC1756_GPIO_Workspace *p = (T_LPC1756_GPIO_Workspace *)aWorkspace;
    const LPC1756_GPIO_PortInfo *p_info = p->iPortInfo;
...
T_uezError LPC1756_GPIO_EnableInterrupts_NotSupported(
        void *aWorkspace,
        TUInt32 aPortPins,
        T_gpioInterruptType aType)
{
...
T_uezError LPC1756_GPIO_DisableInterrupts_NotSupported(
        void *aWorkspace,
        TUInt32 aPortPins,
        T_gpioInterruptType aType)
{
...
T_uezError LPC1756_GPIO_ClearInterrupts_NotSupported(
        void *aWorkspace,
        TUInt32 aPortPins)
{
 ...
 *---------------------------------------------------------------------------*/
const HAL_GPIOPort GPIO_LPC1756_Port0_Interface = {
    {
    "LPC1756 GPIO Port 0",
    0x0100,
    LPC1756_GPIO_Port0_InitializeWorkspace,
    sizeof(T_LPC1756_GPIO_Workspace),
    },

    LPC1756_GPIO_Configure,
    LPC1756_GPIO_Activate,
    LPC1756_GPIO_Deactivate,
    LPC1756_GPIO_SetOutputMode,
    LPC1756_GPIO_SetInputMode,
    LPC1756_GPIO_Set,
    LPC1756_GPIO_Clear,
    LPC1756_GPIO_Read,
    LPC1756_GPIO_SetPull,
    LPC1756_GPIO_SetMux,

    // uEZ v2.02
    LPC1756_GPIO_ConfigureInterruptCallback,
    LPC1756_GPIO_EnableInterrupts,
    LPC1756_GPIO_DisableInterrupts,
    LPC1756_GPIO_ClearInterrupts,
    LPC1756_GPIO_Control,
    LPC1756_GPIO_Lock,
    LPC1756_GPIO_Unlock,
    /* uEZ v2.04 */
    LPC1756_GPIO_GetMux };

const HAL_GPIOPort GPIO_LPC1756_Port1_Interface = {
    {
    "LPC1756 GPIO Port 1",
    0x0100,
    LPC1756_GPIO_Port1_InitializeWorkspace,
    sizeof(T_LPC1756_GPIO_Workspace),
    },
    ...
    // uEZ v2.02
    LPC1756_GPIO_ConfigureInterruptCallback_NotSupported,
    LPC1756_GPIO_EnableInterrupts_NotSupported,
    LPC1756_GPIO_DisableInterrupts_NotSupported,
    LPC1756_GPIO_ClearInterrupts_NotSupported,
    LPC1756_GPIO_Control,
    LPC1756_GPIO_Lock,
    LPC1756_GPIO_Unlock,
    /* uEZ v2.04 */
...
const HAL_GPIOPort GPIO_LPC1756_Port2_Interface = {
    {
    "LPC1756 GPIO Port 2",
    0x0100,
    LPC1756_GPIO_Port2_InitializeWorkspace,
    sizeof(T_LPC1756_GPIO_Workspace),
    },
...
const HAL_GPIOPort GPIO_LPC1756_Port4_Interface = {
    {
    "LPC1756 GPIO Port 4",
    0x0100,
    LPC1756_GPIO_Port4_InitializeWorkspace,
    sizeof(T_LPC1756_GPIO_Workspace),
    },
