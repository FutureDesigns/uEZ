 ...
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the LPC1756 GP DMA controller.
 * Implementation:
 *
 *-------------------------------------------------------------------------*/
...
#include <Source/Processor/NXP/LPC1756/LPC1756_GPDMA.h>
...
} T_LPC1756_GPDMA_Workspace;

/*---------------------------------------------------------------------------*
 * Forward declaration:
 *---------------------------------------------------------------------------*/
static void LPC1756_GPDMA_Enable(void *aWorkspace);
/*static void LPC1756_GPDMA_Disable(void *aWorkspace);*/
static TBool LPC1756_GPDMA_ChannelIsEnabled(void *aWorkspace);
static void IGPDMAProcessInterrupt(T_LPC1756_GPDMA_Workspace *p);
IRQ_ROUTINE( IGPDMA)
;
...
static T_LPC1756_GPDMA_Workspace * G_workspaceGPDMAChannel[8];

T_uezError LPC1756_GPDMA_InitializeWorkspace_ChannelX(
        void *aWorkspace,
        TUInt8 aChannel)
{
    T_LPC1756_GPDMA_Workspace *p = (T_LPC1756_GPDMA_Workspace *)aWorkspace;

    // Setup GPDMA peripheral
    LPC1756_GPDMA_Enable(aWorkspace);
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1756_GPDMA_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC1756 GPDMA iChannel workspaces.
 * Inputs:
 *      void *aW                    -- Particular GPDMA workspace
 * Outputs:
 *      T_uezError                  -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPDMA_InitializeWorkspace_Channel0(void *aWorkspace)
{
    return LPC1756_GPDMA_InitializeWorkspace_ChannelX(aWorkspace,
            GPDMA_CHANNEL0);
}
T_uezError LPC1756_GPDMA_InitializeWorkspace_Channel1(void *aWorkspace)
{
    return LPC1756_GPDMA_InitializeWorkspace_ChannelX(aWorkspace,
            GPDMA_CHANNEL1);
}
T_uezError LPC1756_GPDMA_InitializeWorkspace_Channel2(void *aWorkspace)
{
    return LPC1756_GPDMA_InitializeWorkspace_ChannelX(aWorkspace,
            GPDMA_CHANNEL2);
}
T_uezError LPC1756_GPDMA_InitializeWorkspace_Channel3(void *aWorkspace)
{
    return LPC1756_GPDMA_InitializeWorkspace_ChannelX(aWorkspace,
            GPDMA_CHANNEL3);
}
T_uezError LPC1756_GPDMA_InitializeWorkspace_Channel4(void *aWorkspace)
{
    return LPC1756_GPDMA_InitializeWorkspace_ChannelX(aWorkspace,
            GPDMA_CHANNEL4);
}
T_uezError LPC1756_GPDMA_InitializeWorkspace_Channel5(void *aWorkspace)
{
    return LPC1756_GPDMA_InitializeWorkspace_ChannelX(aWorkspace,
            GPDMA_CHANNEL5);
}
T_uezError LPC1756_GPDMA_InitializeWorkspace_Channel6(void *aWorkspace)
{
    return LPC1756_GPDMA_InitializeWorkspace_ChannelX(aWorkspace,
            GPDMA_CHANNEL6);
}
T_uezError LPC1756_GPDMA_InitializeWorkspace_Channel7(void *aWorkspace)
{
    return LPC1756_GPDMA_InitializeWorkspace_ChannelX(aWorkspace,
            GPDMA_CHANNEL7);
}
 ...
 *      T_uezError                  -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPDMA_Stop(void *aWorkspace)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1756_GPDMA_ChannelIsEnabled
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                  -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPDMA_Start(void *aWorkspace)
{
//    TUInt32 regValue = 0;
    T_LPC1756_GPDMA_Workspace *p = (T_LPC1756_GPDMA_Workspace *)aWorkspace;
    ...
    if (p->iControl == 0 || ETrue == LPC1756_GPDMA_ChannelIsEnabled(aWorkspace))
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1756_GPDMA_PrepareRequest
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                  -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1756_GPDMA_PrepareRequest(
        void *aWorkspace,
        ...
        void *aISRCallbackWorkspace)
{
    ...
    // General output to match Table 675 from UM102347: LPC1756 user's manual
    flow = 0;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1756_GPDMA_IsValidAddress
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static TBool LPC1756_GPDMA_IsValidAddress(TUInt32 aAddr)
{
    // On the LPC1756, all addresses are valid
    return ETrue;
}
 ...
 *              service routine.
 * Inputs:
 *      T_LPC1756_GPDMA_Workspace *p      -- Port's definition to process interrupt
 *---------------------------------------------------------------------------*/
static void IGPDMAProcessInterrupt(T_LPC1756_GPDMA_Workspace *p)
{
...
const HAL_GPDMA LPC1756_GPDMA_Channel0_Interface = {
        {
                "GPDMA:LPC1756:CH0",
                0x0100,
                LPC1756_GPDMA_InitializeWorkspace_Channel0,
                sizeof(T_LPC1756_GPDMA_Workspace), },
        LPC1756_GPDMA_PrepareRequest,
        LPC1756_GPDMA_Start,
        LPC1756_GPDMA_Stop,
        LPC1756_GPDMA_IsValidAddress, };

const HAL_GPDMA LPC1756_GPDMA_Channel1_Interface = {
        {
                "GPDMA:LPC1756:CH1",
                0x0100,
                LPC1756_GPDMA_InitializeWorkspace_Channel1,
                sizeof(T_LPC1756_GPDMA_Workspace), },
        LPC1756_GPDMA_PrepareRequest,
        LPC1756_GPDMA_Start,
        LPC1756_GPDMA_Stop,
...
const HAL_GPDMA LPC1756_GPDMA_Channel2_Interface = {
        {
                "GPDMA:LPC1756:CH2",
                0x0100,
                LPC1756_GPDMA_InitializeWorkspace_Channel2,
                sizeof(T_LPC1756_GPDMA_Workspace), },
        LPC1756_GPDMA_PrepareRequest,
        LPC1756_GPDMA_Start,
        LPC1756_GPDMA_Stop,
...
const HAL_GPDMA LPC1756_GPDMA_Channel3_Interface = {
        {
                "GPDMA:LPC1756:CH3",
                0x0100,
                LPC1756_GPDMA_InitializeWorkspace_Channel3,
                sizeof(T_LPC1756_GPDMA_Workspace), },
        LPC1756_GPDMA_PrepareRequest,
        LPC1756_GPDMA_Start,
        LPC1756_GPDMA_Stop,
...
const HAL_GPDMA LPC1756_GPDMA_Channel4_Interface = {
        {
                "GPDMA:LPC1756:CH4",
                0x0100,
                LPC1756_GPDMA_InitializeWorkspace_Channel4,
                sizeof(T_LPC1756_GPDMA_Workspace), },
        LPC1756_GPDMA_PrepareRequest,
        LPC1756_GPDMA_Start,
        LPC1756_GPDMA_Stop,
...
const HAL_GPDMA LPC1756_GPDMA_Channel5_Interface = {
        {
                "GPDMA:LPC1756:CH5",
                0x0100,
                LPC1756_GPDMA_InitializeWorkspace_Channel5,
                sizeof(T_LPC1756_GPDMA_Workspace), },
        LPC1756_GPDMA_PrepareRequest,
        LPC1756_GPDMA_Start,
        LPC1756_GPDMA_Stop,
...
const HAL_GPDMA LPC1756_GPDMA_Channel6_Interface = {
        {
                "GPDMA:LPC1756:CH6",
                0x0100,
                LPC1756_GPDMA_InitializeWorkspace_Channel6,
                sizeof(T_LPC1756_GPDMA_Workspace), },
        LPC1756_GPDMA_PrepareRequest,
        LPC1756_GPDMA_Start,
        LPC1756_GPDMA_Stop,
...
const HAL_GPDMA LPC1756_GPDMA_Channel7_Interface = {
        {
                "GPDMA:LPC1756:CH7",
                0x0100,
                LPC1756_GPDMA_InitializeWorkspace_Channel7,
                sizeof(T_LPC1756_GPDMA_Workspace), },
        LPC1756_GPDMA_PrepareRequest,
        LPC1756_GPDMA_Start,
        LPC1756_GPDMA_Stop,
...
void LPC1756_GPDMA0_Require(void)
{
    ...
    HALInterfaceRegister("GPDMA0",
            (T_halInterface *)&LPC1756_GPDMA_Channel0_Interface, 0, 0);
}
...
void LPC1756_GPDMA1_Require(void)
{
    ...
    HALInterfaceRegister("GPDMA1",
            (T_halInterface *)&LPC1756_GPDMA_Channel1_Interface, 0, 0);
}
...
void LPC1756_GPDMA2_Require(void)
{
    ...
    HALInterfaceRegister("GPDMA2",
            (T_halInterface *)&LPC1756_GPDMA_Channel2_Interface, 0, 0);
}
...
void LPC1756_GPDMA3_Require(void)
{
    ...
    HALInterfaceRegister("GPDMA3",
            (T_halInterface *)&LPC1756_GPDMA_Channel3_Interface, 0, 0);
}
...
void LPC1756_GPDMA4_Require(void)
{
    ...
    HALInterfaceRegister("GPDMA4",
            (T_halInterface *)&LPC1756_GPDMA_Channel4_Interface, 0, 0);
}
...
void LPC1756_GPDMA5_Require(void)
{
    ...
    HALInterfaceRegister("GPDMA5",
            (T_halInterface *)&LPC1756_GPDMA_Channel5_Interface, 0, 0);
}
...
void LPC1756_GPDMA6_Require(void)
{
    ...
    HALInterfaceRegister("GPDMA6",
            (T_halInterface *)&LPC1756_GPDMA_Channel6_Interface, 0, 0);
}
