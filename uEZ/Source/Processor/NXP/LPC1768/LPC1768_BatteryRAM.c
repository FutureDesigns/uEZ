/*-------------------------------------------------------------------------*
 * File:  LPC1768_BatteryRAM.c
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the LPC1768 Battery RAM Interface.
 * Implementation:
...
#include <Source/Processor/NXP/LPC1768/LPC1768_BatteryRAM.h>
...
typedef struct {
    ...
    TBool iContentValid;
} T_LPC1768_BatteryRAM_Workspace;

/*---------------------------------------------------------------------------*
 * Routine:  LPC1768_BatteryRAM_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC1768 BatteryRAM workspace.
 * Inputs:
 *      void *aW                    -- Particular BatteryRAM workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1768_BatteryRAM_InitializeWorkspace(void *aWorkspace)
{

    T_LPC1768_BatteryRAM_Workspace *p =
            (T_LPC1768_BatteryRAM_Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1768_BatteryRAM_Read
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1768_BatteryRAM_Read(
        void *aWorkspace,
        const TUInt32 aAddr,
        TUInt8 *aData)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1768_BatteryRAM_Write
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1768_BatteryRAM_Write(
        void *aWorkspace,
        const TUInt32 aAddr,
        const TUInt8 aData)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1768_BatteryRAM_Valid
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
TBool LPC1768_BatteryRAM_IsValid(void *aWorkspace)
{
    T_LPC1768_BatteryRAM_Workspace *p =
            (T_LPC1768_BatteryRAM_Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1768_BatteryRAM_GetSize
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
TUInt32 LPC1768_BatteryRAM_GetSize(void *aWorkspace)
{
 ...
 *---------------------------------------------------------------------------*/
const HAL_BatteryRAM LPC1768_BatteryRAM_Interface = {
        {
                "LPC1768:BatteryRAM",
                0x0100,
                LPC1768_BatteryRAM_InitializeWorkspace,
                sizeof(T_LPC1768_BatteryRAM_Workspace), },

        LPC1768_BatteryRAM_Read,
        LPC1768_BatteryRAM_Write,
        LPC1768_BatteryRAM_IsValid,
        LPC1768_BatteryRAM_GetSize };

/*---------------------------------------------------------------------------*
 * Requirement routines:
 *---------------------------------------------------------------------------*/
void LPC1768_BatteryRAM_Require(void)
{
    HAL_DEVICE_REQUIRE_ONCE();
    HALInterfaceRegister("BatteryRAM",
            (T_halInterface *)&LPC1768_BatteryRAM_Interface, 0, 0);
}

/*-------------------------------------------------------------------------*
 * End of File:  LPC1768_BatteryRAM.c
 *-------------------------------------------------------------------------*/
