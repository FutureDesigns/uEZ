 ...
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the LPC1768 SSP Bus Interface.
 * Implementation:
...
typedef struct {
    TVUInt32 iICR0;  // 0x00
        ...
        #define SSP_FRF_MW  (2<<4)      // Frame format: MiICRowire
...
static void ISSPProcessInterrupt(T_LPC1768_SSP_Workspace *aW);
static void ISSPOutput(T_LPC1768_SSP_Workspace *aW);

/*---------------------------------------------------------------------------*
 * Globals:
 *---------------------------------------------------------------------------*/
static T_LPC1768_SSP_Workspace *G_SSP0Workspace;
static T_LPC1768_SSP_Workspace *G_SSP1Workspace;
 ...
 * Outputs:
 *      T_LPC1768_SSP_Workspace *aW -- Particular SSP workspace
 *      SPI_Request *aRequest      -- Configuration of SSP device
 *---------------------------------------------------------------------------*/
static void ISSPConfig(T_LPC1768_SSP_Workspace *aW, SPI_Request *aRequest)
{
    T_LPC1768_SSP_Registers *p = aW->iReg;
    TUInt32 scr;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1768_SSP_TransferPolled
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
T_uezError LPC1768_SSP_TransferPolled(void *aWorkspace, SPI_Request *aRequest)
{
    TUInt32 i;
    T_LPC1768_SSP_Workspace *aW = (T_LPC1768_SSP_Workspace *)aWorkspace;
    T_LPC1768_SSP_Registers *p = aW->iReg;
    TUInt8 *p_dataMISO8;
    TUInt8 *p_dataMOSI8;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1768_SSP_SSP0_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC1768 SSP0 workspace.
 * Inputs:
 *      void *aW                    -- Particular SSP workspace
 * Outputs:
 *      T_uezError                   -- Error code
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1768_SSP_SSP1_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC1768 SSP1 workspace.
 * Inputs:
 *      void *aW                    -- Particular SSP workspace
 * Outputs:
 *      T_uezError                   -- Error code
 ...
 *---------------------------------------------------------------------------*/
void LPC1768_SSP_Start(void *aWorkspace, SPI_Request *aRequest)
{
    // Setup SSP for this configuration
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1768_SSP_TransferInOut
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
static TUInt32 LPC1768_SSP_TransferInOut(
        void *aWorkspace,
 ...
 *      SPI_Request *aRequest       -- SSP request being processed
 *---------------------------------------------------------------------------*/
static void LPC1768_SSP_Stop(void *aWorkspace, SPI_Request *aRequest)
{
    ISSPEnd(aRequest);
}
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
static T_uezError LPC1768_SSP_TransferInOutBytes(
        void *aWorkspace,
        SPI_Request *aRequest)
{
    T_LPC1768_SSP_Workspace *aW = (T_LPC1768_SSP_Workspace *)aWorkspace;
...
static T_uezError LPC1768_SSP_TransferNoBlock(
        void *aWorkspace,
        SPI_Request *aRequest,
...
static TBool LPC1768_SSP_IsBusy(void *aWorkspace)
{
    T_LPC1768_SSP_Workspace *aW = (T_LPC1768_SSP_Workspace *)aWorkspace;
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const HAL_SPI SSP_LPC1768_Port0_Interface = {
    {
    "NXP:LPC1768:SSP0",
    0x0100,
    LPC1768_SSP_SSP0_InitializeWorkspace,
    sizeof(T_LPC1768_SSP_Workspace),
    },

    LPC1768_SSP_TransferPolled,

    LPC1768_SSP_Start,
        LPC1768_SSP_TransferInOut,
        LPC1768_SSP_Stop,
        LPC1768_SSP_TransferInOutBytes,

        // uEZ v2.02
        LPC1768_SSP_TransferNoBlock,
        LPC1768_SSP_IsBusy,
};

const HAL_SPI SSP_LPC1768_Port1_Interface = {
    {
    "NXP:LPC1768:SSP1",
    0x0100,
    LPC1768_SSP_SSP1_InitializeWorkspace,
    sizeof(T_LPC1768_SSP_Workspace),
    },
        ...
        T_uezGPIOPortPin aPinSSEL0)
{
    ...
    HALInterfaceRegister("SSP0",
            (T_halInterface *)&SSP_LPC1768_Port0_Interface, 0, 0);
    LPC1768_IOCON_ConfigPinOrNone(aPinSCK0, sck0, ARRAY_COUNT(sck0));
    LPC1768_IOCON_ConfigPinOrNone(aPinSSEL0, ssel0, ARRAY_COUNT(ssel0));
    LPC1768_IOCON_ConfigPinOrNone(aPinMISO0, miso0, ARRAY_COUNT(miso0));
    LPC1768_IOCON_ConfigPinOrNone(aPinMOSI0, mosi0, ARRAY_COUNT(mosi0));
    LPC1768PowerOn(1<<21);
        ...
        T_uezGPIOPortPin aPinSSEL1)
{
    ...
    HALInterfaceRegister("SSP1",
            (T_halInterface *)&SSP_LPC1768_Port1_Interface, 0, 0);
    LPC1768_IOCON_ConfigPinOrNone(aPinSCK1, sck1, ARRAY_COUNT(sck1));
    LPC1768_IOCON_ConfigPinOrNone(aPinSSEL1, ssel1, ARRAY_COUNT(ssel1));
    LPC1768_IOCON_ConfigPinOrNone(aPinMISO1, miso1, ARRAY_COUNT(miso1));
    LPC1768_IOCON_ConfigPinOrNone(aPinMOSI1, mosi1, ARRAY_COUNT(mosi1));
    LPC1768PowerOn(1<<10);
