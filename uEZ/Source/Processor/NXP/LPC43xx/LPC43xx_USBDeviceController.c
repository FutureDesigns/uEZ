/*-------------------------------------------------------------------------*
 * File:  LPC43xx_USBDev.c
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the LPC43xx USBDev (Ethernet) Driver.
 * Implementation:
 *
 *-------------------------------------------------------------------------*/
...
} T_LPC43xx_USBDev_Workspace;

/*---------------------------------------------------------------------------*
 * Globals:
 *---------------------------------------------------------------------------*/
extern const HAL_USBDeviceController LPC43xx_USBDev_Interface;
T_LPC43xx_USBDev_Workspace *G_LPC43xx_USBDev_Workspace=0;

/*---------------------------------------------------------------------------*
 * Prototypes:
 *---------------------------------------------------------------------------*/
static void LPC43xx_USBDev_EndpointConfigure(
        void *aWorkspace,
        TUInt8 aEndpoint,
 ...
 * Inputs:
 *      T_LPC43xx_USBDev_Workspace *p -- workspace
...
/*-------------------------------------------------------------------------*
 * Internal Function:  ILPC43xx_USBDev_Command
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *-------------------------------------------------------------------------*/
static void ILPC43xx_USBDev_Command(
            T_LPC43xx_USBDev_Workspace *p,
            TUInt8 aCommand)
{
...
/*-------------------------------------------------------------------------*
 * Internal Function:  ILPC43xx_USBDev_CommandAndWrite
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *-------------------------------------------------------------------------*/
static void ILPC43xx_USBDev_CommandAndWrite(
                T_LPC43xx_USBDev_Workspace *p,
                TUInt8 aCommand,
                TUInt16 aData)
{
    // First, write the command
    ILPC43xx_USBDev_Command(p, aCommand);
...
/*-------------------------------------------------------------------------*
 * Internal Function:  ILPC43xx_USBDev_CommandAndRead
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *-------------------------------------------------------------------------*/
static TUInt8 ILPC43xx_USBDev_CommandAndRead(
                T_LPC43xx_USBDev_Workspace *p,
                TUInt8 aCommand)
{
...
/*-------------------------------------------------------------------------*
 * Function:  LPC43xx_USBDev_InterruptNakEnable
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *-------------------------------------------------------------------------*/
static void LPC43xx_USBDev_InterruptNakEnable(
            void *aWorkspace,
            TUInt8 aNakBits)
{
    T_LPC43xx_USBDev_Workspace *p = (T_LPC43xx_USBDev_Workspace *)aWorkspace;

    ILPC43xx_USBDev_CommandAndWrite(p, USB_DEVICE_CMD_SET_MODE, aNakBits);
}

/*-------------------------------------------------------------------------*
 * Function:  LPC43xx_USBDev_EndpointInterruptsEnable
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *-------------------------------------------------------------------------*/
static void LPC43xx_USBDev_EndpointInterruptsEnable(
                void *aWorkspace,
                TUInt8 aEndpoint)
{
...
/*-------------------------------------------------------------------------*
 * Function:  LPC43xx_USBDev_EndpointInterruptsDisable
 *-------------------------------------------------------------------------*
 * Description:
 *      Allow an endpoint to generate interrupts.
 * Inputs:
 ...
 *      TUInt8 aEndpoint            -- Endpoint to trigger interrupts
 *-------------------------------------------------------------------------*/
static void LPC43xx_USBDev_EndpointInterruptsDisable(
                void *aWorkspace,
                TUInt8 aEndpoint)
{
    TUInt32 aIndex = IEndpointToIndex(aEndpoint);
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC43xx_USBDev_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
T_uezError LPC43xx_USBDev_InitializeWorkspace(void *aWorkspace)
{
    T_LPC43xx_USBDev_Workspace *p = (T_LPC43xx_USBDev_Workspace *)aWorkspace;
    ...
    G_LPC43xx_USBDev_Workspace = p;

    return UEZ_ERROR_NONE;
}

/*---------------------------------------------------------------------------*
 * Routine:  LPC43xx_USBDev_Configure
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                 -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC43xx_USBDev_Configure(
        void *aWorkspace,
        T_USBDeviceControllerSettings *iSettings)
{
    T_LPC43xx_USBDev_Workspace *p = (T_LPC43xx_USBDev_Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  ILPC43xx_USBDev_InterruptProcess
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      void *aWorkspace          -- USBDev Controller's workspace
 *---------------------------------------------------------------------------*/
static void ILPC43xx_USBDev_InterruptProcess(T_LPC43xx_USBDev_Workspace *p)
{
    TUInt32 status;
    ...
    if (status & DEV_STAT) {
        ...
        usbStatus = (T_USBStatus)ILPC43xx_USBDev_CommandAndRead(
                        p,
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC43xx_USBDev_ProcessEndpoints
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      void *aWorkspace          -- USBDev Controller's workspace
 *---------------------------------------------------------------------------*/
void LPC43xx_USBDev_ProcessEndpoints(void *aWorkspace)
{
    T_LPC43xx_USBDev_Workspace *p = (T_LPC43xx_USBDev_Workspace *)aWorkspace;
    TUInt32 i;
...
/*---------------------------------------------------------------------------*
 * Routine:  ILPC43xx_USBDev_InterruptVector
 *---------------------------------------------------------------------------*
 * Description:
 *      Process USB Device Interrupts
 *---------------------------------------------------------------------------*/
void LPC43xx_InterruptProcess_USBDevice(void)
{
    if (G_LPC43xx_USBDev_Workspace)
        ILPC43xx_USBDev_InterruptProcess(G_LPC43xx_USBDev_Workspace);
}

/*-------------------------------------------------------------------------*
 * Function:  LPC43xx_USBDev_Connect
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *      void *aWorkspace          -- USBDev Controller's workspace
 *-------------------------------------------------------------------------*/
static void LPC43xx_USBDev_Connect(void *aWorkspace)
{
    ILPC43xx_USBDev_CommandAndWrite(
        (T_LPC43xx_USBDev_Workspace *)aWorkspace,
        USB_DEVICE_CMD_STATUS,
        USB_STATUS_CONNECT);
}

/*-------------------------------------------------------------------------*
 * Function:  LPC43xx_USBDev_Disconnect
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *      void *aWorkspace          -- USBDev Controller's workspace
 *-------------------------------------------------------------------------*/
static void LPC43xx_USBDev_Disconnect(void *aWorkspace)
{
    ILPC43xx_USBDev_CommandAndWrite(
        (T_LPC43xx_USBDev_Workspace *)aWorkspace,
        USB_DEVICE_CMD_STATUS,
...
/*-------------------------------------------------------------------------*
 * Function:  LPC43xx_USBDev_SetAddress
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *-------------------------------------------------------------------------*/
void LPC43xx_USBDev_SetAddress(void *aWorkspace, TUInt8 aAddress)
{
    ILPC43xx_USBDev_CommandAndWrite(
        (T_LPC43xx_USBDev_Workspace *)aWorkspace,
...
/*-------------------------------------------------------------------------*
 * Function:  LPC43xx_USBDev_DeviceIsConfigured
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *      void *aWorkspace          -- USBDev Controller's workspace
 *-------------------------------------------------------------------------*/
void LPC43xx_USBDev_DeviceIsConfigured(void *aWorkspace)
{
    ILPC43xx_USBDev_CommandAndWrite(
            (T_LPC43xx_USBDev_Workspace *)aWorkspace,
...
/*-------------------------------------------------------------------------*
 * Function:  LPC43xx_USBDev_DeviceIsNotConfigured
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *      void *aWorkspace          -- USBDev Controller's workspace
 *-------------------------------------------------------------------------*/
void LPC43xx_USBDev_DeviceIsNotConfigured(void *aWorkspace)
{
    ILPC43xx_USBDev_CommandAndWrite(
        (T_LPC43xx_USBDev_Workspace *)aWorkspace,
        USB_DEVICE_CMD_CONFIG,
...
dprintf("[R %d %d]", aEndpoint, iMaxLen);
    ...
    ILPC43xx_USBDev_Command(p, USB_ENDPOINT_CMD_SELECT | aIndex);
    ILPC43xx_USBDev_Command(p, USB_ENDPOINT_CMD_CLEAR_BUFFER);

    return rxLength;
}

/*-------------------------------------------------------------------------*
 * Function:  LPC43xx_USBDev_Write
 *-------------------------------------------------------------------------*
 * Description:
...
dprintf("[W %d %d]", aEndpoint, aLength);
    ...
    ILPC43xx_USBDev_Command(p, USB_ENDPOINT_CMD_SELECT | aIndex);
    ILPC43xx_USBDev_Command(p, USB_ENDPOINT_CMD_VALIDATE_BUFFER);

    return aLength;
}

/*-------------------------------------------------------------------------*
 * Function:  LPC43xx_USBDev_SetStallState
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *                                     not stalled.
 *-------------------------------------------------------------------------*/
void LPC43xx_USBDev_SetStallState(
            void *aWorkspace,
            TUInt8 aEndpoint,
            TBool aStallState)
{
...
/*-------------------------------------------------------------------------*
 * Function:  LPC43xx_USBDev_IsStalled
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *-------------------------------------------------------------------------*/
TBool LPC43xx_USBDev_IsStalled(void *aWorkspace, TUInt8 aEndpoint)
{
    // Determine if the endpoint's stalled bit is set.
    return (ILPC43xx_USBDev_CommandAndRead(
                (T_LPC43xx_USBDev_Workspace *)aWorkspace,
...
/*-------------------------------------------------------------------------*
 * Function:  LPC43xx_USBDev_SetConfiguration
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *-------------------------------------------------------------------------*/
static void LPC43xx_USBDev_SetConfiguration(
                void *aWorkspace,
                TUInt8 aConfigurationIndex)
{
    T_LPC43xx_USBDev_Workspace *p_workspace =
            (T_LPC43xx_USBDev_Workspace *)aWorkspace;
...
dprintf("\n\n---> [Cf %d]", aConfigurationIndex);
    ...
    if (aConfigurationIndex == 0)  {
        ...
        LPC43xx_USBDev_DeviceIsNotConfigured(aWorkspace);
    } else {
...
//                    ((T_USBDescriptorConfiguration *)p)->iConfigurationValue;
                ...
                LPC43xx_USBDev_EndpointConfigure(
                    aWorkspace,
                    p_endpoint->iAddress,
        ...
        LPC43xx_USBDev_DeviceIsConfigured((void *)p_workspace);
    }
...
/*-------------------------------------------------------------------------*
 * Function:  LPC43xx_USBDev_GetConfiguration
 *-------------------------------------------------------------------------*
 * Description:
 *      Go through the descriptor table and enable all endpoints for
 *      the given descriptor index and alternative setting.
 ...
 *-------------------------------------------------------------------------*/
static TUInt8 LPC43xx_USBDev_GetConfiguration(
                void *aWorkspace)
{
    T_LPC43xx_USBDev_Workspace *p_workspace =
            (T_LPC43xx_USBDev_Workspace *)aWorkspace;
...
/*-------------------------------------------------------------------------*
 * Function:  LPC43xx_USBDev_ForceAddressAndConfiguration
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *-------------------------------------------------------------------------*/
void LPC43xx_USBDev_ForceAddressAndConfiguration(
            void *aWorkspace,
            TUInt8 aAddress,
            TUInt8 aConfigNum)
{
    ...
    LPC43xx_USBDev_SetAddress(aWorkspace, aAddress);
    LPC43xx_USBDev_SetConfiguration(aWorkspace, aConfigNum);
...
/*-------------------------------------------------------------------------*
 * Internal Function:  LPC43xx_USBDev_EndpointDisable
 *-------------------------------------------------------------------------*
 * Description:
 *      Disable endpoint.
 * Inputs:
...
/*-------------------------------------------------------------------------*
 * Internal Function:  LPC43xx_USBDev_EndpointRealize
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *-------------------------------------------------------------------------*/
static void LPC43xx_USBDev_EndpointRealize(
                T_LPC43xx_USBDev_Workspace *p,
                TUInt32 aIndex,
                TUInt16 aMaxPacketSize)
{
...
/*-------------------------------------------------------------------------*
 * Function:  LPC43xx_USBDev_EndpointConfigure
 *-------------------------------------------------------------------------*
 * Description:
            ...
            TUInt16 aMaxPacketSize)
{
    ...
    // realise EP
    LPC43xx_USBDev_EndpointRealize(p, aIndex, aMaxPacketSize);

    // enable EP
    LPC43xx_USBDev_EndpointEnable(
            p,
            aIndex);
}
 ...
 *      void *aWorkspace          -- USBDev Controller's workspace
 *-------------------------------------------------------------------------*/
void LPC43xx_USBDev_Initialize(void *aWorkspace)
{
...
//    LPC43xx_USBDev_InterruptNakEnable(aWorkspace, 0);
    LPC43xx_USBDev_InterruptNakEnable(aWorkspace, USB_DEVICE_SET_MODE_INAK_BI);

    // Setup interrupts for endpoints
    for (i=0; i<MAX_NUM_ENDPOINTS; i++)  {
        if (p->iSettings.iUSBDevice->iEndpointFuncs[i].iIn)
            LPC43xx_USBDev_EndpointInterruptsEnable(aWorkspace,
...
/*-------------------------------------------------------------------------*
 * Function:  LPC43xx_USBDev_GetDescriptor
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *-------------------------------------------------------------------------*/
const TUInt8 *LPC43xx_USBDev_GetDescriptor(
            void *aWorkspace,
            TUInt8 aType,
            TUInt16 aIndex,
            TUInt16 *aLength)
{
    const TUInt8 *p = ((T_LPC43xx_USBDev_Workspace *)aWorkspace)->
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const HAL_USBDeviceController LPC43xx_USBDev_Interface = {
	{
	    "NXP:LPC43xx:USB Device Controller",
	    0x0100,
	    LPC43xx_USBDev_InitializeWorkspace,
	    sizeof(T_LPC43xx_USBDev_Workspace),
	},
	
    LPC43xx_USBDev_Configure,
    LPC43xx_USBDev_Initialize,
    LPC43xx_USBDev_Connect,
    LPC43xx_USBDev_Disconnect,
    LPC43xx_USBDev_SetAddress,
    LPC43xx_USBDev_SetConfiguration,
    LPC43xx_USBDev_GetConfiguration,
    LPC43xx_USBDev_EndpointConfigure,
    LPC43xx_USBDev_Read,
    LPC43xx_USBDev_Write,
    LPC43xx_USBDev_SetStallState,
    LPC43xx_USBDev_IsStalled,
    LPC43xx_USBDev_EndpointInterruptsEnable,
    LPC43xx_USBDev_EndpointInterruptsDisable,
    LPC43xx_USBDev_ForceAddressAndConfiguration,
    LPC43xx_USBDev_ProcessEndpoints,
    LPC43xx_USBDev_GetDescriptor,
    LPC43xx_USBDev_InterruptNakEnable,
};

/*---------------------------------------------------------------------------*
 * Requirement routines:
 *---------------------------------------------------------------------------*/
void LPC43xx_USBDevice_PortA_Require(
        const T_LPC43xx_USBDevice_Settings *aSettings)
{
    static const T_LPC43xx_IOCON_ConfigList pinsVBUS[] = {
            {GPIO_P1_30, IOCON_A_DEFAULT(2)}, // USB_VBUS
    };
    static const T_LPC43xx_IOCON_ConfigList pinsDP1[] = {
            {GPIO_P0_29, IOCON_U_DEFAULT(1)}, // USB_D+1
    };
    static const T_LPC43xx_IOCON_ConfigList pinsDM1[] = {
            {GPIO_P0_30, IOCON_U_DEFAULT(1)}, // USB_D-1
    };
    static const T_LPC43xx_IOCON_ConfigList pinsCONNECT1[] = {
            {GPIO_P2_9,  IOCON_D_DEFAULT(1)}, // USB_CONNECT1
    };
    static const T_LPC43xx_IOCON_ConfigList pinsUP_LED1[] = {
    ...
    HALInterfaceRegister("USBDeviceController",
            (T_halInterface *)&LPC43xx_USBDev_Interface, 0, 0);

    LPC43xx_IOCON_ConfigPinOrNone(aSettings->iVBUS, pinsVBUS, ARRAY_COUNT(pinsVBUS));
    LPC43xx_IOCON_ConfigPin(aSettings->iDP, pinsDP1, ARRAY_COUNT(pinsDP1));
    LPC43xx_IOCON_ConfigPin(aSettings->iDM, pinsDM1, ARRAY_COUNT(pinsDM1));
    LPC43xx_IOCON_ConfigPinOrNone(aSettings->iCONNECT, pinsCONNECT1,
            ARRAY_COUNT(pinsCONNECT1));
    LPC43xx_IOCON_ConfigPinOrNone(aSettings->iUP_LED, pinsUP_LED1,
            ARRAY_COUNT(pinsUP_LED1));
}

void LPC43xx_USBDevice_PortB_Require(
        const T_LPC43xx_USBDevice_Settings *aSettings)
{
    static const T_LPC43xx_IOCON_ConfigList pinsVBUS[] = {
            {GPIO_P1_30, IOCON_A_DEFAULT(2)}, // USB_VBUS
    };
    static const T_LPC43xx_IOCON_ConfigList pinsDP2[] = {
            {GPIO_P0_31, IOCON_U_DEFAULT(1)}, // USB_D+2
    };
    static const T_LPC43xx_IOCON_ConfigList pinsCONNECT2[] = {
            {GPIO_P0_14, IOCON_D_DEFAULT(3)}, // USB_CONNECT2
    };
    static const T_LPC43xx_IOCON_ConfigList pinsUP_LED2[] = {
    ...
    LPC43xx_IOCON_ConfigPinOrNone(aSettings->iVBUS, pinsVBUS, ARRAY_COUNT(pinsVBUS));
    LPC43xx_IOCON_ConfigPin(aSettings->iDP, pinsDP2, ARRAY_COUNT(pinsDP2));
    LPC43xx_IOCON_ConfigPinOrNone(aSettings->iDM, 0, 0);
    LPC43xx_IOCON_ConfigPinOrNone(aSettings->iCONNECT, pinsCONNECT2,
