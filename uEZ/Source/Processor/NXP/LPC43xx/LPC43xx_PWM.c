 ...
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the LPC43xx PWMs Interface.
 * Implementation:
...
typedef struct {
    const HAL_PWM *iHAL;
    T_LPC43xx_PWM_Registers *iReg;
    TUInt32 iPeriod;
    TUInt32 iPWM;
} T_LPC43xx_PWM_Workspace;

static T_uezError LPC43xx_PWM_SetMatchRegister(
        void *aWorkspace,
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LPC43xx_PWM_SetMaster(
        void *aWorkspace,
        TUInt32 aPeriod,
        TUInt8 aMatchRegister)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC43xx_PWM_SetMatchRegister
 *---------------------------------------------------------------------------*
 * Description:
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC43xx_PWM_EnableSingleEdgeOutput
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LPC43xx_PWM_EnableSingleEdgeOutput(
        void *aWorkspace,
        TUInt8 aMatchRegister)
{
    T_LPC43xx_PWM_Workspace *p = (T_LPC43xx_PWM_Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC43xx_PWM_DisableOutput
 *---------------------------------------------------------------------------*
 * Description:
 *      PWM is going to have a single edge output (high until matches, then
 *      low, reset counter goes back to high).
 ...
 *      T_uezError              -- UEZ_ERROR_NONE
 *---------------------------------------------------------------------------*/
static T_uezError LPC43xx_PWM_DisableOutput(
        void *aWorkspace,
        TUInt8 aMatchRegister)
{
    T_LPC43xx_PWM_Workspace *p = (T_LPC43xx_PWM_Workspace *)aWorkspace;
 ...
 *      T_uezError              -- UEZ_ERROR_NONE
 *---------------------------------------------------------------------------*/
static T_uezError LPC43xx_PWM_SetMatchCallback(
        void *aWorkspace,
        TUInt8 aMatchRegister,
        void (*aCallback)(void *),
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC43xx_PWM_PWM0_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC43xx PWM0 workspace.
 * Inputs:
 *      void *aW                    -- Particular PWM workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC43xx_PWM_PWM0_InitializeWorkspace(void *aWorkspace)
{
    T_LPC43xx_PWM_Workspace *p = (T_LPC43xx_PWM_Workspace *)aWorkspace;
    p->iReg = (T_LPC43xx_PWM_Registers *)LPC_MCPWM_BASE;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC43xx_PWM_PWM1_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC43xx PWM0 workspace.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC43xx_PWM_PWM1_InitializeWorkspace(void *aWorkspace)
{
    T_LPC43xx_PWM_Workspace *p = (T_LPC43xx_PWM_Workspace *)aWorkspace;
    p->iReg = (T_LPC43xx_PWM_Registers *)LPC_MCPWM_BASE;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC43xx_PWM_PWM2_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC43xx PWM2 workspace.
 * Inputs:
 *      void *aW                    -- Particular PWM workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC43xx_PWM_PWM2_InitializeWorkspace(void *aWorkspace)
{
    T_LPC43xx_PWM_Workspace *p = (T_LPC43xx_PWM_Workspace *)aWorkspace;
    p->iReg = (T_LPC43xx_PWM_Registers *)LPC_MCPWM_BASE;
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const HAL_PWM PWM_LPC43xx_Port0_Interface = {
    {
    "NXP:LPC43xx:PWM0",
    0x0100,
    LPC43xx_PWM_PWM0_InitializeWorkspace,
    sizeof(T_LPC43xx_PWM_Workspace),
    },

    LPC43xx_PWM_SetMaster,
    LPC43xx_PWM_SetMatchRegister,
    LPC43xx_PWM_EnableSingleEdgeOutput,
    LPC43xx_PWM_DisableOutput,
    LPC43xx_PWM_SetMatchCallback,
};

const HAL_PWM PWM_LPC43xx_Port1_Interface = {
    {
    "NXP:LPC43xx:PWM1",
    0x0100,
    LPC43xx_PWM_PWM1_InitializeWorkspace,
    sizeof(T_LPC43xx_PWM_Workspace),
    },
...
const HAL_PWM PWM_LPC43xx_Port2_Interface = {
    {
    "NXP:LPC43xx:PWM2",
    0x0100,
    LPC43xx_PWM_PWM2_InitializeWorkspace,
    sizeof(T_LPC43xx_PWM_Workspace),
    },
...
void LPC43xx_PWM0_Require(void)
{
    ...
    HALInterfaceRegister("PWM0",
            (T_halInterface *)&PWM_LPC43xx_Port0_Interface, 0, 0);
...
void LPC43xx_PWM1_Require(void)
{
    ...
    HALInterfaceRegister("PWM1",
            (T_halInterface *)&PWM_LPC43xx_Port1_Interface, 0, 0);
