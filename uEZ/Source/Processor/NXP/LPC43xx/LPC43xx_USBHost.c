/*-------------------------------------------------------------------------*
 * File:  LPC43xx_USBHost.c
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the LPC43xx USBHost Interface as pulled
...
static T_LPC43xx_USBHost_Workspace *G_usbHostWorkspacePortA = 0;
static T_LPC43xx_USBHost_Workspace *G_usbHostWorkspacePortB = 0;
...
static void Host_Isr_Process (T_LPC43xx_USBHost_Workspace *p)
{
    TUInt32   int_status;
    TUInt32   ie_status;
...
void LPC43xx_InterruptProcess_USBHost(void)
{
...
IRQ_ROUTINE(USB1_IRQn)
{
    ...
    LPC43xx_InterruptProcess_USBHost();
 ...
 *      Fill a setup packet
 * Inputs:
 *      T_LPC43xx_USBHost_Workspace *p -- USB Host controller workspace
...
{
    T_LPC43xx_USBHost_Workspace *p = (T_LPC43xx_USBHost_Workspace *)aWorkspace;
    return IAllocBuffer(p, aSize);
}

/*---------------------------------------------------------------------------*
 * Routine:  LPC43xx_USBHost_InitializeWorkspace_PortA
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LPC43xx_USBHost_InitializeWorkspace_PortA(void *aWorkspace)
{
    T_LPC43xx_USBHost_Workspace *p = (T_LPC43xx_USBHost_Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC43xx_USBHost_InitializeWorkspace_PortB
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                  -- Any error if failed
 *---------------------------------------------------------------------------*/
static T_uezError LPC43xx_USBHost_InitializeWorkspace_PortB(void *aWorkspace)
{
    T_LPC43xx_USBHost_Workspace *p = (T_LPC43xx_USBHost_Workspace *)aWorkspace;
 ...
 *---------------------------------------------------------------------------*
 * Description:
 *      Initializes the LPC43xx host controller with buffers
 * Inputs:
 *      void *aWorkspace            -- USB Host controller workspace
 *---------------------------------------------------------------------------*/
...
const HAL_USBHost G_LPC43xx_USBHost_Interface_PortA = {
    // T_halInterface iInterface
    {
        "LPC43xx:USBHost:PortA",
        0x0100,
        LPC43xx_USBHost_InitializeWorkspace_PortA,
        sizeof(T_LPC43xx_USBHost_Workspace),
    },
...
const HAL_USBHost G_LPC43xx_USBHost_Interface_PortB = {
    // T_halInterface iInterface
    {
        "LPC43xx:USBHost:PortB",
        0x0100,
        LPC43xx_USBHost_InitializeWorkspace_PortB,
        sizeof(T_LPC43xx_USBHost_Workspace),
    },
 ...
 *---------------------------------------------------------------------------*/
//void LPC43xx_USBHost_VBUS_Require(T_uezGPIOPortPin aPin)
//{
//    static const T_LPC43xx_IOCON_ConfigList pins[] = {
//            {GPIO_P1_30, IOCON_A_DEFAULT(2)}, // USB_VBUS
//    };
//    HAL_DEVICE_REQUIRE_ONCE();
//    LPC43xx_IOCON_ConfigPinOrNone(aPin, pins, ARRAY_COUNT(pins));
//
//}
void LPC43xx_USBHost_PortA_Require(const T_LPC43xx_USBHost_Settings *aSettings)
{
    //USB0
    ...
    HALInterfaceRegister("USBHost:PortA",
            (T_halInterface *)&G_LPC43xx_USBHost_Interface_PortA, 0, 0);
}

void LPC43xx_USBHost_PortB_Require(const T_LPC43xx_USBHost_Settings *aSettings)
{
    //USB1
    HAL_DEVICE_REQUIRE_ONCE();
    // Register USB Host driver ports for LPC24xx
