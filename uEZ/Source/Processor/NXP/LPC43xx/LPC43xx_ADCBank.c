 ...
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the LPC43xx ADC Bus Interface.
 * Implementation:
...
} T_LPC43xx_ADC_Registers;

typedef struct {
    const HAL_ADCBank *iHAL;
    T_LPC43xx_ADC_Registers *iReg;
 ...
 * Globals:
 *---------------------------------------------------------------------------*/
T_LPC43xx_ADC_Workspace *G_adc0Workspace = 0;
T_LPC43xx_ADC_Workspace *G_adc1Workspace = 0;

/*---------------------------------------------------------------------------*
 * Routine:  ILPC43xx_ADC_ADC0_ISR
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC43xx ADC0 workspace.
 * Inputs:
 *      T_LPC43xx_ADC_Workspace *aW -- ADC Workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
static void ILPC43xx_ADC_ADC_ISR(T_LPC43xx_ADC_Workspace *p)
{
    TVUInt32 *p_dregs;
//    TVUInt32 stats;
    TUInt32 gdr;
    ...
    // The bit size is always 12 bits from the LPC43xx, but
 ...
 * Outputs:
 *      T_LPC43xx_ADC_Workspace *aW -- Particular ADC workspace
 *      ADC_RequestSingle *aRequest -- Configuration of ADC device
 *---------------------------------------------------------------------------*/
static void IADCConfig(T_LPC43xx_ADC_Workspace *p, ADC_RequestSingle *aRequest)
{
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError ADC_LPC43xx_RequestSingle(
        void *aWorkspace,
...
/*---------------------------------------------------------------------------*
 * Interrupt Routine:  ILPC43xx_ADC_ADC0_Interrupt
 *---------------------------------------------------------------------------*
 * Description:
 *      Catch ADC0 interrupt and use the ISR routine for it
 *---------------------------------------------------------------------------*/
IRQ_ROUTINE(ILPC43xx_ADC_ADC0_Interrupt)
{
    IRQ_START();
    ILPC43xx_ADC_ADC_ISR(G_adc0Workspace);
    IRQ_END();
}

/*---------------------------------------------------------------------------*
 * Interrupt Routine:  ILPC43xx_ADC_ADC1_Interrupt
 *---------------------------------------------------------------------------*
 * Description:
 *      Catch ADC0 interrupt and use the ISR routine for it
 *---------------------------------------------------------------------------*/
IRQ_ROUTINE(ILPC43xx_ADC_ADC1_Interrupt)
{
    IRQ_START();
    ILPC43xx_ADC_ADC_ISR(G_adc1Workspace);
    IRQ_END();
}

T_uezError ADC_LPC43xx_Configure_ADC0(
        void *aWorkspace,
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC43xx_ADC_ADC0_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC43xx ADC0 workspace.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC43xx_ADC_ADC0_InitializeWorkspace(void *aWorkspace)
{
    T_LPC43xx_ADC_Workspace *p = (T_LPC43xx_ADC_Workspace *)aWorkspace;
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC43xx_ADC_ADC1_InitializeWorkspace(void *aWorkspace)
{
    T_LPC43xx_ADC_Workspace *p = (T_LPC43xx_ADC_Workspace *)aWorkspace;
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const HAL_ADCBank ADC_LPC43xx_ADC0_Interface = {
        {
        "ADC:LPC43xx:ADC0",
        0x0100,
        LPC43xx_ADC_ADC0_InitializeWorkspace,
        sizeof(T_LPC43xx_ADC_Workspace),
        },

        ADC_LPC43xx_Configure_ADC0,
        ADC_LPC43xx_RequestSingle, };

const HAL_ADCBank ADC_LPC43xx_ADC1_Interface = {
        {
        "ADC:LPC43xx:ADC1",
        0x0100,
        LPC43xx_ADC_ADC1_InitializeWorkspace,
        sizeof(T_LPC43xx_ADC_Workspace),
        },
...
void LPC43xx_ADC0_Require(void)
{
    ...
    HALInterfaceRegister("ADC0", (T_halInterface *)&ADC_LPC43xx_ADC0_Interface,
            0, 0);
}
...
{
    HAL_DEVICE_REQUIRE_ONCE();
    HALInterfaceRegister("ADC1", (T_halInterface *)&ADC_LPC43xx_ADC1_Interface,
            0, 0);
}

void LPC43xx_ADC0_0_Require(T_uezGPIOPortPin aPin)
{
    static const T_LPC43xx_SCU_ConfigList adc0_0[] = {
    ...
    LPC43xx_SCU_ConfigPinOrNone(aPin, adc0_0, ARRAY_COUNT(adc0_0));
    if(aPin != GPIO_NONE){
        LPC_SCU->ENAIO0 |= (1 << 0);
    }
}
...
{
    static const T_LPC43xx_SCU_ConfigList adc0_1[] = {
    ...
    LPC43xx_ADC0_Require();
    LPC43xx_SCU_ConfigPinOrNone(aPin, adc0_1, ARRAY_COUNT(adc0_1));
    if(aPin != GPIO_NONE){
        LPC_SCU->ENAIO0 |= (1 << 1);
    }
}
...
{
    static const T_LPC43xx_SCU_ConfigList adc0_2[] = {
    ...
    LPC43xx_ADC0_Require();
    LPC43xx_SCU_ConfigPinOrNone(aPin, adc0_2, ARRAY_COUNT(adc0_2));
    if(aPin != GPIO_NONE){
        LPC_SCU->ENAIO0 |= (1 << 2);
    }
}
...
{
    static const T_LPC43xx_SCU_ConfigList adc0_3[] = {
    ...
    LPC43xx_ADC0_Require();
    LPC43xx_SCU_ConfigPinOrNone(aPin, adc0_3, ARRAY_COUNT(adc0_3));
    if(aPin != GPIO_NONE){
        LPC_SCU->ENAIO0 |= (1 << 3);
    }
}
...
{
    static const T_LPC43xx_SCU_ConfigList adc0_4[] = {
    ...
    LPC43xx_ADC0_Require();
    LPC43xx_SCU_ConfigPinOrNone(aPin, adc0_4, ARRAY_COUNT(adc0_4));
    if(aPin != GPIO_NONE){
        LPC_SCU->ENAIO0 |= (1 << 4);
    }
}
...
{
    static const T_LPC43xx_SCU_ConfigList adc0_5[] = {
    ...
    LPC43xx_ADC0_Require();
    LPC43xx_SCU_ConfigPinOrNone(aPin, adc0_5, ARRAY_COUNT(adc0_5));
    if(aPin != GPIO_NONE){
        LPC_SCU->ENAIO0 |= (1 << 5);
    }
}
...
{
    static const T_LPC43xx_SCU_ConfigList adc0_6[] = {
    ...
    LPC43xx_ADC0_Require();
    LPC43xx_SCU_ConfigPinOrNone(aPin, adc0_6, ARRAY_COUNT(adc0_6));
    if(aPin != GPIO_NONE){
        LPC_SCU->ENAIO0 |= (1 << 6);
    }
}
...
{
    static const T_LPC43xx_SCU_ConfigList adc0_7[] = {
    ...
    LPC43xx_ADC0_Require();
    LPC43xx_SCU_ConfigPinOrNone(aPin, adc0_7, ARRAY_COUNT(adc0_7));
    if(aPin != GPIO_NONE){
        LPC_SCU->ENAIO0 |= (1 << 6);
    }
}
...
{
    static const T_LPC43xx_SCU_ConfigList adc1_0[] = {
    ...
    LPC43xx_SCU_ConfigPinOrNone(aPin, adc1_0, ARRAY_COUNT(adc1_0));
    if(aPin != GPIO_NONE){
        LPC_SCU->ENAIO1 |= (1 << 0);
    }
}
...
{
    static const T_LPC43xx_SCU_ConfigList adc1_1[] = {
    ...
    LPC43xx_ADC1_Require();
    LPC43xx_SCU_ConfigPinOrNone(aPin, adc1_1, ARRAY_COUNT(adc1_1));
    if(aPin != GPIO_NONE){
        LPC_SCU->ENAIO1 |= (1 << 1);
    }
}
...
{
    static const T_LPC43xx_SCU_ConfigList adc1_2[] = {
    ...
    LPC43xx_ADC1_Require();
    LPC43xx_SCU_ConfigPinOrNone(aPin, adc1_2, ARRAY_COUNT(adc1_2));
    if(aPin != GPIO_NONE){
        LPC_SCU->ENAIO1 |= (1 << 2);
    }
}
...
{
    static const T_LPC43xx_SCU_ConfigList adc1_3[] = {
    ...
    LPC43xx_ADC1_Require();
    LPC43xx_SCU_ConfigPinOrNone(aPin, adc1_3, ARRAY_COUNT(adc1_3));
    if(aPin != GPIO_NONE){
        LPC_SCU->ENAIO1 |= (1 << 3);
    }
}
...
{
    static const T_LPC43xx_SCU_ConfigList adc1_4[] = {
    ...
    LPC43xx_ADC1_Require();
    LPC43xx_SCU_ConfigPinOrNone(aPin, adc1_4, ARRAY_COUNT(adc1_4));
    if(aPin != GPIO_NONE){
        LPC_SCU->ENAIO1 |= (1 << 4);
    }
}
...
{
    static const T_LPC43xx_SCU_ConfigList adc1_5[] = {
    ...
    LPC43xx_ADC1_Require();
    LPC43xx_SCU_ConfigPinOrNone(aPin, adc1_5, ARRAY_COUNT(adc1_5));
    if(aPin != GPIO_NONE){
        LPC_SCU->ENAIO1 |= (1 << 5);
    }
}
...
{
    static const T_LPC43xx_SCU_ConfigList adc1_6[] = {
    ...
    LPC43xx_ADC1_Require();
    LPC43xx_SCU_ConfigPinOrNone(aPin, adc1_6, ARRAY_COUNT(adc1_6));
    if(aPin != GPIO_NONE){
        LPC_SCU->ENAIO1 |= (1 << 6);
    }
}
...
{
    static const T_LPC43xx_SCU_ConfigList adc1_7[] = {
    ...
    LPC43xx_ADC1_Require();
    LPC43xx_SCU_ConfigPinOrNone(aPin, adc1_7, ARRAY_COUNT(adc1_7));
    if(aPin != GPIO_NONE){
        LPC_SCU->ENAIO1 |= (1 << 7);
    }
}
