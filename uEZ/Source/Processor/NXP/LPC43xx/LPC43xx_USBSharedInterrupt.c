 ...
 * Function Prototypes:
 *---------------------------------------------------------------------------*/
extern void LPC43xx_InterruptProcess_USBDevice(void);
extern void LPC43xx_InterruptProcess_USBHost(void);

/*---------------------------------------------------------------------------*
 * Routine:  ILPC43xx_USB_InterruptVector
 *---------------------------------------------------------------------------*
 * Description:
 *      Interrupt Vector routine that handles both USB device and USB host
 *      (if they are compiled in)
 *---------------------------------------------------------------------------*/
IRQ_ROUTINE(ILPC43xx_USB_InterruptVector)
{
    IRQ_START();

    // Chain together these functions, one for the device, one for the host
    if (LPC_USB->USBClkSt & (1<<1))
        LPC43xx_InterruptProcess_USBDevice();
