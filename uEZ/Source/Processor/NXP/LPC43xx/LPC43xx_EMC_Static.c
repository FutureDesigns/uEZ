...
#include <Source/Processor/NXP/LPC43xx/LPC43xx_ExternalBus.h>
#include <uEZBSP.h>
...
} T_LPC43xx_EMC_Static_Regs;

/*-------------------------------------------------------------------------*
 * Prototypes:
 *-------------------------------------------------------------------------*/
void LPC43xx_EMC_Static_Init(const T_LPC43xx_EMC_Static_Configuration *aConfig)
{
    T_LPC43xx_EMC_Static_Regs *emc;
    TUInt8 numAddrBits;
    TUInt32 remaining;

    if (aConfig->iBaseAddress == 0x80000000) {
        emc = (T_LPC43xx_EMC_Static_Regs *)&LPC_EMC->StaticConfig0;
        /* CSN[0] @ P4.30 */
        LPC_IOCON->P4_30 = (LPC_IOCON->P4_30 & ~7) | 1;
    } else if (aConfig->iBaseAddress == 0x90000000) {
        emc = (T_LPC43xx_EMC_Static_Regs *)&LPC_EMC->StaticConfig1;
        /* CSN[1] @ P4.31 */
        LPC_IOCON->P4_31 = (LPC_IOCON->P4_31 & ~7) | 1;
    } else if (aConfig->iBaseAddress == 0x98000000) {
        emc = (T_LPC43xx_EMC_Static_Regs *)&LPC_EMC->StaticConfig2;
        /* CSN[2] @ P2.14 */
        LPC_IOCON->P2_14 = (LPC_IOCON->P2_14 & ~7) | 1;
    } else {
        emc = (T_LPC43xx_EMC_Static_Regs *)&LPC_EMC->StaticConfig3;
        /* CSN[3] @ P2.15 */
    ...
    LPC43xx_ExternalAddressBus_ConfigureIOPins(numAddrBits);
...
/*-------------------------------------------------------------------------*
 * End of File:  LPC43xx_EMC_Static.c
 *-------------------------------------------------------------------------*/
