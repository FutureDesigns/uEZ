 ...
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the RX63N EMAC (Ethernet) Driver.
 * Implementation:
 *
 *-------------------------------------------------------------------------*/
...
#include "RX63N_EMAC.h"

/*---------------------------------------------------------------------------*
 * Options:
 *---------------------------------------------------------------------------*/
#ifndef DEBUG_RX63N_EMAC_STARTUP
#define DEBUG_RX63N_EMAC_STARTUP  0
#endif

#ifndef DEBUG_RX63N_EMAC_RX_DESC
#define DEBUG_RX63N_EMAC_RX_DESC  0
#endif
...
/* RX63N Descriptor and buffer configuration */
...
} T_RX63N_EMAC_Workspace;

/*---------------------------------------------------------------------------*
 * Globals:
 *---------------------------------------------------------------------------*/
extern const HAL_EMAC EMAC_RX63N_Interface;
 ...
 * Internal functions
 */
#if DEBUG_RX63N_EMAC_RX_DESC
static void IOutputDescriptors(T_RX63N_EMAC_Workspace *p);
#endif
...
/*
static void _phy_mii_write_1_old(void)
{
    TUInt32 j;
...
/*
static void _phy_mii_write_0_old(void)
{
    TUInt32 j;
 ...
 * Inputs:
 *      T_RX63N_EMAC_Workspace *p -- EMAC's Workspace
 *      TUInt32 aPhyReg             -- PHY register to write
 *      TUInt32 aValue              -- Value to write to register
 *---------------------------------------------------------------------------*/
static void IPHYWrite(T_RX63N_EMAC_Workspace *p, TUInt8 PhyReg, TUInt32 Value)
{
#if 0
    TUInt32 timeout;
 ...
 *      TUInt32 aValue              -- Value to write to register
 *---------------------------------------------------------------------------*/
static TUInt16 IPHYRead(T_RX63N_EMAC_Workspace *p, TUInt8 PhyReg)
{
 ...
 *      T_RX63N_EMAC_Workspace *p -- EMAC's Workspace
 *---------------------------------------------------------------------------*/
static T_uezError IEMACConfigPHY_National(T_RX63N_EMAC_Workspace *p)
{
 ...
 *      T_RX63N_EMAC_Workspace *p -- EMAC's Workspace
 *---------------------------------------------------------------------------*/
static T_uezError IEMACConfigPHY_NationalDSP83640(T_RX63N_EMAC_Workspace *p)
{
    T_uezError error = UEZ_ERROR_NONE;
    TUInt32 timeout, regv;
 ...
 *      T_RX63N_EMAC_Workspace *p -- EMAC's Workspace
 *---------------------------------------------------------------------------*/
static T_uezError IEMACConfigPHY_Micrel(T_RX63N_EMAC_Workspace *p)
{
    T_uezError error = UEZ_ERROR_NONE;
    TUInt32 timeout, regv;
#if DEBUG_RX63N_EMAC_STARTUP
    TUInt32 reg;
#endif
    // auto negotiation, restart AN
    // RE_AN should be self cleared
...
/*---------------------------------------------------------------------------*
 * Routine:  RX63N_EMAC_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
T_uezError RX63N_EMAC_InitializeWorkspace(void *aWorkspace)
{
    T_RX63N_EMAC_Workspace *p = (T_RX63N_EMAC_Workspace *)aWorkspace;
 ...
 *      T_uezError                 -- Error code
 *---------------------------------------------------------------------------*/
T_uezError RX63N_EMAC_Configure(void *aWorkspace, T_EMACSettings *aSettings)
{
    T_uezError error = UEZ_ERROR_NONE;
    T_RX63N_EMAC_Workspace *p = (T_RX63N_EMAC_Workspace *)aWorkspace;
...
// TODO: Only supporting RMII Mode currently
    ...
    EDMAC.RDLAR = (unsigned long)p->le0.rxcurrent; /* initialize Rx Descriptor List Address */
    EDMAC.TDLAR = (unsigned long)p->le0.txcurrent; /* initialize Tx Descriptor List Address */
...
static void IFrameError(T_RX63N_EMAC_Workspace *p)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  RX63N_EMAC_CheckFrameReceived
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
TBool RX63N_EMAC_CheckFrameReceived(void *aWorkspace)
{
    T_RX63N_EMAC_Workspace *p = (T_RX63N_EMAC_Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  RX63N_EMAC_StartReadFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
TUInt16 RX63N_EMAC_StartReadFrame(void *aWorkspace)
{
    T_RX63N_EMAC_Workspace *p = (T_RX63N_EMAC_Workspace *)aWorkspace;
    TUInt32 temp_size;
    ethfifo *p_current;
...
/*---------------------------------------------------------------------------*
 * Routine:  RX63N_EMAC_EndReadFrame
 *---------------------------------------------------------------------------*
 * Description:
 *      End the last frame.
 * Inputs:
 *      void *aWorkspace          -- EMAC Controller's workspace
 *---------------------------------------------------------------------------*/
void RX63N_EMAC_EndReadFrame(void *aWorkspace)
{
#if DEBUG_RX63N_EMAC_RX_DESC
    IOutputDescriptors((T_RX63N_EMAC_Workspace *)aWorkspace);
#endif
    // Do nothing in this driver
}

/*---------------------------------------------------------------------------*
 * Routine:  RX63N_EMAC_RequestSend
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      void *aWorkspace          -- EMAC Controller's workspace
 *---------------------------------------------------------------------------*/
void RX63N_EMAC_RequestSend(void *aWorkspace)
{
    // Do nothing for now
}

/*---------------------------------------------------------------------------*
 * Routine:  RX63N_EMAC_DoSend
 *---------------------------------------------------------------------------*
 * Description:
 *      Send the frame
 * Inputs:
 ...
 *---------------------------------------------------------------------------*/
void RX63N_EMAC_DoSend(void *aWorkspace, TUInt16 aFrameSize)
{
    // Do nothing for now (it really happens in CopyToFrame)
}

/*---------------------------------------------------------------------------*
 * Routine:  RX63N_EMAC_CopyToFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
void RX63N_EMAC_CopyToFrame(void *aWorkspace, void *aSource, TUInt32 aSize)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  RX63N_EMAC_CopyFromFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const HAL_EMAC RX63N_EMAC_Interface = {
        "RX63N:EMAC",
        0x0100,
        RX63N_EMAC_InitializeWorkspace,
        sizeof(T_RX63N_EMAC_Workspace),

        RX63N_EMAC_Configure,
        RX63N_EMAC_CheckFrameReceived,
        RX63N_EMAC_StartReadFrame,
        RX63N_EMAC_EndReadFrame,
        RX63N_EMAC_RequestSend,
        RX63N_EMAC_DoSend,
        RX63N_EMAC_CopyToFrame,
        RX63N_EMAC_CopyFromFrame,
};

/*---------------------------------------------------------------------------*
 * Requirement routines:
 *---------------------------------------------------------------------------*/
void RX63N_100_EMAC_RMII_Require()
{	
	static const T_RX63N_MPC_ConfigList pins[] = {
		{GPIO_PA5, 0x11},	// ET_LINKSTA
		{GPIO_PA3, 0x11},	// ET_MDIO
		{GPIO_PA4, 0x11},	// ET_MDC
		{GPIO_PB0, 0x12},	// RMII_RXD1
		{GPIO_PB1, 0x12},	// RMII_RXD0
		{GPIO_PB2, 0x12},	// REF50CK
		{GPIO_PB3, 0x12},	// RMII_RX_ER
		{GPIO_PB4, 0x12},	// RMII_TXD_EN
		{GPIO_PB5, 0x12},	// RMII_TXD0
		{GPIO_PB6, 0x12},	// RMII_TXD1
		{GPIO_PB7, 0x12},	// RMII_CRS_DV
    };

    HAL_DEVICE_REQUIRE_ONCE();
    HALInterfaceRegister("EMAC", (T_halInterface *)&RX63N_EMAC_Interface, 0, 0);
 ...
 * Requirement routines:
 *---------------------------------------------------------------------------*/
void RX63N_176_EMAC_RMII_Require()
{
	static const T_RX63N_MPC_ConfigList pins[] = {
		{GPIO_P54, 0x11},	// ET_LINKSTA
		{GPIO_P71, 0x11},	// ET_MDIO
		{GPIO_P72, 0x11},	// ET_MDC
		{GPIO_P74, 0x12},	// RMII_RXD1
		{GPIO_P75, 0x12},	// RMII_RXD0
		{GPIO_P76, 0x12},	// REF50CK
		{GPIO_P77, 0x12},	// RMII_RX_ER
		{GPIO_P80, 0x12},	// RMII_TXD_EN
		{GPIO_P81, 0x12},	// RMII_TXD0
		{GPIO_P82, 0x12},	// RMII_TXD1
		{GPIO_PB7, 0x12},	// RMII_CRS_DV
    };
