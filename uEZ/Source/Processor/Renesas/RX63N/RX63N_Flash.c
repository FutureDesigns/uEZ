...
uint32_t block_addresses(uint32_t block)
{
    if(block < BLOCK_DB0)
        return rom_block_addresses[block];
    else
        return DF_START_ADDR + (DF_BLOCK_SIZE * (block-BLOCK_DB0));
}
...
static uint8_t _Enter_PE_Mode(uint32_t flash_addr, uint16_t bytes)
{
    ...
    if ((FLASH.FSTATR0.BIT.ILGLERR == 1) || (FLASH.FSTATR0.BIT.ERSERR == 1)
            || (FLASH.FSTATR0.BIT.PRGERR == 1) || (FLASH.FSTATR1.BIT.FCUERR
            == 1)) {
...
uint8_t R_FlashWrite(uint32_t flash_addr, uint32_t buffer_addr, uint16_t bytes)
{
    ...
    /* Check for ROM area */
    else if ((flash_addr >= ROM_PE_ADDR) && (flash_addr < ROM_PE_ADDR
            + ROM_SIZE)) {
...
uint8_t R_Flash_Status_Check(uint32_t wait_cnt)
{
 ...
 *                 dataflash, at the given location. If the
 *                 number of bytes to write to flash is not a multiple of 8, the
 *                 data is padded with null bytes (0x00) to make up to the
 *                 nearest multiple of 8.
 * Argument     : offset -- Offset byte from start of flash
 *                aData -- Pointer to bytes to write to flash
 *                aSize -- number of bytes to write to flash
 * Return value : 0 = success, else failure
 *******************************************************************************/
uint8_t Flash_Write(uint16_t offset, uint8_t *aData, uint16_t aSize)
{
	uint16_t num_bytes;
    ...
    gFlashWriteAddr = (uint32_t)(DF_START_ADDR + offset);
    flash_ptr = (uint32_t *)gFlashWriteAddr;

    /* Declare the number of bytes variable, and initialise with the number of
     bytes the variable gFlashWriteBuffer contains */
    num_bytes = aSize; // sizeof(gFlashWriteBuffer);
    ...
    memcpy((char*)pad_buffer, (char*)aData, num_bytes);
    ...
    ret |= memcmp(aData, flash_ptr, aSize);

    /* Halt in while loop when flash API errors detected */
    return ret;
}

uint8_t Flash_Read(uint16_t offset, uint8_t *aData, uint16_t aSize)
{
    uint8_t * flash_ptr;

    gFlashWriteAddr = (uint32_t)(DF_START_ADDR + offset);
    flash_ptr = (uint8_t *)gFlashWriteAddr;
    memcpy(aData, flash_ptr, aSize);
 ...
 * Outline      : Flash_Erase
 * Description  : This function enters a for loop, and erases blocks of data
 *                 flash memory that cover the given address range.  This
 *                 routine is not smart enough to keep the existing data
 *                 in a block that is erased.
 * Argument     : none
 * Return value : none
 *******************************************************************************/
uint8_t Flash_Erase(uint16_t offset, uint16_t aSize)
{
    /* Declare flash API error flag */
    uint8_t ret = 0;
    /* Declare loop count and block count variables */
    ...
    uint32_t offset_address = offset + block_addresses(BLOCK_DB0);
    ...
    for (loop_counter = 0; loop_counter < DF_NUM_BLOCKS; loop_counter++) {
        /* Fetch beginning address of DF block */
        address = block_addresses(BLOCK_DB0 + loop_counter);
        end_address = address + DF_BLOCK_SIZE-1;

        /* Does this address touch the range we are covering? */
        if (((address >= offset_address) && (address <= (offset_address + aSize)))
                || ((end_address >= offset_address)
                        && (end_address <= (offset_address + aSize)))) {
            /* Erase data flash block */
            ret |= R_FlashErase(block_count);
