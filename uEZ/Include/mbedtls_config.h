...
/***********************************************************************************
 * If you define MBEDTLS_CONFIG_FILE in the library build preprocessor macro, 
 * you can change the include path to point to a different copy of this file, 
 * then just rebuild the uEZ library. 
 *
 * FDI will have a goal of trying to enable every feature that will work on 
 * Cortex-M4 or above and that isn't PC-specfic. We will target features that 
 * are required for network stack software to work. (and probably only test those)
 ***********************************************************************************/
...
// PSA interface required for TLS 1.3. Future MbedTLS will require PSA interface to be used.
// Note that support can be included, but you can force TLS 1.2 at run time if needed.
// Search FORCE_TLS_12_EVEN_IF_13_SUPPORTED for run time disable example.
#define ENABLE_TLS_1P3_AND_OPTIONS 1 // Set to 0 to disable TSL 1.3. 
 ...
 */
//#define MBEDTLS_ECP_RESTARTABLE // From the last test on a previous release this caused handshake fail.
...
#ifdef DEBUG
// This requires MBEDTLS_MEMORY_BUFFER_ALLOC_C
//#define MBEDTLS_MEMORY_DEBUG // enable printf on allocation failure.
#else //Release build NDEBUG
 ...
 */
#define MBEDTLS_PKCS1_V21 // required for AWS demo
 ...
 */
#ifdef DEBUG
#define MBEDTLS_SSL_DEBUG_ALL // enable for a bit
#else //Release build NDEBUG
 ...
 *
 */
#define MBEDTLS_SSL_RENEGOTIATION // This may be causing issues with AWS or required for AWS?
 ...
 */
#if (ENABLE_TLS_1P3_AND_OPTIONS == 1)
#define MBEDTLS_SSL_PROTO_TLS1_3 // Requires PSA interface
#endif
 ...
 *
 */
#define MBEDTLS_SSL_TLS1_3_COMPATIBILITY_MODE // ChangeCipherSpec invalid in TLS 1.3 without compatibility mode, recieved from AWS 
 ...
 *
 */
//#define MBEDTLS_SSL_EARLY_DATA // use defualt setting
 ...
 */
#define MBEDTLS_USE_PSA_CRYPTO // REQUIRED for Future MbedTLS 4.0 and above. REQUIRED for TLS 1.3.
 ...
 */
//#define MBEDTLS_X509_TRUSTED_CERTIFICATE_CALLBACK // Note might want this feature.
 ...
 */
#define MBEDTLS_X509_RSASSA_PSS_SUPPORT // required for AWS demo
 ...
 */
//#define MBEDTLS_PSA_CRYPTO_STORAGE_C // we might want this
 ...
 */
//#define MBEDTLS_PSA_ITS_FILE_C // we might want this
 ...
 * absolute or relative to a directory on the include search path.
 */
#define MBEDTLS_PSA_CRYPTO_CONFIG_FILE "mbedtls_crypto_config.h"
...
#include <port/time_alt.h> // always use time_alt then include time.h from there only

#include <uEZ.h> // required for the below FreeRTOS functions that we use, but not required for time_alt.
...
// Cannot use these and the ones below. Must pick a set.
//#define MBEDTLS_PLATFORM_STD_PRINTF            DEBUG_RTT_Printf0
...
#define MBEDTLS_PLATFORM_CALLOC_MACRO           pvPortCalloc // use configUSE_MALLOC_FAILED_HOOK with vApplicationMallocFailedHook otherwise you may not see an error
#define MBEDTLS_PLATFORM_PRINTF_MACRO           DEBUG_RTT_Printf0
...
#define MBEDTLS_PLATFORM_TIME_MACRO            get_system_rtc_time_t
//#define MBEDTLS_PLATFORM_TIME_TYPE_MACRO       time_t 

// These may not be used anywhere, but go ahead and add them as an example
#define MBEDTLS_SSL_SEND                        mbedtls_bio_socket_send
#define MBEDTLS_SSL_RECV                        mbedtls_bio_socket_recv
 ...
 * This option has no effect when #MBEDTLS_PSA_CRYPTO_C is disabled.
 */
#define MBEDTLS_PSA_KEY_SLOT_COUNT 64 // increase for use with PSA interface and TLS 1.3, default config size is now 32
 ...
 */
#define MBEDTLS_SSL_TLS1_3_TICKET_AGE_TOLERANCE 6000 // required for TLS1.3 build on 3.4+
 ...
 */
#define MBEDTLS_SSL_TLS1_3_TICKET_NONCE_LENGTH 32 // required for TLS1.3 build on 3.4+
 ...
 *
 */
#define MBEDTLS_SSL_TLS1_3_DEFAULT_NEW_SESSION_TICKETS 1 // required for TLS1.3 build on 3.4+
