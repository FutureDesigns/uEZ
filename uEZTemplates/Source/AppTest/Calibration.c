                ...
                TBool aForceCalibrate)
{
    ...
    while (1) {
        ...
        if (needCalib) {
            // Clear the screen
            CalibrateScreen();            
            // Raise Volume slightly
            UEZAudioMixerSetLevel(UEZ_AUDIO_MIXER_OUTPUT_ONBOARD_SPEAKER, 160);
        ...
        if((!needCalib) || (!aForceCalibrate)) { 
        // We already have programed calibration, and we didn't get any errors
        // loading it. So don't waste time and flash saving NVSettings
        } else {
 ...
 *
 *  @param [in] aForceCalibrate     ETrue to force calibration.
 */
 /*---------------------------------------------------------------------------*/
