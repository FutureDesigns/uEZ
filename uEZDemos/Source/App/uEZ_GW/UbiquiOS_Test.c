...
static void wlan_power_on_callback(ub_status_t status_code)
{
    ...
    {
        const uint8_t* mac = ub_network_interface_descriptor->get_mac_addr ();
        //strcpy(G_WIFI_MAC, (const char*)mac);
        ub_ustdio_sprintf(G_WIFI_MAC, "%m", ub_network_interface_descriptor->get_mac_addr ());

        ub_ustdio_printf ("WLAN power on completed successfully. MAC address=%m\n",
                          ub_network_interface_descriptor->get_mac_addr ());
...
static void bcm_wlan(void)
{
    ...
    ub_wlan_bcmfmac_args_t wlan_driver_init_args = sterling_lwb_fcc_args; //ub_wlan_bcmfmac_4343w_args;
    //const ub_wlan_scan_args_t wlan_scan_args = {
    //        .ssid = { 0 },
    //        .ssid_length = 0,
    //        .max_scan_results = 0 };
...
static void power_on_bt(void)
{
    ...
    ub_bt_bcm434x_init_args_t bt_bcm434x_init_args;
    ub_status_t status;

    memset(&bt_init_args, 0, sizeof(bt_init_args));
    memset(&bt_bcm434x_init_args, 0, sizeof(bt_bcm434x_init_args));

    bt_init_args.device_name = LOCALNAME;
    ub_bt_init(&bt_init_args, &bt_bcm434x_init_args);
