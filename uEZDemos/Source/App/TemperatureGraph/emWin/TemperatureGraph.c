...
#define     ID_EXIT_BUTTON          (GUI_ID_USER + 0x0B)
...
static const GUI_WIDGET_CREATE_INFO _iTemperatureGraphDialog[] = {
  ...
  {TEXT_CreateIndirect      , ""    , ID_TIME_DATE_TEXT , TIME_DATE_XPOS+25        , TIME_DATE_YPOS        , TIME_DATE_XSIZE       , TIME_DATE_YSIZE+20           , GUI_TA_HCENTER, 0, 0},
  ...
  {BUTTON_CreateIndirect    , "Settings"    , ID_SETTINGS_BUTTON, 0        ,UEZ_LCD_DISPLAY_HEIGHT - TEMP_HUMIDITY_YSIZE - SPACING, TEMP_HUMIDITY_XSIZE+10  , TEMP_HUMIDITY_YSIZE       , 0, 0, 0},
  {TEXT_CreateIndirect      , ""    , ID_ALERT_TEXT  , TEMP_HUMIDITY_XPOS    , SPACING , TEMP_HUMIDITY_XSIZE   , TEMP_HUMIDITY_YSIZE       , 0, 0, 0},
#if RENAME_INI
  {BUTTON_CreateIndirect     , "X"   , ID_EXIT_BUTTON    , EXIT_XPOS         , EXIT_YPOS                 , EXIT_XSIZE             , EXIT_YSIZE       , 0, 0, 0},    
#endif
};
...
#if RENAME_INI
  { ID_EXIT_BUTTON      , "X"   , GUI_GRAY      , GUI_BLACK     , &FONT     ,LAFSetupButton,    IHandleExit},
#endif
  {0},
};
...
static TBool IHandleUpdateGraph(WM_MESSAGE * pMsg, int aNCode)
{
    ...
    if(UEZTemperatureRead(G_TempSensor, &tempReading) == UEZ_ERROR_NONE){
        ...
        GRAPH_DATA_YT_AddValue(G_Humidity_Data, (tempReading - 1) * 4);//times 4 to match graph .25 factor
