...
typedef TUInt32 datum;     /* Set the data bus width to 32 bits. */
 ...
 *
 * Notes:       Requires optimization to be turned off.
 *
...
void FuncTestSDRAM(const T_testAPI *aAPI, T_testData *aData, TUInt16 aButton)
{
    ...
    if (aButton == OPT_INIT) 
    {
        ...
        /* Start with Data Bus test */
        RTOS_ENTER_CRITICAL();
        status = memTestDataBus((volatile datum *)UEZBSP_SDRAM_BASE_ADDR);
        RTOS_EXIT_CRITICAL();
        
        if(status == 0)
        {
            RTOS_ENTER_CRITICAL();
            /* Only perform the Address Bus test if the Data bus test passes */
            status = (TUInt32)memTestAddressBus((volatile datum *)UEZBSP_SDRAM_BASE_ADDR, UEZBSP_SDRAM_SIZE);
            RTOS_EXIT_CRITICAL();
        }
        ...
        {
            RTOS_ENTER_CRITICAL();
            /* Only perform the Device test if both the Data bus and Address bus test passes */
            status = (TUInt32)memTestDevice((volatile datum *)UEZBSP_SDRAM_BASE_ADDR, UEZBSP_SDRAM_SIZE);
            RTOS_EXIT_CRITICAL();
        }
        ...
        if(status == 0)
        {
            ...
            aAPI->iShowResult(aData, 0, TEST_RESULT_PASS, 0);
            aAPI->iSetTestResult(aData, TEST_RESULT_PASS);
            sprintf(aData->iResultValue, "0x%08X", UEZBSP_SDRAM_SIZE);
        }
        else
        {
            /* SDRAM test failed */
            aAPI->iTextLine(aData, 1, "  Failed SDRAM Test!");
            aAPI->iShowResult(aData, 0, TEST_RESULT_FAIL, 0);
            aAPI->iSetTestResult(aData, TEST_RESULT_FAIL);
            sprintf(aData->iResultValue, "0x%8x", status);
        }
#else
...
//    { FCTPIN_P5_4,          FCTPIN_P0_25,               "P5[4] -> P0[25]"},
//    { FCTPIN_P5_3,          FCTPIN_P0_12,               "P5[3] -> P0[12]"},
//    { FCTPIN_P5_2,          FCTPIN_P0_13,               "P5[2] -> P0[13]"}, // Cannot test the used USB pin on 43WQN! P0_13 untested.
//    { FCTPIN_P1_12,         FCTPIN_P0_0,                "P1[12] -> P0[0]"},
//    { FCTPIN_P1_11,         FCTPIN_P0_1,                "P1[11] -> P0[1]"},
//    { FCTPIN_P1_7,          FCTPIN_P1_3,                "P1[7] -> P1[3]"},
//    { FCTPIN_P1_6,          FCTPIN_P1_5,                "P1[6] -> P1[5]"},
...
//    { FCTPIN_P0_1,          FCTPIN_P1_11,               "P0[1] -> P1[11]"},
//    { FCTPIN_P0_0,          FCTPIN_P1_12,               "P0[0] -> P1[12]"},
//    { FCTPIN_P0_13,         FCTPIN_P5_2,                "P0[13] -> P5[2]"}, // Cannot test the used USB pin on 43WQN!
//    { FCTPIN_P0_12,         FCTPIN_P5_3,                "P0[12] -> P5[3]"},
//    { FCTPIN_P0_25,         FCTPIN_P5_4,                "P0[25] -> P5[4]"},
};
#endif
...
//        p->iStart = UEZTickCounterGet();
        ...
        for (pin=0; pin<ARRAY_COUNT(G_gpioMapping); pin++) {
            ...
            aAPI->iTextLine(aData, 1, p->iLine);
            UEZTaskDelay(GPIO_PIN_SETTLE_TIME);
        ...
        if (p->iState < ARRAY_COUNT(G_gpioTestArray)) {
            ...
            for (pin=0; pin<ARRAY_COUNT(G_gpioMapping); pin++) {
                ...
                if (p_test->iPinsDrive & (((TUInt64)1)<<pin)) {
                    ...
                    (*p_port)->SetPull(p_port, p_pin->iMapPin, GPIO_PULL_UP);
                }
            }
